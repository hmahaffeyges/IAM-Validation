...
## Part 23: Tissue-class CHK-3.1A floor specification (added 2026-05-01 from bladder-epic v0.1)

**Source:** VAL-120 + VAL-121 + VAL-122 prereg amendments 002 (sealed 2026-05-01). LESSONS_LEARNED.md DISC-BLADDER-002. TESTING_CHECKLIST.md CHK-2.16.

### Why this section exists

CHK-3.1A is a **substrate-validity gate** — it tests whether a given β file looks like a real Illumina HM450/EPIC sesame Level 3 file (bimodal methylation distribution, low middle-fraction, near-complete CpG count). It is NOT a tissue-class gate.

Bladder-epic v0.1 surfaced a structural omission in the cookbook spec: prior cards (cardio, prostate) implicitly applied a kidney+prostate-derived f_extreme floor (≥ 0.50) and f_middle ceiling (≤ 0.12) without specifying that the floor was tissue-class-specific. Bladder mucosa has a substantially less bimodal methylation distribution than kidney/prostate solid parenchyma, and the prior floor failed on 76% of TCGA-BLCA samples — a tissue-class threshold mismatch, not a data integrity failure.

### Tissue-class brackets (canonical specification)

The CHK-3.1A floor is parameterized by tissue class. Three tissue classes have anchor calibrations:

| Tissue class | f_extreme floor | f_middle ceiling | Anchor VAL | Cohort N | Observed baseline |
|---|---|---|---|---|---|
| **Solid parenchyma** | ≥ 0.50 | ≤ 0.12 | VAL-106 | 210 | f_extreme 55.87% ± 2.44%, f_middle 7.42% ± 0.75% |
| **Mucosal / epithelial-lined-organ** | ≥ 0.387 | ≤ 0.184 | VAL-120/121/122 amendment 002 | 440 | f_extreme 47.23% ± 4.85%, f_middle 11.17% ± 2.95% |
| **Cultured-cell** | cohort-internal q1/q99 | cohort-internal q1/q99 | DISC-CARDIO-005 | varies | substrate-equivalence test required per cohort |

Tissue classes covered by precedent:

- **Solid parenchyma:** kidney (TCGA-KIRC), prostate (TCGA-PRAD, GSE269244), breast (TCGA-BRCA), liver (TCGA-LIHC), thyroid (TCGA-THCA), and other solid-tissue parenchymal cohorts.
- **Mucosal / epithelial-lined-organ:** bladder (TCGA-BLCA). **Expected to extend to:** lung airways (TCGA-LUAD, TCGA-LUSC mucosal subset), colon epithelium (TCGA-COAD, TCGA-READ), GI epithelium (TCGA-STAD, TCGA-ESCA), cervical mucosa (TCGA-CESC), bladder/urothelium other cohorts (Bryan UK NMIBC, Chen 2022 NMIBC blood EPIC).
- **Cultured-cell:** primary endothelial culture (PEC; cardio-epic VAL-109), iPSC-derived lines, sorted-cell preparations from primary tissue.

Tissue classes without precedent (require Phase B substrate calibration before Phase C scoring):

- Hematopoietic (TCGA-LAML, TCGA-DLBC) — likely distinct from solid parenchyma due to high turnover and lineage-mixture composition.
- Connective tissue (TCGA-SARC) — likely intermediate between solid parenchyma and mucosal.
- Neural tissue (TCGA-LGG, TCGA-GBM) — likely distinct due to neuron + glia composition.

### Application protocol at prereg-write time

Every card prereg specifies the tissue-class CHK-3.1A floor explicitly under `chk_3_1_thresholds_per_substrate.<substrate_id>.tissue_class`:

```json
{
  "TCGA HM450K sesame Level 3 — bladder cohort": {
    "tissue_class": "mucosal_epithelial_lined_organ",
    "anchor_val": "VAL-120/121/122 amendment 002",
    "chk_3_1a_f_extreme_min": 0.387,
    "chk_3_1a_f_middle_max": 0.184,
    "chk_3_1a_pass_rate_min": 0.75,
    "rationale": "DISC-BLADDER-002 — bladder is mucosal class; solid parenchyma floor (0.50) inappropriate"
  }
}
```

If the cohort spans a tissue class without precedent, the prereg either (a) uses the broadest known floor (currently mucosal-class 0.387/0.184) with explicit caveat, (b) pre-specifies a cohort-internal q1/q99 floor explicitly with rationale, or (c) runs a Phase B substrate calibration on a structurally-separated healthy cohort of the target tissue class first.

### Cookbook-wide retroactive task

Card preregs sealed before 2026-05-01 (cardio-epic, prostate-epic, breast-epic, lung-epic, ad-immune, hcc-epic, crc-epic, kidney-epic, cervical-epic) used the solid-parenchyma floor and that was appropriate for those tissues — no retroactive correction needed. Cards spanning mucosal cohorts (lung-epic LUAD-mucosal, crc-epic, cervical-epic) get tissue-class-floor specification added at the next card publish (additive update, no sealed VAL outcomes changed).

### Reference

CHK-2.16 in TESTING_CHECKLIST.md. DISC-BLADDER-002 in LESSONS_LEARNED.md. VAL-120/121/122 prereg amendments 002 in `Biological_Physics/validation_runs/`.


---

## Part 24: Stage 1 panel cohort-substrate coverage validation (added 2026-05-01 from bladder-epic v0.1)

**Source:** VAL-120 sealed outcome O4_STAGE1_DATA_INTEGRITY_FAILURE. LESSONS_LEARNED.md DISC-BLADDER-004. TESTING_CHECKLIST.md CHK-2.17.

### Why this section exists

Bladder-epic v0.1 surfaced that the Xu-538 Stage 1 panel — substrate-applicable on HM450 chemistry — produced 78.0% mean per-sample coverage on TCGA-BLCA and only 51.1% pass rate at the ≥80% per-sample CHK-3.1B threshold. The panel CpGs are all valid HM450 designs; the cohort-specific per-sample variation in HM450 detection-pass produced the gate failure. **Stage 1 panel transferability is cohort-specific, not platform-specific.**

### Validation procedure (at prereg-write time)

Every card prereg using a Stage 1 panel must include a cohort-substrate coverage precheck:

1. Sample 5-10 random Phase C cohort β files from the cohort manifest.
2. For each, compute per-sample panel coverage = fraction(panel CpGs present and detection-pass / panel total CpGs).
3. Compute sampled-mean coverage and sampled-q5 coverage.
4. Apply FLAG decision tree:
   - **Sampled mean ≥ 90% AND sampled q5 ≥ 80% — COHORT-VALIDATED.** Proceed with prereg.
   - **Sampled mean ≥ 90% AND sampled q5 < 80% — COHORT-FLAGGED.** Either dynamic panel-trimming, defer to validated panel, or accept flag and pre-lock O4.
   - **Sampled mean < 90% — COHORT-INCOMPATIBLE.** Defer to validated panel before scoring.

### Wave 1 calibration integration

VAL-114 (planned) calibrates a Stage 1 panel on Hannum 2013 GSE40279 n=656 healthy aging blood. Per CHK-2.17, VAL-114's protocol must include the cohort-substrate precheck for **every Phase C target cohort** the calibrated panel will be used on, not just the calibration cohort. This means VAL-114 produces both:

1. The Stage 1 panel itself (calibrated CpG set + per-class A-score formula).
2. A precheck output table indicating which Phase C cohorts the panel is COHORT-VALIDATED, COHORT-FLAGGED, or COHORT-INCOMPATIBLE for.

Cards using the calibrated panel reference VAL-114's precheck output table in their prereg block.

### Cookbook-wide retroactive task (future-when-time-permits)

Apply the precheck retroactively to existing Stage 1 cards (breast-epic VAL-060, lung-epic VAL-063, prostate-epic VAL-058 + VAL-118, ad-immune VAL-051, ccfDNA panels, etc.). Cards where the precheck would have FLAGGED are upgraded to v0.X+1 with panel-trimming or panel-replacement protocol. The retroactive expansion does not block current per-card publication.

### Reference

CHK-2.17 in TESTING_CHECKLIST.md. DISC-BLADDER-004 in LESSONS_LEARNED.md. VAL-120 outcome.md in `Biological_Physics/validation_runs/VAL-120_bladder_stage1_xu538/`.


---

**End of v2 reference document additions (Parts 23 + 24, added 2026-05-01 from bladder-epic v0.1).** All prior Parts 1-22 remain unchanged. CHK-2.16 + CHK-2.17 added to TESTING_CHECKLIST.md. DISC-BLADDER-001 through DISC-BLADDER-004 added to LESSONS_LEARNED.md (89 lines, 1407 → 1496). bladder-epic v0.1 sealed VAL-119 + VAL-120 + VAL-121 + VAL-122 published to GitHub commit 404eed3 on `hmahaffeyges/IAM-Validation` main.
