...
|---|---|---|---|
| VAL-114 | Xu-538 immune panel calibration on GSE40279 + per-cohort substrate-coverage precheck (per DISC-BLADDER-004) | (Shared Task A) | ▢ Reserved |
| VAL-119 | EpiSCORE BladderRef Phase B calibration on TCGA-KIRC + TCGA-PRAD adjacent-normal n=210 | bladder-epic v0.1 | ✅ DONE 2026-05-01 (sealed O1) |
| VAL-120 | Stage 1 Xu-538 immune red flag on TCGA-BLCA n=440 | bladder-epic v0.1 | ✅ DONE 2026-05-01 (sealed O4 — Xu-538 cohort-substrate coverage gate; diagnostic d_paired=+1.90 reported) |
| VAL-121 | Stage 2 multi-atlas on TCGA-BLCA n=440 | bladder-epic v0.1 | ✅ DONE 2026-05-01 (sealed O2 — BladderRef Epi NEGATIVE -1.46 vs Loyfer Bladder POSITIVE +1.91 dual-atlas direction divergence) |
| VAL-122 | Stage 3 immune fine-tune on TCGA-BLCA n=440 | bladder-epic v0.1 | ✅ DONE 2026-05-01 (sealed O1 — broad immune-architectural drift, all 6/6 Salas IDOL tiles POSITIVE) |
...
## ✅ Reference example #2: bladder-epic card — DONE 2026-05-01

This is the second worked example. Read the cardio reference (above) first for the canonical Phase 0/A/B/C/D/E/F template; this section captures **what changes when the cohort is mucosal-tissue substrate** rather than solid parenchyma. The bladder sprint produced **four sealed VALs** (VAL-119 + VAL-120 + VAL-121 + VAL-122) and **four DISC-BLADDER discoveries** in 36 hours of session time. The sprint is the cookbook's reference example for how prereg discipline self-corrects when a silent assumption meets data that violates it.

**Phase 0 executed (cohort survey):** Searched GEO + ArrayExpress + GDC + dbGaP + SRA for `methylation bladder cancer`, `methylation urothelial carcinoma`, `bladder cfDNA methylation`, `urine methylation marker`, `NMIBC blood methylation`, plus disease-stage-specific terms. Output: `cards/bladder_epic/cohort_survey.md` enumerated 9 plausibly-relevant cohorts. Three were selected for Phase C (TCGA-BLCA n=440 primary tumor + adjacent normal HM450 sesame Level 3, GSE52955 multi-cancer urological n=72 HM450 deferred to v0.2 cross-cohort verification, Bryan UK NMIBC + Chen 2022 NMIBC blood EPIC n=603 deferred to v0.2 blood-substrate verification). Six were deferred to v1.0+ (WHI bladder pre-diagnostic n=440+440 biobank-gated; CTUBI cfDNA-MeDIP cohort access pending DUA; etc.). Heath signed off on TCGA-BLCA selection; Phase A began.

**Phase A executed:**
- *A.1 inventory.* Card JSON `atlases_run` opened with five entries: Layered Moss+Loyfer 25-tile (calibration anchor VAL-112), EpiSCORE BladderRef CpG-bridged 4-tile (NEW; calibration anchor VAL-119), Caggiano CelFiE TIM 19-tile (calibration anchor VAL-113), Salas Blood.EPIC IDOL 6-tile (production calibrated), UniLIFE 19-tile (within-cohort self-cal v0.1; VAL-115 promotion path).
- *A.2 atlas acquisition + CHK-5.13 web-verification.* EpiSCORE BladderRef sourced from `aet21/EpiSCORE` GitHub master commit, file `mrefBladder.m` (163 Entrez Gene IDs × 4 cell types + weight). CHK-5.13 web-verified the Zhu et al. 2022 Nature Methods citation (DOI 10.1038/s41592-022-01412-7) — confirmed first-author Zhu, journal Nature Methods, year 2022.
- *A.3 bridge engineering.* Adapted the prostate VAL-117 EpiSCORE bridge template to BladderRef. Bridge methodology: source `mrefBladder.m` (163 EIDs × 4 cell types) → CpG-resolved matrix via EpiSCORE's `probeInfo450k.lv` Entrez→450K-CpG manifest (485,577 array probes; 331,229 with EID; 19,357 unique EIDs). Coverage: 158 of 163 EIDs mapped (5 unmapped: 1880, 2252, 26521, 51699, 54829). Output: 2,696 unique 450K CpGs × 4 bladder cell types after CHK-3.1C deduplication. Atlas SHA-256: `3005663b4ede4b20199bacff641952390b1434764b8cf0915cdc9d6a6c1517c6`.
- *A.4 integration testing.* CHK-3.1C dedup audit passed on bridged matrix; smoke test on calibration cohort verified no all-NaN tiles, no all-1.0 A-scores.

**Phase B executed:**

*VAL-119 — EpiSCORE BladderRef Phase B calibration on TCGA-KIRC + TCGA-PRAD adjacent-normal n=210.* Same VAL-106 calibration cohort used cookbook-wide. CHK-3.1A 206/210 (98.1%), CHK-3.1B 210/210 (q5 86.15%), CHK-3.1C 0 duplicates. Per-tile healthy-floor distributions sealed: EC mean=0.4087 sd=0.0100, **Epi mean=0.4135 sd=0.0066 q5=0.4004** (tightest within-cohort variance, operational cell-of-origin tile), Fib mean=0.4875 sd=0.0090 (separates), IC mean=0.4106 sd=0.0086. Max within-cohort tile range 0.0694, well above tissue-floor-dominated threshold 0.02. **DISC-BLADDER-001 sealed:** gene-promoter atlas family fitness depends on cell-type DISTINCTNESS, not cell-type COUNT (4-cell-type BladderRef separates more cleanly than 6-cell-type ProstateRef and 5-cell-type collapsed HeartRef). Outcome: `O1_BLADDERREF_CALIBRATION_SEALED`. Sealed prereg SHA: `04d9d0d36faaf3bc6051c5f852f3bcb9d2ab48c437982b714ff8573d4cad178a`. Sealed amendment SHA: `c3015ca3ba25f6c13f4f93fec85edea8506f64472657d03b59ed9ccda8355787` (atlas-NaN-serialization correction; β not observed at amendment seal).

**Phase C executed (run-everything across one cohort with five atlases per Guardrail #12):**

The unified Phase C runner pattern (one β-pass produces per-sample tables for all three VALs simultaneously) was developed during this sprint and is now reusable. Runtime 270.7 sec for n=440 cohort. Script at `Biological_Physics/validation_runs/unified_phaseC_runner.py`.

**The CHK-3.1A tissue-class amendment 002 (DISC-BLADDER-002).** All three Phase C VALs prereg-locked CHK-3.1A at the kidney+prostate-derived solid-parenchyma floor (f_extreme ≥ 0.50, f_middle ≤ 0.12) inherited implicitly from VAL-106's TCGA HM450K substrate baseline. Under that floor, TCGA-BLCA cohort observed pass rate was 23.9% — bladder is a **mucosal-tissue substrate** with a substantially less bimodal methylation distribution than kidney/prostate solid parenchyma. Zero samples in cohort had genuine substrate corruption. The 76% gate failure was a tissue-class threshold mismatch, not data integrity failure. **Honest amendment 002 sealed AFTER β observed with full CCL-041 second-best-path disclosure** corrected the floor to bladder-cohort q1/q99 (f_extreme ≥ 0.387, f_middle ≤ 0.184). Pass rate under amended floor: 98.0%. CHK-2.16 cookbook gate added: tissue-class CHK-3.1A floor verification at prereg-write time.

*VAL-120 — Stage 1 Xu-538 immune red flag on TCGA-BLCA n=440.* Outcome `O4_STAGE1_DATA_INTEGRITY_FAILURE` per CHK-3.1B Xu-538 per-sample coverage 51.1% pass at ≥80% threshold (Xu-538 panel was breast-derived; cohort-substrate transferability flagged). Diagnostic d_paired=+1.8977 (n=21, p=3.14×10⁻⁸) reported as diagnostic-not-sealed finding. **DISC-BLADDER-004 sealed:** Stage 1 panel transferability is cohort-specific, not platform-specific. CHK-2.17 cookbook gate added.

*VAL-121 — Stage 2 multi-atlas on TCGA-BLCA n=440.* Outcome `O2_BLADDER_TILE_DIFFERENTIATING_DIRECTION_AMBIGUOUS`. Loyfer bulk Bladder tile fires d_paired=+1.91 POSITIVE; EpiSCORE BladderRef Epi tile fires d_paired=−1.46 NEGATIVE on same n=21 paired pairs. CHK-3.2 cross-tile sanity flagged ALL 14 Loyfer non-bladder solid-tissue tiles uniformly POSITIVE +2.34 to +2.92, confirming substrate-distribution mismatch on bulk-WGBS atlas. BladderRef Epi NEGATIVE satisfies CCL-039 cell-of-origin direction expectation (urothelial dedifferentiation, structurally consistent with prostate VAL-118 LE pattern). **DISC-BLADDER-003 sealed:** bulk-WGBS atlases on mucosal-cohort substrates produce inflated cross-tile A-scores; multi-atlas readings on mucosal cohorts MUST include a gene-promoter sub-cell-type atlas as primary cell-of-origin reader.

*VAL-122 — Stage 3 immune fine-tune on TCGA-BLCA n=440.* Outcome `O1_STAGE_3_IMMUNE_DIFFERENTIATING`. All 6/6 Salas IDOL tiles fire POSITIVE |d| range 0.49 to 1.24. Pre-locked O2 (lymphoid-dominant; Chen 2022 NMIBC blood RFS pattern) and O3 (myeloid-dominant; MDSC) did NOT fire. Broad-positive multi-lineage infiltration consistent with mixed TIL+TAM+MDSC of MIBC fired across three independent immune atlases.

**Phase D executed:** Phase C re-scoring against amended CHK-3.1A floor confirmed all sealed outcomes. Compared bladder-epic v0.1 results to prostate-epic v0.3 precedent (VAL-118 LE NEGATIVE pattern; bladder VAL-121 BladderRef Epi NEGATIVE structurally identical). Compared CHK-3.1A floor mismatch to cardio-epic DISC-CARDIO-005 (substrate-specific within-cohort self-cal envelopes) — the cardio lesson generalizes from substrate-class (sesame vs minfi vs GenomeStudio) to tissue-class (solid parenchyma vs mucosal vs cultured-cell).

**Phase E executed (E.1 through E.10):**

*E.1 — Card version bump.* New card sprint; bladder-epic v0.1 ships with no prior version. Validation tier `multi_modal_validated + multi_atlas_calibrated` from launch.

*E.2 — atlases_used_and_deferred block.* atlases_run = [Layered_Moss+Loyfer, EpiSCORE_BladderRef, Caggiano_CelFiE_TIM, Salas_Blood.EPIC_IDOL, UniLIFE]. atlases_deferred = [Bladder-substrate-coverage-validated_Stage1_panel (target v0.2, dependency = VAL-114 Wave 1 + CHK-2.17 precheck), Loyfer-corrected_mucosal-aware_A-score_normalization (target v0.2/v0.3, dependency = substrate-distribution-aware research), EpiSCORE_LungRef + ColonRef + future_mucosal-tissue_atlases (card-specific)].

*E.3 — chk_3_1_thresholds_per_substrate block.* Two substrate-class brackets: TCGA HM450K sesame Level 3 — solid parenchyma class (anchor VAL-106; f_extreme ≥ 0.50, f_middle ≤ 0.12); TCGA HM450K sesame Level 3 — mucosal/epithelial-lined-organ class (anchor VAL-120/121/122 amendment 002; f_extreme ≥ 0.387, f_middle ≤ 0.184). card-specific CHK-3.1B panels: Xu-538 (TRIGGERED O4 per DISC-BLADDER-004; cohort-substrate-coverage-validated panel deferred to v0.2 promotion path).

*E.4 — v0_1_run_everything_phase_c_results block.* Stage 1 Xu-538 (O4 panel coverage gate fired; diagnostic d=+1.90), Stage 2 multi-atlas (O2 dual-atlas direction divergence; BladderRef Epi NEGATIVE satisfies CCL-039), Stage 3 multi-atlas (O1 broad immune-architectural drift; mixed TIL+TAM+MDSC pattern).

*E.5 — Per-disease scoring policy.* Bladder cancer (HM450K sesame Level 3 tissue): Stage 1 immune red flag d=+1.90 reported as diagnostic only (panel-coverage gate fired); Stage 2 cell-of-origin uses BladderRef Epi as PRIMARY tile (Loyfer bulk reading is interpretive context only on mucosal cohort); Stage 3 reports broad immune-architectural drift descriptively (NOT lymphoid-dominant, NOT myeloid-dominant, NOT lineage-skewed). NOT claimed: NMIBC vs MIBC stratification, immunotherapy response prediction, BCG response prediction, blood-substrate validation, single-atlas Stage 2 reading.

*E.6 — DISC-BLADDER-NNN discoveries section.* Four discoveries logged (each propagated to LESSONS_LEARNED.md): DISC-BLADDER-001 (gene-promoter atlas family fitness depends on cell-type distinctness, not count); DISC-BLADDER-002 (CHK-3.1A floors are tissue-class-dependent); DISC-BLADDER-003 (bulk-WGBS atlases on mucosal cohorts produce inflated cross-tile A-scores); DISC-BLADDER-004 (Stage 1 panel transferability is cohort-specific).

*E.7 — What we chose not to claim + what remains open.* Five explicit non-claims (NMIBC vs MIBC stratification, PD-L1 / BCG response prediction, Stage 1 panel production-validated for bladder, Loyfer Bladder POSITIVE as biology, Stage 3 lineage-skewed signature). Seven items remaining open (Stage 1 panel calibration via VAL-114 + CHK-2.17, Stage 2 mucosal-aware A-score normalization, Stage 3 stage-stratified analysis, cohort expansion to GSE52955 + Bryan UK NMIBC + Chen 2022 NMIBC blood EPIC, WHI bladder pre-diagnostic biobank-gated, future EpiSCORE per-tissue bridges following BladderRef pattern, cookbook rollout of CHK-2.16 + CHK-2.17).

*E.8 — Validation evidence summary per VAL.* Four VAL entries (VAL-119, VAL-120, VAL-121, VAL-122) each formatted with cohort/n/substrate/design/QC/outcome/Cohen's d/interpretation/prereg SHA-256/amendment SHA-256.

*E.9 — Cookbook-wide CCL cross-references.* CCL-039 (cell-of-origin direction expectation; satisfied by BladderRef Epi); CCL-041 (honest second-best path used for amendment 002); CCL-046 (canonical-document atlas selection); CCL-049 (multi-atlas reporting); CHK-2.7 (magnitude-based |d| with direction labels); CHK-2.8 (substrate-floor-based CHK-3.1B coverage thresholds); CHK-3.1A/B/C (full-genome substrate gate + card-specific marker subset gate + atlas dedup gate); CHK-3.2 (cross-tile sanity check); CHK-7.6 (reproducibility triple); newly added: CHK-2.16 (tissue-class CHK-3.1A floor verification); CHK-2.17 (Stage 1 panel cohort-substrate coverage validation).

*E.10 — atlas_vault INVENTORY.json update.* 90 → 94 entries. EpiSCORE BladderRef bridge script + bridged CSV + Entrez source CSV + per-atlas README each entered with calibration anchor VAL-119 and SHA-256.

**Phase F executed (F.1 PUSH GitHub + F.2 DELIVER Heath same-as-pushed + F.3 DELIVER Heath-only cookbook IP + F.4 update TODO):**
- F.1: One commit pushed to `hmahaffeyges/IAM-Validation` main: `404eed3` (55 files: 4 atlas_vault BladderRef files + 4 VAL directories with prereg + amendment + outcome + script + results JSON + per-sample CSV + cohort manifest + clinical metadata + stratified results each + unified Phase C runner + post-pass + Biological_Physics/README.md update + atlas_vault/INVENTORY.json update).
- F.2: Heath received local copies of every Python script + every results JSON + every stratified JSON pushed.
- F.3: Heath received the seven canonical cookbook IP files updated additively: GAPE_Evidence_Report_UPDATED.html (+159 lines bladder VAL block before VAL-059), README_MASTER bumped to v2.6 (+amendment line + ship section), card README + JSON, LESSONS_LEARNED.md (+89 lines DISC-BLADDER-001 through DISC-BLADDER-004), TESTING_CHECKLIST.md (+71 lines CHK-2.16 + CHK-2.17), EDEAR_PIPELINE_OFFICIAL_REFERENCE_v2.md (+107 lines Parts 23 + 24), GAPE_Reproduction_Paper_v1.md (+25 lines §7.27 sprint section), atlas_vault README (+19 lines for HeartRef + ProstateRef + BladderRef + family-fitness rule).
- F.4: this TODO updated to v0.6 with bladder-epic reference example + CHK-2.16 + CHK-2.17 + Shared Task A protocol update + VAL ID reservations updated.

**Sprint outcome.** Bladder-epic v0.1 sealed. Four DISC-BLADDER discoveries propagated. Two new cookbook check gates (CHK-2.16 + CHK-2.17). Cookbook is stronger by four lessons and two new gates. The sprint is the cookbook's reference example for how prereg discipline self-corrects when a silent assumption meets data that violates it. **The discipline did not fail; the cookbook caught its own assumption the moment that assumption met a tissue class that violated it. That is what discipline looks like.**
...
**Substrate:** HM450
**Output:** `validation_runs/VAL-114_xu538_calibrate/calibration_results.json` + per_sample CSV + **per-cohort substrate-coverage precheck table** (NEW per DISC-BLADDER-004 / CHK-2.17 — see below)

**MANDATORY ADDITION per DISC-BLADDER-004 (added 2026-05-01).** VAL-114 must produce two outputs, not one:
1. The Stage 1 panel itself (calibrated CpG set + per-class A-score formula on Hannum 2013 GSE40279 n=656 healthy aging blood).
2. **A per-cohort substrate-coverage precheck output table** indicating which Phase C target cohorts the calibrated panel is COHORT-VALIDATED, COHORT-FLAGGED, or COHORT-INCOMPATIBLE for. Cohorts to precheck (one-time effort, applied to all): TCGA-BLCA HM450K (the cohort that surfaced the lesson), TCGA-BRCA HM450K, TCGA-LUAD HM450K, TCGA-LUSC HM450K, TCGA-LIHC HM450K, TCGA-COAD HM450K, TCGA-READ HM450K, TCGA-CESC HM450K, TCGA-KIRC HM450K, TCGA-PRAD HM450K, TCGA-STAD HM450K, TCGA-ESCA HM450K, plus any AD/cardio/heme cohorts using the panel.
3. Precheck procedure (per CHK-2.17): sample 5-10 random β files per target cohort; compute per-sample panel coverage = fraction(panel CpGs present and detection-pass / panel total CpGs); compute sampled-mean coverage and sampled-q5 coverage; FLAG decision: COHORT-VALIDATED if mean ≥ 90% AND q5 ≥ 80%; COHORT-FLAGGED if mean ≥ 90% AND q5 < 80% (requires dynamic panel-trimming OR pre-locked O4 acceptance); COHORT-INCOMPATIBLE if mean < 90% (defer to validated panel).
4. Cards using the calibrated panel reference VAL-114's precheck output table in their prereg block under `chk_3_1_thresholds_per_substrate.card_specific_chk_3_1b_panels.<panel_id>.cohort_substrate_coverage_precheck`.

The precheck step prevents future bladder-epic-style O4 surprises where a substrate-applicable panel fails on a specific cohort due to per-sample TSS-site processing variability.
...
- **v0.6 (2026-05-01):** Added (1) bladder-epic v0.1 reference example as the second worked card-sprint template (parallel to cardio-epic) — first mucosal-tissue card, four sealed VALs (VAL-119/120/121/122), four DISC-BLADDER discoveries. (2) Mandatory addition to Shared Task A VAL-114 protocol: per-cohort substrate-coverage precheck output table per DISC-BLADDER-004 / CHK-2.17, applied across all TCGA + AD + cardio + heme target cohorts as a one-time effort. (3) VAL ID reservations table extended with VAL-119/120/121/122 as DONE 2026-05-01. (4) Two new check gates added to the enumerated discipline checklist (CHK-2.16 tissue-class CHK-3.1A floor verification per DISC-BLADDER-002; CHK-2.17 Stage 1 panel cohort-substrate coverage validation per DISC-BLADDER-004). (5) Bladder-epic Phase F.3 deliverables enumerated as the canonical seven-files-update package: Evidence Report HTML +159 lines, README_MASTER bump, card README + JSON, LESSONS_LEARNED +89 lines, TESTING_CHECKLIST +71 lines, PIPELINE_REFERENCE +107 lines, Reproduction Paper +25 lines, atlas_vault README +19 lines. v0.5 content preserved verbatim.
