...
def build_batch_arrays(cpg_batch: list, cpg_to_obs: dict,
    ...
    SWAMP FIX (v0.1.1): in addition to the per-observation arrays, build the
    set of (cpg, celltype) PAIRS that actually have at least one observation,
    and map each observation to its pair index. The model's z/mu parameters
    span only these observed pairs, not the full n_cpg*n_ct grid, so there are
    no unconstrained parameters. pair_cpg_idx / pair_ct_idx let the output
    reconstruct per-cpg/per-celltype posteriors after sampling.
    """
    ...
    # First pass: enumerate observed (cpg, ct) pairs in deterministic order
    pair_index = {}            # (cpg_i, ct_i) -> pair position
    pair_cpg_idx, pair_ct_idx = [], []
    for cpg in cpg_batch:
        ci = cpg_idx[cpg]
        for (atlas, ct, beta, n, w) in cpg_to_obs.get(cpg, []):
            key = (ci, ct_idx[ct])
            if key not in pair_index:
                pair_index[key] = len(pair_cpg_idx)
                pair_cpg_idx.append(ci)
                pair_ct_idx.append(ct_idx[ct])

    # Second pass: per-observation arrays, each mapped to its pair index
    obs_idx_cpg, obs_idx_atlas, obs_idx_ct, obs_idx_pair = [], [], [], []
    obs_beta, obs_n_donors, obs_weight = [], [], []
    for cpg in cpg_batch:
        ci = cpg_idx[cpg]
        for (atlas, ct, beta, n, w) in cpg_to_obs.get(cpg, []):
            obs_idx_cpg.append(ci)
            obs_idx_atlas.append(atlas_idx[atlas])
            obs_idx_ct.append(ct_idx[ct])
            obs_idx_pair.append(pair_index[(ci, ct_idx[ct])])
    ...
    return {
        ...
        "obs_idx_pair": np.array(obs_idx_pair, dtype=np.int32),
        ...
        "n_pairs": len(pair_cpg_idx),
        "pair_cpg_idx": np.array(pair_cpg_idx, dtype=np.int32),
        "pair_ct_idx": np.array(pair_ct_idx, dtype=np.int32),
    }
...
def build_class_model(data: dict) -> pm.Model:
    ...
    with pm.Model() as model:
        ...
        # SWAMP + RIDGE FIX (v0.1.1):
        #
        # SWAMP: parameters span ONLY observed (cpg, celltype) pairs (n_pairs),
        # not the full n_cpg*n_ct grid. The grid created a parameter for every
        # cell at every CpG even where never measured; those were unconstrained
        # and formed a high-dimensional swamp NUTS could not traverse. Stromal,
        # with 200:1 coverage imbalance, was ~63% unconstrained at 9 cells.
        #
        # RIDGE: the previous form mu_logit = mu_class_logit + sigma_class_logit*z
        # made sigma_class_logit (spread of means) trade off against log_kappa
        # (per-atlas precision) — two dispersion knobs the data identifies only
        # in combination, creating a posterior ridge. ESS collapsed on exactly
        # sigma_class_logit and log_kappa (~8) while everything else mixed.
        # Strong-data classes overpowered it; stromal's marginal data did not.
        # FIX: model each pair's logit-mean DIRECTLY with a wide Normal prior,
        # identified by its own observations. sigma_class_logit removed ->
        # nothing for log_kappa to trade against -> ridge eliminated.
        # Verified: lopsided 9-cell case converges (R-hat 1.02, ESS 1055, 0 div);
        # reproduces the old model on balanced data (posterior-mean corr 0.99996).
        # mu_class_logit retained as a weak centering anchor on the prior mean.
        n_pairs = data["n_pairs"]
        mu_logit = pm.Normal("mu_logit", mu=mu_class_logit, sigma=3.0,
                             shape=n_pairs)
        # Transform to (0,1) for use in Beta likelihood. mu is per-pair.
        mu = pm.Deterministic("mu", pm.math.sigmoid(mu_logit))

        # Per-atlas precision. exp(3.0) ~ 20, typical methylation Beta
        # concentration. No longer trades against a class-spread parameter.
        log_kappa = pm.Normal("log_kappa", mu=3.0, sigma=1.0, shape=n_atlas)
        ...
        # Observation model — unchanged likelihood structure. mu is now indexed
        # per-pair (SWAMP FIX): each observation maps to the flat pair index for
        # its (cpg, celltype). obs_idx_pair is built in build_batch_arrays.
        mu_obs = mu[data["obs_idx_pair"]]
...
def sample_batch(target_class: str, data: dict, batch_idx: int, n_batches: int,
    ...
    summary_sampled = az.summary(
        idata, var_names=["mu_logit", "mu_class_logit", "log_kappa"],
        stat_focus="mean"
    )
    ...
    # Extract posteriors. SWAMP FIX: mu is now flat per-pair, shape
    # (chain, draw, n_pairs). Reduce over chain+draw to per-pair summaries,
    # then scatter back into a dense (n_cpg, n_ct) grid for output. Pairs with
    # no observation are left as NaN — we have no estimate for a cell at a CpG
    # where it was never measured, and emitting NaN is the honest result rather
    # than a fabricated prior-only number.
    mu_post = idata.posterior["mu"].values            # (chain, draw, n_pairs)
    mu_flat = mu_post.reshape(-1, mu_post.shape[2])    # (samples, n_pairs)
    pair_mean = mu_flat.mean(axis=0)
    pair_sd = mu_flat.std(axis=0)
    pair_lo = np.quantile(mu_flat, 0.025, axis=0)
    pair_hi = np.quantile(mu_flat, 0.975, axis=0)

    n_cpg = len(data["cpg_ids"])
    n_ct = len(data["celltype_ids"])
    mu_mean = np.full((n_cpg, n_ct), np.nan)
    mu_sd = np.full((n_cpg, n_ct), np.nan)
    mu_lo = np.full((n_cpg, n_ct), np.nan)
    mu_hi = np.full((n_cpg, n_ct), np.nan)
    pc = data["pair_cpg_idx"]
    pt = data["pair_ct_idx"]
    mu_mean[pc, pt] = pair_mean
    mu_sd[pc, pt] = pair_sd
    mu_lo[pc, pt] = pair_lo
    mu_hi[pc, pt] = pair_hi

    # Per-CpG class summaries: average across observed cell types only
    # (nan-aware), so unobserved pairs don't drag the class mean.
    cpg_class_mean = np.nanmean(mu_mean, axis=1)
    cpg_class_sd = np.sqrt(np.nanmean(mu_sd**2, axis=1)
                           + np.nanvar(mu_mean, axis=1))
    cpg_class_lo = np.nanmean(mu_lo, axis=1)
    cpg_class_hi = np.nanmean(mu_hi, axis=1)

    # Predictive check: map each observation to its pair's posterior mean
    mu_obs_mean = pair_mean[data["obs_idx_pair"]]
