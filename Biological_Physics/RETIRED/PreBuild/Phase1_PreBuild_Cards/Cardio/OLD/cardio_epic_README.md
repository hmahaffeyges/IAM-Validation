# cardio-epic v0.2 — Cardiovascular EDEAR Card

**Version:** 0.2
**Built:** 2026-04-29
**Validation tier:** multi_modal_validated (3 disease cohorts across 3 substrates plus 1 atlas-integration test)
**Built under:** CHK-3.1A/B split convention (locked 2026-04-28); first card built natively under split. v0.2 brings the card up to full Block 1-20 + CHK-5.7/5.8/5.9/5.10 structural-parity with breast-epic v2.3 / crc-epic v2.4 and adds VAL-111 sealed atlas-deferral outcome.
**Supersedes:** cardio-epic v0.1 (2026-04-28). All sealed VAL outcomes from v0.1 are preserved unchanged. v0.2 is an additive structural rebuild — no biology was rewritten.
...
**Stage 2 (cell-of-origin):** Loyfer 25-tile run-everything reference. Per-tile per-class A-score against frozen H_min anchor for each tile's mapped class. Cardio-relevant tiles emphasized in scoring report:
...
**Stage 3 (immune subcomposition):** UniLIFE 19-cell + Salas Blood.EPIC IDOL 6-cell pooled-entropy. Run-everything per cookbook signoff 2026-04-26.

**`atlases_run` at v0.2 (CHK-5.8):**
- Loyfer 25-tile (Stage 2, anchor VALs 108/109/110)
- UniLIFE 19-cell (Stage 3, anchor VALs 108/109/110)
- Salas Blood.EPIC IDOL 6-cell (Stage 3, anchor VALs 108/109/110)

**`atlases_deferred` at v0.2 (CHK-5.8):**
- EpiSCORE HeartRef CM/EC/FB/MP/SMC — anchor VAL-111, target v0.3, unblock dependency = atlas re-bridging from gene-promoter integer markers to a tile-coverage CpG layout that preserves cell-type contrast on heterogeneous β
- Caggiano CelFiE TIM heart_meth + endothelial_meth — no anchor VAL yet, target v0.3, unblock dependency = HM450 hg19 manifest acquisition (Locations.rda DFrame readable in container)
