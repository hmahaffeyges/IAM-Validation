...
Kit path prefixes below are relative to the kit root. Repo paths are relative to the repo root; `ENGINE/` = `Biological_Physics/AstroGenetics/CPG_KISS_Commercial_Engine/`, `VAULT/` = `Biological_Physics/PreIAMAtlas_Build/atlas_vault/`.
...
|---|---|---|---|
| IAM Atlas (483,092 CpGs × 8 classes + 115 cells) | REPO `ENGINE/IAM_Atlas/IAMAtlasREBUILD.csv.xz` | **not shipped** — `data/IAMAtlasREBUILD.csv` (decompress; sha256 in `DATA_CHECKSUMS.sha256`) | canonical |
| atlas provenance + build | REPO `ENGINE/IAM_Atlas/IAMAtlasREBUILD_provenance.json` | `runtime/` | canonical |
| cell type → class map (115 → 8) | REPO `ENGINE/IAM_Atlas/IAMAtlasREBUILD_celltype_to_class.json` | `runtime/` | canonical |
...
| age reference band (8 classes × 10 decades) | REPO `ENGINE/` (trial bundle copy identical) `age_reference_matrix.json` | `runtime/` | canonical; compiled as H(β̄)/H_min |
...
| conductor (presence-paired scoring, DETECT_FLOOR 0.01; 'replaces walther_clinical.py', 2026-07) | **YOUR FOLDER only** — supplied to this session four times as an upload; **NOT in the repo at 66f37fe, NOT in CPG_TRIAL_CODE.zip** → **must be committed to REPO** `ENGINE/` | `engine/cpg_conductor.py` (289-line version; a 95-line stub also circulates — discard it) | repo is missing the file the chain is defined by (RECON D2 for the 3% second floor) |
...
|---|---|---|---|
| foundation-cohort anchors v1 (GSE51032 n=460, GSE51057 n=188, 115 cells) | REPO `Biological_Physics/PreIAMAtlas_Build/Preliminary_Test_Results/validation_runs/foundation_cohort/` | `anchors_v1/` | SUPERSEDED by v2 (RULING M1b); keep |
...
|---|---|---|---|
| Issue 003 build chain (build, data003, gape002_lib, 002 script) | **KIT** `issue003_build/` → **should be committed to REPO** `ENGINE/builders/` | yes | new |
...
| LESSONS_LEARNED.md, CPG_Lessons_Learned_2026-06-29.md, CHANGELOG.md, README_FOR_FUTURE_AI.md | REPO `ENGINE/` and `ENGINE/README's/` | no | canonical |
...
5. promote `cpg_kit.py` and the five `PROC_*.py` into `ENGINE/tests/`, and `issue003_build/` into `ENGINE/builders/`
...
8. **commit `cpg_conductor.py` (289-line, 2026-07) to `ENGINE/`** — the running chain's orchestrator exists only in your folder
