...
|---|---|---|---|
| IAM Atlas, decompressed | `data/IAMAtlasREBUILD.csv` (605 MB) | repo `Biological_Physics/AstroGenetics/CPG_KISS_Commercial_Engine/IAM_Atlas/IAMAtlasREBUILD.csv.xz` → `xz -dk` | `52ff4ccb35752ba0337f45c9563d7309c6fe9c4bdb8720daa196ab30f4596985` |
...
## 4. The eight repo commits this folder implies (from COMPONENT_MAP §G)

1. `runtime/iamatlas_celltype_markers_v0_2.json` → `ENGINE/Runtime Matrices/Celltype_Marker/` (RULING M1b)
2. `anchors_v2/` → beside `foundation_cohort/`; mark v1 SUPERSEDED in `cohort_manifest.json`
3. SOP v1.4.0 → `atlas_vault/walther_clinical_runtime/`; amend §105 per RULING A3
4. the two extra plate PNGs from your zip → `atlas_vault/IAMAtlas_v0_1/plates/` (or delete locally)
5. `cpg_kit.py` + `PROC_*.py` → `ENGINE/tests/`; `issue003_build/` → `ENGINE/builders/`
6. amend `test_a_score_canonical.py` guard to name its surface
7. rename `n_bio` → Mahaffey number (20.94) in engine constants and 002 card text; reconcile Hubble2GAPE l.2192
8. `engine/cpg_conductor.py` → `ENGINE/`
9. **new:** `repo_staging/…/CPG_VAL_047_Breast_per_patient_prediagnostic/` → `Preliminary_Test_Results/validation_runs/`

I have no GitHub credential here, so I cannot push. If you add one (Customize → Credentials), I can open a single branch with all nine and you review the diff. Otherwise `repo_staging/` mirrors the target tree — copy it over the repo root.
