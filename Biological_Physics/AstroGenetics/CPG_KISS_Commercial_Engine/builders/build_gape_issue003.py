...
def secV_val_index(story):
    ...
    for o in D.VAL_INDEX:
        ...
        rows.append((o["val"], t, o["date"][:10], o["cohorts"], o["decision"][:34], o["record"]+(" · RETIRED" if o["retired"] else ""), o["path"].replace("Biological_Physics/PreIAMAtlas_Build/","BP/PreIAMAtlas_Build/")))
    story.append(Paragraph("Path prefix <font face=\"Courier\">BP/</font> = <font face=\"Courier\">Biological_Physics/</font>. Paths are directories; the prereg, outcome and results files sit inside them.", sMut))
