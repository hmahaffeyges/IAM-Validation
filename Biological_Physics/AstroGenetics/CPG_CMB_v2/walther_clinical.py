...
Mahalanobis distance, cellular age, brilliance maps. NILC runs IN Link 3 (parallel
...
class Stage8Output:
    ...
    route_A_architectural_alarm: dict  # stands down in lean v1 (second chain: Mahalanobis)
    route_C_bidirectional: dict        # stands down in lean v1 (shelf: bidirectional)
    status: str = "OK"


def _build_patient_departure_profile(stage4_output, mapping_path, floor=1.0):
    """Derived A-departure per matrix column: (A - floor) for each DETECTED cell (A not None),
    atlas-cell -> matrix-column via the v0.2 map (averaged when several cells share a column).
    floor = 1.0 is the derived healthy baseline. No sigma, no cohort, no standardization."""
...
CONCORDANCE_SIGNAL_FLOOR = 0.15      # |patient departure| below this is baseline noise, not signal
CONCORDANCE_MIN_SIGNAL_CELLS = 3     # need this many signal-bearing shared cells for a resemblance
...
def _classify_match_specificity(signal_cells, patient_departure):
    """A match is NON-SPECIFIC when its signal-bearing cells are all one generic theme:
    all myeloid and elevated (a myeloid/neutrophil expansion) or all lymphoid and suppressed
    (the lymphopenia half). Either is the non-specific systemic / neutrophil-to-lymphocyte
    shift seen in infection, inflammation, stress and many cancers -- not a disease-specific
    fingerprint. Returns 'NON_SPECIFIC_MYELOID', 'NON_SPECIFIC_LYMPHOID', or 'SPECIFIC'."""
    if not signal_cells:
        return "SPECIFIC"
    dirs = [patient_departure.get(c, 0.0) for c in signal_cells]
    all_mye = all(c in _MYELOID_CELLS for c in signal_cells)
    all_lym = all(c in _LYMPHOID_CELLS for c in signal_cells)
    if all_mye and all(d > 0 for d in dirs):
        return "NON_SPECIFIC_MYELOID"
    if all_lym and all(d < 0 for d in dirs):
        return "NON_SPECIFIC_LYMPHOID"
    return "SPECIFIC"


def _concordance(patient_departure, signature, min_shared=3):
    """Direction-gated cosine concordance over the cells present in BOTH the patient's
    derived departure vector and the disease signature. Scale-invariant. Returns None when
    too few cells are shared to say anything honest.

    CRITICAL (added after the leukemia/lung false-flag review): cosine is direction-only and
    ignores magnitude, so a patient at BASELINE on a shared cell (CD4 at +0.047, monocytes at
    +0.054) still contributes its sign and inflates the cosine. A mild generic neutrophils-up
    shift then 'resembles' every signature with neutrophils-up. So we also count n_signal:
    shared cells where the patient ACTUALLY departs (|dep| >= CONCORDANCE_SIGNAL_FLOOR). A
    resemblance is trustworthy only when the patient carries real signal on enough shared
    cells -- not when noise-level values happen to point the right way."""
    import numpy as np
    shared = [c for c in signature if c in patient_departure]
    if len(shared) < min_shared:
        return None
    p = np.array([patient_departure[c] for c in shared], dtype=float)
    s = np.array([signature[c] for c in shared], dtype=float)
    pn, sn = float(np.linalg.norm(p)), float(np.linalg.norm(s))
    if pn == 0.0 or sn == 0.0:
        return None
    cosine = float(np.dot(p, s) / (pn * sn))
    dir_agree = float(np.mean(np.sign(p) == np.sign(s)))
    signal_cells = [shared[i] for i in range(len(shared))
                    if abs(p[i]) >= CONCORDANCE_SIGNAL_FLOOR]
    n_signal = len(signal_cells)
    return {"cosine": cosine, "direction_agreement": dir_agree,
            "n_shared": len(shared), "n_signal": n_signal,
            "signal_cells": signal_cells}


def _resemblance_label(cosine, dir_agree, n_signal):
    """Plain-language strength of resemblance (NOT a probability). Calibration of these bands
    against real patient outcomes is future work; v1 reports resemblance only.

    A resemblance requires the patient to carry REAL signal on enough shared cells. Below
    CONCORDANCE_MIN_SIGNAL_CELLS signal-bearing cells the match is direction-alignment of
    near-baseline noise -- reported as INSUFFICIENT_SIGNAL, never a resemblance."""
    if n_signal < CONCORDANCE_MIN_SIGNAL_CELLS:
        return "INSUFFICIENT_SIGNAL"
    if cosine >= 0.6 and dir_agree >= 0.7:
        return "STRONG_RESEMBLANCE"
    if cosine >= 0.3 and dir_agree >= 0.5:
...
def stage_8_dual_matching(stage4_output, stage5_output, stage4_5_report,
    ...
    (Mahalanobis architectural alarm) and C (bidirectional) stand down in the lean v1
    ...
    for r in rows:
        ...
        scored.append({
            ...
            "resemblance": _resemblance_label(con["cosine"], con["direction_agreement"], con["n_signal"]),
        })
    ...
    route_A = {"fired": None,
               "note": "Route A (Mahalanobis architectural alarm) stands down in the lean v1 "
...
def detect_cell_of_origin_presence(stage2_output, detect_floor=0.01):
...
def run_pipeline(beta_calibrated, *, context=None, patient_id="patient", test_id=None,
    ...
    # NILC-raw surfaces faint non-blood classes Walther floors. RECORD which it surfaced (the
    # cell-of-origin presence detector / Mode 2 reads them), but DO NOT promote them into the
    # A-scoring. A class at ~1% is ~99% blood-diluted: its architecture cannot be scored from
    # the bulk beta, and forcing it in only produces meaningless A-scores and pollutes the Mode 1
    # concordance vector. Scoring stays on the cells abundant enough to read their own loci.
    rescued = []
    if s2:
        raw = (s2.get("nilc_fractions") or {}).get("raw_fractions", {}) or {}
        for cls, frac in raw.items():
            try:
                fv = float(frac)
            except (TypeError, ValueError):
                continue
            if fv >= detect_floor and not (class_present or {}).get(cls, False):
                rescued.append({"class": cls, "nilc_raw_fraction": round(fv, 4)})
    ...
    # L7 — SECOND CHAIN (confirmation). Fires only when Stage 8 Route B flagged a
    # disease (any match above threshold). Derived Mahalanobis adjudicates; the
    # literature anchor labels; the residual map (if one exists) concords. When no
    # flag fired, run_second_chain returns None and the report shows no confirmation.
    try:
...
def _baseline_payload(bundle, visit_label):
    ...
    maha = (s5.get("mahalanobis") or {}) if isinstance(s5, dict) else {}
    ...
    return {
        ...
        "mahalanobis_distance": maha.get("distance"),
        "mahalanobis_beyond_band": maha.get("beyond_healthy_band"),
...
def _compute_trajectory(bundle, priors, now_label, cfg):
    ...
    gd = None
    if now.get("mahalanobis_distance") is not None and prev.get("mahalanobis_distance") is not None:
        dn, dp = now["mahalanobis_distance"], prev["mahalanobis_distance"]
        gd = {"prior": round(dp, 3), "now": round(dn, 3), "delta": round(dn - dp, 3),
              "trend": ("moving away from healthy" if dn - dp > 0.3
                        else "returning toward healthy" if dn - dp < -0.3 else "steady")}
    ...
    elif gd and gd["trend"] == "moving away from healthy":
        lead = f"the overall departure rose ({gd['prior']}\u2192{gd['now']}, moving away from healthy)"
    elif rel_up:
    ...
    return {"prior_visit_label": prev["visit_label"], "first_visit_label": first["visit_label"],
            ...
            "cell_changes": cell_changes, "rotation": rotation, "self_alignment": self_align,
            "global_departure": gd, "headline": head}
