# CPG-VAL-003 — Pre-Registration

**VAL ID:** CPG-VAL-003
**Title:** Per-CpG residual map — CPG_breast_panel_v1 seed
...
- **Signal column:** `a_score (top residual CpG)`
- **Effect direction expected:** Per-CpG (observed β − class-fraction-predicted β) residual signal at hundreds of class marker CpGs; cross-cohort concordance > random; new candidate panel emerges that doesn't overlap with Xu-538
...
- **Result narrative:** 1,392 concordant CpGs across cohorts (|d|>0.2 in both AND same sign); 1,173 hypomethylated vs 219 hypermethylated (5.4:1 ratio); 1,389 NEW candidates not in Xu-538. CPG_breast_panel_v1 candidate panel emitted from this map.
...
- Sealed reproducer script (`cpg_val_003.py`)
