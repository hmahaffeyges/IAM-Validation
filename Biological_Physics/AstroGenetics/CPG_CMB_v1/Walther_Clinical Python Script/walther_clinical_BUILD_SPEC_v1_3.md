...
- §5 **new Stage 4.6** — per-class healthy brightness comparison + patient Mollweide projection. The customer's personal Cosmic Methylome Background (Plate 1 analog). **`patient_brightness_comparison.py` built**.
...
**Changes v1.2 → v1.2.1 (2026-06-07 patch):**
...
| `mahalanobis_healthy_reference_v0_5.json` (current production) | `Mahalanobis_healthy_reference/` | `artifact_id`, `feature_names_valid`, `n_features`, `centroid`, `covariance_matrix`, `shrinkage`, `route_A_calibration_v0_3` (p95 threshold + percentile distribution under n=1,721 HC), `hc_cohort_sources` (foundation + Hannum + Tsaprouni), `phase_planning` (Phase 3 + Phase 4 queued). v0_1 (n=601), v0_2 (n=1,257), v0_3 (n=1,721), v0_4 (n=2,481) preserved at same folder for lineage. See §3.4b hull versioning protocol. |
...
|---|---|---|
| `CPG_Plate_01_Cosmic_Methylome_Background.png` | 8-panel Mollweide of per-class posterior β across 481,966 CpGs (the healthy reference) | Stage 4.6 patient CMM mirrors this projection |
...
| **L6** Covariance modeling | FILLED via Mahalanobis hull | Stage 5 (Mahalanobis distance against `mahalanobis_healthy_reference_v0_5.json`'s pooled HC covariance n=2,523 across 8 cohorts spanning 4 populations (EU + US + UK + Han Chinese) and 2 platforms (HM450 + EPIC) including the first Asian-population representation, Ledoit-Wolf shrinkage, percentile-calibrated Route A threshold) |
...
│   ├── mahalanobis_healthy_reference_v0_5.json (CURRENT PRODUCTION: n_hc=2,523, 8 cohorts, 4 populations, HM450+EPIC)
│   ├── mahalanobis_healthy_reference_v0_4.json (n_hc=2,481, +AddNeuroMed/AIBL/GIFT — lineage)
│   ├── mahalanobis_healthy_reference_v0_3.json (n_hc=1,721, +Tsaprouni — lineage)
│   ├── mahalanobis_healthy_reference_v0_2.json (n_hc=1,257, +Hannum — lineage)
...
**Rationale.** The IAMAtlas REBUILD MCMC produced per-CpG, per-class posterior mean + SD across 481,966 CpGs (the brightness CSVs inside `IAMAtlas_v0_1/class_archives/*.tar.xz`). These ARE the per-class healthy reference — the data behind Plate 1, the Cosmic Methylome Background. Patient runtime SHOULD consult this reference: compute per-CpG z-score of patient β versus each class's healthy posterior, then project the result onto the same HEALPix grid as Plate 1. The customer's personal CMB (8-panel Mollweide) becomes the visualization endpoint of the report.
...
# Production loads CURRENT version — currently v0_5 (n=2,523 pooled HC, 8 cohorts, 4 populations, HM450 + EPIC).
hull = MahalanobisHealthyHull("Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v0_5.json")
...
**Route A trigger threshold is percentile-based, NOT a fixed value.** With 112 features under multivariate normality, expected median Mahalanobis distance is √112 ≈ 10.58 — a fixed threshold like `d ≥ 2.0` would fire on ALL samples (the v0_1 mistake). The current v0_5 thresholds are:
- p95 (default Route A): d ≥ 13.62
- p99 (strict): d ≥ 18.43
calibrated against the pooled n=2,523 HC distance distribution stored in the artifact under `route_A_calibration_v0_5`. Production engine reads the threshold from the artifact at session startup — not hard-coded in any card or module.

**Validation anchor lineage** (preserved as HC hull expands):
- Breast pre-dx Cohen's d (GSE51057 n=11): v0_1 +1.871 → v0_2 +0.981 → v0_3 +0.896 → v0_4 +0.593 → v0_5 +0.599
- Breast pre-dx Cohen's d (GSE51032 n=36): v0_1 +2.088 → v0_2 +1.653 → v0_3 +1.611 → v0_4 +1.450 → v0_5 +1.450
- Case detection % at p95 threshold (GSE51032): v0_2 50.0% → v0_3 55.6% → v0_4 38.9% → v0_5 41.7%
- Case detection % at p95 threshold (GSE51057): v0_2 9.1% → v0_3 27.3% → v0_4 9.1% → v0_5 9.1%
- v0_5 lineage is essentially flat from v0_4 (small +42 Han Chinese samples; population diversification + platform validation, not centroid shift)

**Honest finding at v0_4:** GIFT cohort's HC samples (FTD-research-context HC) sit at median d=12.18 vs other HCs at d=7-10, pulling Cohen's d down by widening the HC envelope into the case-distance range. Kept in v0_4 because GIFT samples ARE genuine HC by design; broader hull is more honest about real cross-cohort variance than a narrower hull would be.
...
- **Phase 3 ✓** (2026-06-06): v0_3 → v0_4 by adding AddNeuroMed n=96, AIBL n=471 (EPIC — FIRST CROSS-PLATFORM), GIFT n=193. Brings +cross-platform via AIBL EPIC, +neurodegen-cohort HC context.
- **Phase 4 ✓** (2026-06-06): v0_4 → v0_5 by adding Han Chinese GSE141682 n=42 (EPIC — FIRST ASIAN POPULATION). Brings +Asian population representation. Han Chinese samples sit at median d=10.51 (within typical HC range), confirming canonical 115-cell A-scoring transfers across populations without systematic offset.
- **Phase 5+** (queued): Routine maintenance. Specific targets to monitor: HELIOS Singapore EPIC if accessible; OEP001178 Han Chinese schizophrenia n=476 HC if Chinese NODE access granted; Korean/Japanese cohorts as they surface. Phase 4's small n=42 should be expanded with larger Asian cohorts before clinical deployment in Asian populations.
