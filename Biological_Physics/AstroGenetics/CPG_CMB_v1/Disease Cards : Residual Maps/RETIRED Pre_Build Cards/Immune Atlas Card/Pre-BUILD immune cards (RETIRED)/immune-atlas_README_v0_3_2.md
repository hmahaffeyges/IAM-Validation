...
**Amended 2026-05-03 (v0.3.2):** Gastric-esophageal-epic v0.1 sprint additive integration (VAL-126 STAD + VAL-127 ESCA + VAL-128 Crohn's, all sealed 2026-05-02 against KIRC+PRAD anchor n=210). Updates: §5 disease cross-reference table extended with esophageal + IBD pathway rows; gastric row amended with VAL-126 outcome (v0.3 prediction PRESERVED VERBATIM, v0.3.2 outcome added as parallel block); §6.6b NEW Pattern 7 candidate (population-fraction-shift signature, DISC-GE-005); §11 CCL-027 four-question tables i/ii/iii/iv expanded with gastric outcome + esophageal ESCC + EAC + IBD pathway rows (v0.3 predictions preserved verbatim); §13 Stage 3 sealed signatures expanded with §13.6b gastric VAL-126 + §13.6c esophageal VAL-127 + §13.6d Crohn's VAL-128 entries; §13.8 NEW operational guidance subsection — how to interpret IBD-style population-fraction-shift Stage 3 readings (companion to §13.7 bladder-style broad-positive guidance); §14 cross-card syntheses expanded with §14.11 gastric_vs_esophageal + §14.12 crohns_vs_everything Stage 3 population-fraction-shift hub + §14.13 gastric_vs_bladder tier-3 substrate baseline GI continuum + §14.14 esophageal subtype discrimination vs prostate LE tile DISC-PROSTATE-002 extension; §22.5 NEW v0.3.2 lessons (immune-atlas-LL-014 through LL-019 covering DISC-GE-001 through DISC-GE-006). All v0.1, v0.2, v0.3, and v0.3.1 content preserved verbatim. The card position remains #15 of 15.
...
### §6.6b Pattern 7 — Population-fraction-shift signature (DISC-GE-005, added 2026-05-03)

Stage 1 INFORMATIVE NULL across all cell-type strata (cycling-class architectural-drift panel does not detect the disease) AND Stage 3 BIDIRECTIONAL by population-fraction-shift mechanism (T-cell expansion + myeloid depletion or vice versa) AND sorted-cell substrate ELIMINATES the signal (mixture-attenuation reversal: sorted d SMALLER than whole-blood d, OPPOSITE direction predicted by mixture-attenuation hypothesis).

**Canonical case (sealed):** Crohn's disease + UC on GSE87650 GPL13534 (VAL-128 sealed 2026-05-02). Stage 1 |d_CD-HC| < 0.5 across all four cell-type strata (monocytes, CD4, CD8, whole blood). Stage 3 max |d_CD-HC| = 1.72 on UniLIFE aCD8Tnv. Sorted-cell sub-experiment: 40/93 tiles pass = 43% (whole blood STRONGER than sorted cells; OPPOSITE direction predicted by mixture-attenuation hypothesis).

**The diagnostic signature.** When this pattern fires, three checks confirm it as population-fraction-shift biology rather than within-cell-type drift:

1. **Stage 1 NULL across all cell-type strata.** The Xu-538 cycling-class panel does not detect the disease in any cell type or in whole blood. This is the informative-null signature distinguishing chronic-inflammatory diseases (IBD, autoimmune, chronic viral infection) from cancer cycling-acceleration diseases.

2. **Stage 3 bidirectional with cross-atlas reproduction.** T-cell expansion (CD4 + CD8 + Treg + NK |d| 1.4-1.7) + myeloid depletion (Mono + Neu |d| 1.0-1.3) reproduces across UniLIFE 19-cell + Salas IDOL 6-cell + Loyfer EPIC immune subset + Caggiano TIM immune subset. Cross-atlas reproduction is the convergence check ruling out single-atlas artifact.

3. **Sorted-cell substrate yields SMALLER signal than whole-blood substrate.** This is the discriminating test for population-fraction-shift mechanism vs within-cell-type drift mechanism. If the disease drives within-cell-type chronic-inflammation drift (e.g., TIL exhaustion in solid tumor microenvironment), sorting cells AMPLIFIES the signal because the sorted population is enriched for the affected cell type. If the disease drives population-fraction shifts (IBD, viral infection, sepsis, autoimmune), sorting cells ELIMINATES the signal because the population-fraction shift exists only in mixed-population substrates.

**The cookbook rule (DISC-GE-005 sealed 2026-05-02):** Stage 3 deconvolution atlases (Salas IDOL, Loyfer immune tiles, UniLIFE 19-cell, Caggiano TIM) measure cell-type COMPOSITION SHIFTS in mixed-population substrates, NOT within-cell-type chronic-inflammation drift. Cards interpreting Stage 3 results should anchor the interpretation in population-fraction-shift language ("T-cell expansion + myeloid depletion in whole blood") not within-cell-type-drift language. Future card preregs should pre-specify whether the disease is expected to drive a population-fraction shift (Pattern 7) vs a within-cell-type drift (Pattern 1, 2, or 6).

**Expected disease scope.** IBD (Crohn's, UC) — sealed VAL-128. Predicted: chronic viral infection (HIV chronic, chronic HBV, EBV reactivation), autoimmune diseases driving peripheral immune-population shifts (rheumatoid arthritis active, multiple sclerosis active flare, T1D active autoimmune phase), sepsis acute, GVHD post-transplant. NOT predicted: solid-tumor TIL exhaustion (within-cell-type drift; produces Pattern 1 or 2 instead).

**How this differs from Pattern 3 (AD-instance pooled-null + directional-pass).** Pattern 3 has the directional signature WITHIN the Stage 1 panel via z-scored decomposition (the AD 7-CpG Rule A panel and the PDAC 324-CpG directional panel both live inside the Xu-538 panel). Pattern 7 has Stage 1 NULL with the signature living in Stage 3 cell-fraction deconvolution atlases entirely outside the Stage 1 panel. Pattern 3 is bidirectional cancellation in the per-CpG direction sense; Pattern 7 is bidirectional in the per-cell-type proportion sense.

**How this differs from Pattern 6 (substrate-distribution mismatch on mucosal cohort).** Pattern 6 is methodology — bulk-WGBS atlas family on mucosal cohort produces inflated A-scores from substrate-distribution mismatch; the apparent biology is artifact. Pattern 7 is biology — population-fraction-shift in mixed-population substrate; the bidirectional Stage 3 reading is the real signal. Pattern 6 disappears when the gene-promoter atlas family is used; Pattern 7 disappears when cells are sorted (which is the discriminating test).
...
| Gastric (v0.3.2 outcome) | POSITIVE within-cohort observed (VAL-126 d=+3.34 vs anchor; subtype hierarchy MSI ≈ EBV > CIN > GS preserves CIMP-amplification ordering) | VAL-126 sealed 2026-05-02 | PRE-LOCKED NEGATIVE PREDICTION (above row) FAILED PRE-LOCK under CHK-3.2 tier-3 -5.02 anchor-SD; within-cohort subtype contrasts immune to baseline shift form load-bearing v0.3.2 finding |
| Esophageal ESCC | POSITIVE | VAL-127 sealed 2026-05-02 (n=96 d=+2.64 within cohort vs anchor; p=9.24×10⁻⁴¹) | sealed v0.3.2 — CHK-3.2 tier-3 -4.31 anchor-SD; within-cohort discrimination layer load-bearing |
| Esophageal EAC | POSITIVE (stronger than ESCC by -1.06 d-units within cohort, p=1.50×10⁻¹¹) | VAL-127 sealed 2026-05-02 (n=89 d=+3.70 within cohort vs anchor; p=9.39×10⁻⁵⁰) | sealed v0.3.2 — Barrett-positive amplification +1.69 d-units within cohort |
| IBD pathway (Crohn's + UC) | NULL_pooled at Stage 1 (informative null clarifying Stage 1 specificity — cycling-class panel does NOT detect IBD) | VAL-128 sealed 2026-05-02 | sealed v0.3.2 — DISC-GE-006 Stage 1 panel scope clarification cookbook-wide; Stage 3 cell-fraction-shift is the operative deployment readout |
...
| Gastric STAD (VAL-126 sealed 2026-05-02) | LOW for Stage 1 architectural drift on tumor tissue substrate — VAL-126 shows monotonic positive across all 5 molecular subtypes (MSI +4.03, EBV +3.85, CIN +3.30, POLE +2.98 underpowered, GS +2.89); no bidirectional cancellation. UNKNOWN for blood-substrate. | Bass 2014 Nature 513:202; Cristescu 2015 Nat Med 21:449 |
| Esophageal cancer ESCC + EAC (VAL-127 sealed 2026-05-02) | LOW — both subtypes monotonic positive; subtype discrimination is in MAGNITUDE not direction. EAC +3.70 vs ESCC +2.64 within cohort (d_ESCC-EAC = -1.06, p=1.50×10⁻¹¹). No bidirectional cancellation pattern. | Krause 2016 Nat Genet; Hao 2018 Mol Cancer Res |
| IBD pathway Crohn's + UC (VAL-128 sealed 2026-05-02) | HIGH for Stage 3 readout BY DESIGN — T-cell expansion + myeloid depletion is the operational signature, not a confound. This is BIDIRECTIONAL BY POPULATION-FRACTION-SHIFT MECHANISM (DISC-GE-005), distinct from AD/PDAC bidirectional-cancellation Pattern 3. Stage 1 NULL with Stage 3 bidirectional is Pattern 7 (population-fraction-shift signature). | Strober 2007 Nat Rev Immunol; Globig 2014 J Crohns Colitis; VAL-128 sealed 2026-05-02 |
...
| Gastric (v0.3.2 outcome) | (not built — pooled A_immune fires consistently above 80-cell baseline NORMAL cutoff for all 5 molecular subtypes; no directional decomposition needed at v0.1) | NOT NEEDED at v0.1 (sealed VAL-126); v0.2 substrate-matched anchor expansion may revisit |
| Esophageal (v0.3.2 outcome) | (not built — pooled A_immune fires consistently above cutoff for both ESCC and EAC; subtype discrimination via Caggiano TIM panel covers Stage 2 layer) | NOT NEEDED at v0.1 (sealed VAL-127) |
| IBD pathway | (not applicable — Stage 1 cycling-class panel is informative null; the IBD signature is captured by Stage 3 cell-fraction deconvolution atlases, not by Stage 1 directional decomposition. Future IBD-epic v0.0 sprint may construct dedicated IBD-discriminating panel from top-20-30 CpGs of VAL-128 Stage 3 whole-blood findings — UniLIFE aCD8Tnv, Loyfer CD4T-cells_EPIC, Salas CD4T, Salas Neu) | NOT APPLICABLE at v0.1 deployment scope; future IBD-epic v0.0 path documented |
...
| Gastric STAD (TCGA-STAD VAL-126 sealed 2026-05-02) | down (T-cell + B-cell + NK depletion: Salas IDOL CD4T -1.92, CD8T -1.58, NK -0.86, Bcell -0.20) | down (Mono -0.90, Neu -0.45) | T-cell + myeloid depletion bidirectional pattern (advanced gastric cancer immune evasion); consistent with Lin 2019 J Hepatol, Kang 2020 Theranostics, Sundar 2021 Nat Commun |
| Esophageal cancer (TCGA-ESCA VAL-127 sealed 2026-05-02) | EAC > ESCC discrimination on Caggiano TIM tcell tile d_ESCC-EAC=-1.96; full Salas + UniLIFE lineage profile pending v0.2 expansion | EAC > ESCC discrimination on Caggiano TIM macrophage -2.08, neutrophil -2.09, monocyte -2.03; full lineage profile pending v0.2 | EAC homogenization gradient relative to ESCC across 13+ Caggiano TIM tiles is the strongest subtype discriminator |
| IBD pathway Crohn's + UC (GSE87650 VAL-128 sealed 2026-05-02) | UP — T-cell expansion: UniLIFE aCD8Tnv +1.72, UniLIFE CD8T +1.70, Loyfer CD4T-EPIC +1.66, UniLIFE CD4T +1.64, UniLIFE aCD4Tmem +1.61, Salas CD4T +1.58, UniLIFE aCD4Tnv +1.58, UniLIFE aTreg +1.56, Loyfer CD8T-EPIC +1.38, Loyfer NK-EPIC +1.38 | DOWN — myeloid depletion: Salas Neu -1.34, Salas Mono -1.14, Loyfer Neu-EPIC -1.01 | BIDIRECTIONAL POPULATION-FRACTION-SHIFT signature (DISC-GE-005); cross-atlas reproduction across UniLIFE + Salas + Loyfer EPIC. Strober 2007; Globig 2014; VAL-128 |
...
### §13.6b Gastric cancer STAD (VAL-126 sealed 2026-05-02)

**Cohort:** TCGA-STAD HM450 sesame Level 3 n=395 primary tumor + n=2 paired adjacent-normal; full molecular subtype + MSI scores + H. pylori + EBV serology + Lauren classification joined from cBioPortal stad_tcga (CIN n=202, MSI n=59, GS n=46, EBV n=29, POLE n=7, NotClassified n=52). Anchor TCGA-KIRC + TCGA-PRAD adjacent-normal n=210.
**Stage 3 atlases:** Salas Blood.EPIC IDOL 6-cell (production calibrated; primary), UniLIFE Guo 2025 19-cell (within-cohort self-cal v0.1), Loyfer EPIC immune subset 5-tile (CD4T_EPIC, CD8T_EPIC, B-cells_EPIC, NK_EPIC, Neutrophils_EPIC), Caggiano CelFiE TIM immune subset 8-cell.

**Salas IDOL 6-tile contrasts (n=395 vs anchor n=210):**

| Tile | Cell type | d_vs_anchor |
|---|---|---|
| CD4T | Helper T cells | **−1.92** |
| CD8T | Cytotoxic T cells | **−1.58** |
| Mono | Monocytes | **−0.90** |
| NK | Natural killer | **−0.86** |
| Neu | Neutrophils | **−0.45** |
| Bcell | B lymphocytes | **−0.20** |

**Lineup ranking (most negative to least negative):** CD4T < CD8T < Mono < NK < Neu < Bcell. **All NEGATIVE direction.**

**Pattern interpretation.** T-cell + myeloid depletion bidirectional pattern. Consistent with Lin 2019 J Hepatol, Kang 2020 Theranostics, Sundar 2021 Nat Commun documenting peripheral T-cell exhaustion + reduced lymphocyte/myeloid blood-compartment proportions in advanced gastric cancer. Direction reproduces published advanced-gastric-cancer immune-evasion signature.

**CHK-3.2 tier-3 caveat.** Stage 3 readout is a within-tumor-cohort vs anchor-cohort reading on TCGA HM450 sesame Level 3; cohort substrate baseline shift -5.02 anchor-SD vs KIRC+PRAD anchor affects absolute magnitudes. Direction reproduces published literature pattern; absolute magnitudes tier-3-invalidated for cross-cohort comparison.

**Within-cohort molecular subtype hierarchy (Stage 1 layer, robust to tier-3).** MSI (n=59 d=+4.03) ≈ EBV (n=29 d=+3.85) > CIN (n=202 d=+3.30) > POLE (n=7 d=+2.98 underpowered) > GS (n=46 d=+2.89). Preserves prereg-predicted CIMP-amplification ordering. MSI-H (n=67 d=+3.85) vs MSS (n=326 d=+3.27) +0.58 d-unit amplification. H. pylori Yes (n=20 d=+3.94) vs No (n=168 d=+3.71) modest +0.23 d-unit amplification, underpowered at v0.1.

**Card line in no-overclaim language:** *Stage 3 immune fine-tune fires consistent with peripheral T-cell + myeloid depletion in advanced gastric cancer immune-evasion biology. All six Salas IDOL immune-cell-type A-scores decrease in tumor vs anchor; pattern reproduces Lin 2019 / Kang 2020 / Sundar 2021 literature.*

### §13.6c Esophageal cancer ESCC + EAC (VAL-127 sealed 2026-05-02)

**Cohort:** TCGA-ESCA HM450 sesame Level 3 n=185 primary tumor (96 ESCC + 89 EAC) + n=16 paired adjacent-normal + n=1 metastatic; cBioPortal HISTORY_BARRETTS_ESOPHAGEAL_INDICATOR coding (Yes-USA 18 + Yes-UK 10 = 28 / No 118 / NotReported 39). Anchor TCGA-KIRC + TCGA-PRAD adjacent-normal n=210 (shared with VAL-126).
**Stage 3 atlases:** Salas IDOL + UniLIFE 19-cell + Caggiano TIM 19-tile (immune subset 8-cell + 11 broader tissue tiles all relevant to ESCC vs EAC subtype discrimination).

**Caggiano TIM panel ESCC vs EAC subtype discrimination (within-cohort d_ESCC-EAC):**

| Tile | Cell/tissue type | d_ESCC − d_EAC |
|---|---|---|
| erythroblast | Erythroid lineage | **−2.26** |
| eosinophil | Granulocytic lineage | **−2.23** |
| small_intestine | Intestinal epithelium | **−2.17** |
| megakaryocyte | Platelet lineage | **−2.17** |
| neutrophil | Granulocytic lineage | **−2.09** |
| dendritic | Dendritic cells | **−2.09** |
| macrophage | Tissue macrophages | **−2.08** |
| monocyte | Blood monocytes | **−2.03** |
| tcell | T lymphocytes | **−1.96** |
| fibroblast | Stromal | **−1.93** |

**Lineup pattern (most negative to least negative; all 13+ tiles fire EAC > ESCC at ~2 d-units within cohort).**

**Pattern interpretation.** EAC's homogenized methylation pattern looks more like generic-epithelial tissue than ESCC's preserved squamous-specific structure across the 19-tile broad-cell-type panel. Strongest ESCC vs EAC subtype discriminator in the GE sprint. **First cookbook example of within-cancer histological-subtype methylation discrimination at >1 d-unit magnitude outside MSI-tracking-Lauren cases (DISC-GE-002).** Consistent with Krause 2016 Nat Genet (EAC clonal Barrett-derived) + Hao 2018 Mol Cancer Res (ESCC squamous transformation) + Kaz 2011 Cancer Res (Barrett-EAC chronic CIMP methylation drift).

**Cell-of-origin retention finding (Stage 2 layer, also card-relevant for §13 interpretation).** EsoRef Epi_stratified d=-0.99 in ESCC (cell-of-origin retention in target tissue — squamous tumor retains methylation patterns consistent with squamous-stratified epithelial origin) and d=-0.05 in EAC (cell-of-origin signature lost). **First cookbook example of a gene-promoter atlas reading its target biology in one disease subtype within the same multi-cohort sprint (DISC-GE-003).**

**Within-cohort Barrett's amplification (Stage 1 layer, robust to tier-3).** Barrett+ (n=28) d=+4.50 vs Barrett- (n=118) d=+2.81 = +1.69 d-units within cohort — cleanest within-cohort biological signal in the GE sprint. Smoking strata (Lifelong-non, Reformed-≥15yr, Current, Reformed-<15yr) all within 0.6 d-units — informative null distinguishing methylation drift from mutational burden mechanisms.

**Salas + UniLIFE lineage profile.** Pending v0.2 expansion (sample sizes per subtype underpowered for full 19-cell sub-classification at v0.1; n=89 EAC + n=96 ESCC).

**Card line in no-overclaim language:** *Stage 3 Caggiano TIM panel fires consistent with EAC homogenization gradient relative to ESCC across 13+ tiles at ~2 d-units within cohort. EAC adenocarcinoma reads more like generic-epithelial tissue than ESCC's squamous-stratified epithelial signature. EsoRef Epi_stratified target-tissue retention (d=-0.99 in ESCC) is the discriminating per-subtype gene-promoter atlas signal.*

### §13.6d Crohn's disease + ulcerative colitis (VAL-128 sealed 2026-05-02)

**Cohort:** GSE87650 GPL13534 sorted-cell sub-experiment n=240 (CD=77, UC=79, HC=84; cell types monocytes 60 + CD4 59 + CD8 56 + wh-blood-companion 65). Main Ventham n=384 deferred to v0.1.1 expansion sprint (1.4 GB compressed supplementary file requires separate download + preprocessing).
**Stage 3 atlases:** Salas IDOL 6-cell + UniLIFE 19-cell + Loyfer EPIC immune subset 5-tile + Caggiano TIM immune subset 8-cell. Cross-atlas reproduction is the convergence check.

**Top T-cell expansion tiles (whole-blood substrate, n=65 wh-blood-companion subset):**

| Tile | Atlas | d_CD vs HC |
|---|---|---|
| aCD8Tnv | UniLIFE | **+1.72** |
| CD8T | UniLIFE | **+1.70** |
| CD4T-cells_EPIC | Loyfer | **+1.66** |
| CD4T | UniLIFE | **+1.64** |
| aCD4Tmem | UniLIFE | **+1.61** |
| CD4T | Salas IDOL | **+1.58** |
| aCD4Tnv | UniLIFE | **+1.58** |
| aTreg | UniLIFE | **+1.56** |
| CD8T-cells_EPIC | Loyfer | **+1.38** |
| NK-cells_EPIC | Loyfer | **+1.38** |

**Top myeloid depletion tiles:**

| Tile | Atlas | d_CD vs HC |
|---|---|---|
| Neu | Salas IDOL | **−1.34** |
| Mono | Salas IDOL | **−1.14** |
| Neutrophils_EPIC | Loyfer | **−1.01** |

**Lineup pattern.** T-cell expansion (CD4 + CD8 + Treg + NK) + myeloid depletion (Mono + Neu) bidirectional population-fraction-shift signature in whole-blood substrate. **Cross-atlas reproduction across UniLIFE 19-cell + Salas IDOL 6-cell + Loyfer EPIC immune subset.** Cross-atlas reproduction rules out single-atlas artifact.

**Stage 1 informative null.** |d_CD-HC| < 0.5 across all four cell-type strata (monocytes, CD4, CD8, whole blood). **DISC-GE-006: Stage 1 cycling-class architectural-drift panel does NOT detect IBD.** Stage 1 is tissue/cancer-specific, not generic chronic-inflammation marker.

**Mixture-attenuation reversal (DISC-GE-005, foundational cookbook lesson).** Pre-locked hypothesis: sorted-cell d ≥ 1.5x whole-blood d (mixture-attenuation hypothesis: separating cells should AMPLIFY within-cell-type drift signal). Observed: 40/93 tiles pass = 43%. Whole blood STRONGER than sorted cells. **Direction REVERSED from prereg expectation.** The IBD signature is a population-fraction shift, not within-cell-type drift; sorting cells eliminates the very signal being detected. **Stage 3 deconvolution atlases measure cell-type COMPOSITION SHIFTS in mixed-population substrates, NOT within-cell-type chronic-inflammation drift.** Re-frames Stage 3 interpretation across the cookbook.

**CD vs UC discrimination.** Modest overall; CD8 cleanest discriminator at d_CD-UC = -0.72 (consistent with CD's stronger Th1/CD8 effector memory signatures vs UC's Th2-skewed presentation per Strober 2007). monocytes_d_CD-UC = +0.35; CD4_d_CD-UC = +0.33; wh_blood_d_CD-UC = +0.26.

**Amendment-language scope.** Module_3 amendment language is supported but NOT a deployed per-patient IBD-detection product at v0.1. A dedicated IBD-discriminating panel (top 20-30 CpGs from VAL-128 Stage 3 whole-blood findings — UniLIFE aCD8Tnv, Loyfer CD4T-cells_EPIC, Salas CD4T, Salas Neu) is the path to a clean per-patient IBD-specific A-score, tracked for future IBD-epic v0.0 sprint NOT in scope for gastric+esophageal-epic.

**Cancer comorbidity routing.** When VAL-128 IBD signature fires AND a cancer signature (gastric, esophageal, HCC, etc.) ALSO fires, route to gastric-esophageal-epic Pattern H clinical_action_matrix (dual cancer + IBD comorbidity). Clinically plausible per Marcus stepbrother case (Crohn's history + HCC documented in Heath's motivating case).

**Card line in no-overclaim language:** *Stage 3 multi-atlas Stage 3 immune fine-tune fires consistent with active inflammatory bowel disease peripheral-blood population-fraction-shift signature: T-cell expansion (CD4 + CD8 + Treg + NK |d| 1.4-1.7) + myeloid depletion (Mono + Neu |d| 1.0-1.3). Cross-atlas reproduction across UniLIFE + Salas + Loyfer EPIC. Sorting cells eliminates the signal — the signature lives in population proportions, not in any single cell type's methylation pattern.*

### §13.8 How to interpret IBD-style population-fraction-shift Stage 3 readings (operational guidance for the future-AI-2-years-from-now interpreter; added 2026-05-03)

**This subsection exists because the Crohn's VAL-128 result is the cookbook's first sealed example of Pattern 7 (population-fraction-shift signature) — a Stage 3 firing pattern fundamentally distinct from the bladder VAL-122 broad-positive pattern documented in §13.7. A future AI looking at EDEAR Stage 3 output where T-cells fire POSITIVE at moderate-to-large magnitude AND myeloid fires NEGATIVE at moderate magnitude AND Stage 1 is INFORMATIVE NULL needs explicit interpretive guidance to distinguish this pattern from solid-tumor mixed-infiltration (§13.7 bladder) and from the four Stage-1-positive Stage-2-null pathways already in §9.**

**The pattern.** Stage 3 multi-atlas reading produces:
- Stage 1 INFORMATIVE NULL across all cell-type strata (|d| < 0.5; no architectural-drift signature on cycling-class panel).
- Stage 3 Salas IDOL + UniLIFE 19-cell + Loyfer EPIC immune subset converge: T-cell tiles (CD4T, CD8T, NK, Treg) fire POSITIVE at |d| 1.4-1.7 with cross-atlas reproduction.
- Stage 3 myeloid tiles (Mono, Neu) fire NEGATIVE at |d| 1.0-1.3 (Loyfer Neutrophils_EPIC, Salas Mono, Salas Neu).
- Cross-atlas convergence (the multi-atlas reproducibility check baked into Stage 3 RUN-everything Phase C).
- Stage 2 atlases (Boccellato, EsoRef, OEref, Layered Moss+Loyfer 25-tile, Caggiano TIM) read INFORMATIVE NULL on the cohort's GI-tissue tiles — the disease signature is in blood-compartment composition, not in any GI-tissue methylation pattern.
- Distinguishing test: sorted-cell sub-experiment yields SMALLER signal than whole-blood substrate (mixture-attenuation reversal — sorting eliminates the signal because the signature is in proportions, not in any single cell type's methylation pattern).

**The distinguishing biology.** When this pattern fires, the underlying biology is **active chronic inflammatory disease driving peripheral immune-population redistribution** — T-cell expansion (CD4 + CD8 + Treg + NK) with proportional decrease in monocytes and neutrophils. Substrates that produce this pattern: whole blood from active IBD (Crohn's + UC confirmed VAL-128); predicted: chronic viral infection (HIV chronic, chronic HBV, EBV reactivation); autoimmune diseases driving peripheral immune shifts (RA active, MS active flare, T1D active autoimmune phase); sepsis acute; GVHD post-transplant.

**How this is NOT §13.7 (bladder-style broad-positive multi-lineage infiltration).** §13.7 fires when the substrate is tumor TISSUE (not blood), the disease is solid-tumor microenvironment infiltration, and Stage 3 reads ALL cell types POSITIVE at moderate magnitudes (no NEGATIVE-firing tiles). Pattern 7 (this subsection) fires when the substrate is whole BLOOD (not tissue), the disease is chronic inflammatory immune-population redistribution, and Stage 3 reads bidirectional (T-cells UP + myeloid DOWN). Distinguishing test: what was the SUBSTRATE (blood vs tumor tissue)? Are myeloid tiles POSITIVE (§13.7) or NEGATIVE (Pattern 7)? If blood + bidirectional → Pattern 7. If tumor tissue + uniformly positive + Stage 2 cell-of-origin NEGATIVE → §13.7.

**How this is NOT Pathway 1 (terminal-class disease hidden by specimen problem).** Pathway 1 fires when Stage 1 is POSITIVE. Pattern 7 has Stage 1 NULL. Distinguishing test: what is the Stage 1 reading?

**How this is NOT Pathway 2 (hematologic/immune-compartment disease).** Pathway 2 fires when Stage 1 is POSITIVE on whole blood with malignant immune cells THEMSELVES being the disease. Pattern 7 has Stage 1 NULL with healthy host immune cells redistributing in proportion. Distinguishing test: Stage 1 NULL + Stage 3 bidirectional + sorted-cell signal SMALLER than whole-blood signal → Pattern 7. Stage 1 POSITIVE + heme-epic Stage 3 lineage profile (e.g., DLBCL B-cell-led) → Pathway 2.

**How this is NOT Pathway 3 (cardiovascular/systemic inflammatory).** Pathway 3 fires when Stage 1 is POSITIVE on whole-blood substrate. Pattern 7 has Stage 1 NULL. Distinguishing test: Stage 1 NULL → Pattern 7 (NOT Pathway 3).

**How this is NOT Pathway 4 (unexplained immune drift).** Pathway 4 applies to single Stage 1 positive at moderate magnitude with no Stage 2 or Stage 3 corroboration. Pattern 7 has Stage 1 NULL with strong Stage 3 cross-atlas corroboration. Distinguishing test: Stage 1 NULL + Stage 3 bidirectional with cross-atlas reproduction → Pattern 7 high-confidence chronic-inflammation pattern (NOT Pathway 4).

**How this is NOT Pattern 3 (AD-instance pooled-null + directional-pass).** Pattern 3 has the directional signature WITHIN the Stage 1 panel via z-scored decomposition (AD 7-CpG Rule A, PDAC 324-CpG directional). Pattern 7 has Stage 1 NULL with the signature living in Stage 3 cell-fraction deconvolution atlases entirely outside the Stage 1 panel. Distinguishing test: does a directional decomposition of the Stage 1 panel pass on holdout? If yes → Pattern 3 (AD or PDAC). If no, but Stage 3 fires bidirectional → Pattern 7 (IBD or autoimmune or chronic viral).

**The single-sentence rule for this pattern.** When Stage 3 RUN-everything fires bidirectional (T-cell tiles POSITIVE + myeloid tiles NEGATIVE) on whole-blood substrate, AND Stage 1 reads INFORMATIVE NULL across all cell-type strata, AND Stage 2 reads NULL on all GI-tissue + non-immune tissue tiles, AND a sorted-cell sub-experiment yields SMALLER signal than the whole-blood substrate, the EDEAR output should be reported as: *"peripheral-blood immune-population-fraction-shift signature consistent with active chronic inflammatory disease (IBD pattern); the signature lives in cell-type proportions, not in within-cell-type methylation drift."*

**The "is this gastric+esophageal+IBD comorbidity?" check.** If a cancer signature ALSO fires (Stage 1 + Stage 2 fire on a GI tissue) AT THE SAME TIME AS Pattern 7 fires on Stage 3, the patient may have coincident GI cancer + IBD comorbidity — clinically plausible scenario per Marcus stepbrother case (Crohn's history + HCC documented in Heath's motivating case). Route to gastric-esophageal-epic Pattern H clinical_action_matrix (dual cancer + IBD comorbidity workup).
...
### §14.11 Gastric vs Esophageal (gastric-esophageal-epic v0.1 sprint, sealed 2026-05-02)

Gastric STAD (VAL-126 n=395) and esophageal ESCA (VAL-127 n=185) share a cycling-class architectural-drift signature with subtype hierarchies determined by methylation drift mechanisms. STAD subtype hierarchy (MSI ≈ EBV > CIN > GS) reflects CIMP-amplification ordering; ESCA histological discrimination (EAC > ESCC by -1.06 d-units within cohort) reflects Barrett-driven chronic methylation drift accumulation in EAC vs acute squamous transformation in ESCC. Both cohorts share CHK-3.2 tier-3 substrate baseline shift on TCGA HM450 sesame Level 3 (-5.02 vs -4.31 anchor-SD) reflecting GI-tissue-class structural distribution differences vs solid-parenchyma anchor. Card-level deployment uses within-cohort molecular/histological subtype hierarchies as load-bearing layer; cell-of-origin direction interpretation pending v0.2 substrate-matched anchor.

### §14.12 Crohn's vs Everything (Pattern 7 population-fraction-shift hub)

Crohn's disease + UC (VAL-128) is the cookbook's first sealed example of a Pattern 7 candidate population-fraction-shift signature: Stage 1 informative null + Stage 3 bidirectional T-cell-up + myeloid-down + sorting-eliminates-signal. **DISC-GE-005 reframes Stage 3 atlas interpretation cookbook-wide:** Stage 3 measures cell-type composition shifts in mixed populations, not within-cell-type chronic-inflammation drift. Future autoimmune cards (RA, MS, T1D), chronic viral infection cards (chronic HBV, HIV, EBV reactivation), sepsis-acute, and GVHD-post-transplant cards should pre-specify population-fraction-shift expected at Stage 3 with Stage 1 informative null. The mixture-attenuation reversal test (sorted-cell d SMALLER than whole-blood d) is the discriminating diagnostic for population-fraction-shift mechanism vs within-cell-type drift mechanism.

### §14.13 Gastric vs Bladder (tier-3 substrate baseline + GI continuum)

Gastric VAL-126 (-5.02 anchor-SD on TCGA HM450 sesame Level 3) and bladder VAL-122 (mucosal cohort with tissue-class-mismatched CHK-3.1A floor, DISC-BLADDER-002) both surface tissue-class structural caveats on the cookbook-default solid-parenchyma anchor calibration. Different mechanisms — substrate-distribution shift on GI-tissue-class cohort (gastric) vs floor parameterization mismatch on mucosal cohort (bladder) — but related lesson: tissue-class-aware calibration is the v0.2+ path to clean cross-cohort interpretation. Both cards' v0.1 deployment narratives lean on within-cohort discrimination layers (subtype hierarchies for gastric, paired tumor-vs-adjacent-normal for bladder) that are immune to the substrate caveat.

### §14.14 Esophageal subtype discrimination vs prostate LE tile (DISC-PROSTATE-002 extension)

Esophageal VAL-127 EsoRef Epi_stratified d=-0.99 in ESCC and d=-0.05 in EAC extends DISC-PROSTATE-002 magnitude-based |d| threshold doctrine: gene-promoter atlases applied to multi-subtype cancer cohorts should be evaluated PER-SUBTYPE rather than at cohort level. ESCC retains squamous-stratified epithelial origin signature (cell-of-origin retention); EAC loses the cell-of-origin signature (cell-of-origin homogenization). First cookbook example of a gene-promoter atlas reading its target biology in one disease subtype within the same multi-cohort sprint (DISC-GE-003). The DIFFERENTIATING_CROSS_TISSUE_OVERREAD outcome class is now bidirectional: when a gene-promoter atlas reads strong in cohorts of non-target tissue, the discriminating test for whether the signal is genuine cross-tissue gene-promoter biology or atlas overread is a within-cohort subtype split on a target-tissue-adjacent cohort. The kidney-card cross-card calibration test (running EsoRef on TCGA-KIRC tumor) is the discriminating experiment, sharpened by this finding.
...
### v0.3.2 lessons (added 2026-05-03 from gastric-esophageal-epic v0.1 sprint)

This subsection exists because the gastric-esophageal-epic v0.1 sprint produced six DISC-GE discoveries that propagate to immune-class interpretation, two of which (DISC-GE-005 mixture-attenuation reversal and DISC-GE-006 Stage 1 panel scope clarification) are foundational cookbook-wide lessons. **A future AI 2 years from now reading EDEAR output will need this section to interpret IBD-style population-fraction-shift Stage 3 patterns and to correctly route cycling-class Stage 1 nulls on chronic-inflammatory cohorts.**

- **immune-atlas-LL-014 — DISC-GE-001 (2026-05-02): Within-cohort molecular-subtype hierarchy is robust under CHK-3.2 tier-3 substrate baseline shift.** When CHK-3.2 fires tier-3, within-cohort subtype d-hierarchies are immune to baseline shift because all subtypes share the same baseline. VAL-126 STAD demonstrates: substrate shift -5.02 anchor-SD invalidates absolute d-magnitudes for cell-of-origin direction interpretation, but within-cohort hierarchy MSI ≈ EBV > CIN > POLE ≈ GS preserves prereg-predicted CIMP-amplification ordering. **Operational implication for immune-atlas card:** when interpreting EDEAR output under tier-3 substrate caveat (per §2.3.4 CHK-3.2), the within-cohort molecular-subtype/risk-factor stratification block becomes the primary load-bearing analytical layer; cell-of-origin direction predictions get explicitly flagged as pending-substrate-matched-anchor in the EDEAR report. The future AI should not interpret a tier-3 substrate caveat as data integrity failure; it is a calibration-anchor-mismatch flag.

- **immune-atlas-LL-015 — DISC-GE-002 (2026-05-02): ESCC vs EAC subtype discrimination on methylation alone via Caggiano TIM panel.** First cookbook example of within-cancer histological-subtype methylation discrimination at >1 d-unit magnitude outside MSI-tracking-Lauren cases. Caggiano TIM panel produces consistent EAC > ESCC pattern across 13+ tiles (~2 d-units within cohort, p=1.5×10⁻¹¹ on Stage 1 alone). **Operational implication for immune-atlas card:** cards covering cancers with multiple distinct histological subtypes should pre-lock magnitude-based |d_subtype1-subtype2| ≥ 0.5 as a discrimination criterion. Future card preregs targeting cancers with multiple histological subtypes should include the Caggiano TIM panel as a primary discrimination atlas. The §11.4 lymphoid-vs-myeloid table now includes esophageal entries with EAC > ESCC discrimination magnitudes per Caggiano TIM tile.

- **immune-atlas-LL-016 — DISC-GE-003 (2026-05-02): Gene-promoter atlas reads target biology in target subtype.** EsoRef Epi_stratified d=-0.99 in ESCC (cell-of-origin retention) vs d=-0.05 in EAC (cell-of-origin signature lost). First cookbook example of a gene-promoter atlas reading its target biology in one disease subtype within the same multi-cohort sprint. **Operational implication for immune-atlas card:** gene-promoter atlases applied to multi-subtype cancer cohorts should be evaluated per-subtype rather than at cohort level. The DIFFERENTIATING_CROSS_TISSUE_OVERREAD outcome class is now bidirectional: when a gene-promoter atlas reads strong in cohorts of non-target tissue, the discriminating test for whether the signal is genuine cross-tissue gene-promoter biology or atlas overread is a within-cohort subtype split on a target-tissue-adjacent cohort. Extends DISC-PROSTATE-002 (magnitude-based |d| thresholds for cell-of-origin atlas preregs).

- **immune-atlas-LL-017 — DISC-GE-004 (2026-05-02): Risk-factor amplification within cancer cohort is robust to substrate baseline shift.** Within-cohort risk-factor stratification produces interpretable amplification signal even under tier-3 substrate baseline shift because the risk-factor-positive and risk-factor-negative subgroups share the same baseline. Barrett+ ESCA (n=28) Stage 1 d=+4.50 vs Barrett- ESCA (n=118) d=+2.81 = +1.69 d-units within cohort. Smoking strata informative null (all four within 0.6 d-units) — methylation drift ≠ mutational burden discriminator. **Operational implication for immune-atlas card:** every multi-cohort card should pre-lock at least one within-cohort risk-factor stratification when clinical metadata supports it (Barrett for esophageal, MSI for STAD, H. pylori for gastric, pack-years for lung, BMI/diabetes for HCC, family history for breast). Within-cohort risk-factor stratification is the cleanest signal under tier-3 substrate caveat.

- **immune-atlas-LL-018 — DISC-GE-005 (2026-05-02): FOUNDATIONAL — Mixture-attenuation reversal — Stage 3 deconvolution atlases measure cell-type composition not within-cell-type drift.** Pre-locked test (sorted-cell-d ≥ 1.5x whole-blood-d) FAILED in OPPOSITE direction predicted. 40/93 tiles pass = 43%; whole blood STRONGER than sorted cells. **Operational implication for immune-atlas card (FOUNDATIONAL):** Stage 3 deconvolution atlases (Salas IDOL, Loyfer immune tiles, UniLIFE 19-cell, Caggiano TIM) measure cell-type COMPOSITION SHIFTS in mixed-population substrates, NOT within-cell-type chronic-inflammation drift. Cards interpreting Stage 3 results should anchor the interpretation in population-fraction-shift language ("T-cell expansion + myeloid depletion in whole blood") not within-cell-type-drift language. Future card preregs should pre-specify whether the disease is expected to drive a population-fraction shift (Pattern 7 per §6.6b) vs a within-cell-type drift (Pattern 1, 2, or 6). **RE-FRAMES STAGE 3 INTERPRETATION ACROSS THE COOKBOOK.** Sealed Pattern 7 candidate added to ccl_031 taxonomy (§6.6b).

- **immune-atlas-LL-019 — DISC-GE-006 (2026-05-02): Stage 1 cycling-class architectural-drift panel does NOT detect IBD — informative null clarifying Stage 1 specificity.** |d_CD-HC| < 0.5 across all four cell-type strata (monocytes, CD4, CD8, wh blood) on GSE87650 Crohn's cohort. UC vs HC similar near-null. Class-of-disease finding: Stage 1 is tissue/cancer-specific, not generic chronic-inflammation marker. **Operational implication for immune-atlas card:** card preregs targeting chronic-inflammatory diseases (IBD, autoimmune, chronic viral infection) should NOT pre-lock Stage 1 elevation as a primary outcome and should expect Stage 3 immune-fraction shifts to be the load-bearing readout instead. **STAGE 1 PANEL SCOPE CLARIFICATION COOKBOOK-WIDE.** When EDEAR output shows Stage 1 NULL on a chronic-inflammatory-disease cohort, the future AI should not flag this as data integrity failure — it is the expected biology per DISC-GE-006.
...
| **v0.3.1 → v0.3.2** | additive gastric-esophageal-epic v0.1 sprint integration: §5 disease cross-reference table extended with esophageal + IBD pathway rows; §6.6b NEW Pattern 7 candidate (population-fraction-shift signature, DISC-GE-005); §11.1/.2/.3/.4 CCL-027 four-question tables expanded with gastric outcome + esophageal ESCC + EAC + IBD pathway rows (v0.3 prediction rows preserved verbatim); §13.6b gastric VAL-126 + §13.6c esophageal VAL-127 + §13.6d Crohn's VAL-128 sealed Stage 3 sub-cell-type signature blocks added; §13.8 NEW operational guidance subsection — how to interpret IBD-style population-fraction-shift Stage 3 readings (companion to §13.7 bladder-style broad-positive guidance); §14.11/.12/.13/.14 four new cross-card syntheses (gastric_vs_esophageal, crohns_vs_everything Pattern 7 hub, gastric_vs_bladder tier-3 GI-continuum, esophageal_vs_prostate DISC-PROSTATE-002 extension); §22.5 v0.3.2 lessons (immune-atlas-LL-014 through LL-019) covering DISC-GE-001 through DISC-GE-006 as they apply to immune-class interpretation, with DISC-GE-005 as foundational Stage 3 atlas interpretation reframe and DISC-GE-006 as Stage 1 panel scope clarification cookbook-wide |

---

*End of immune-atlas card v0.3.2 README.*
