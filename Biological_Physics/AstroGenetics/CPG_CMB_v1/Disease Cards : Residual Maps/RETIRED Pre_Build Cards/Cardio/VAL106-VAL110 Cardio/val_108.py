"""VAL-108 — pandas streaming, filter rows, then process per-sample."""
import csv
import json
import math
import statistics
...
LOYFER_TILE_CLASS = {
    ...
    'Erythrocyte_progenitors': 'progenitor', 'Adipocytes': 'stromal',
    'Cortical_neurons': 'terminal', 'Hepatocytes': 'secretory', 'Lung_cells': 'cycling',
    ...
    'Colon_epithelial_cells': 'cycling', 'Left_atrium': 'terminal', 'Bladder': 'cycling',
    'Breast': 'secretory', 'Head_and_neck_larynx': 'cycling', 'Kidney': 'cycling',
    'Prostate': 'secretory', 'Thyroid': 'secretory', 'Upper_GI': 'cycling',
...
print(f"Loyfer: {loyfer_df.shape}", flush=True)
...
needed_cpgs = set(loyfer_df.index) | unilife_cpgs | salas_cpgs
print(f"Needed CpGs: {len(needed_cpgs)}", flush=True)

# Read β matrix BUT only AVG_Beta columns + TargetID, and only relevant rows
# Use python csv to stream-filter to a small temp matrix
import csv as _csv

# First pass: get header and identify columns we want
with open('/home/claude/edear_working/SUBSTRATE_EQUIV/gse69138_ave_beta.txt') as f:
    rdr = _csv.reader(f, delimiter='\t')
    header = next(rdr)

# Want: TargetID column + all AVG_Beta columns (skip Detection Pval, ProbeID_A/B)
keep_indices = [1]  # TargetID is at column 1 (Index=0, TargetID=1)
sample_ids = []
for i, h in enumerate(header):
    if h.endswith('.AVG_Beta'):
        keep_indices.append(i)
        sample_ids.append(h.replace('.AVG_Beta', ''))
print(f"Keeping {len(keep_indices)-1} AVG_Beta columns + TargetID", flush=True)

# Stream the file, filter rows, write filtered TSV
filtered_path = 'gse69138_filtered.tsv'
with open('/home/claude/edear_working/SUBSTRATE_EQUIV/gse69138_ave_beta.txt') as fin, \
     open(filtered_path, 'w', newline='') as fout:
    rdr = _csv.reader(fin, delimiter='\t')
    wtr = _csv.writer(fout, delimiter='\t')
    h = next(rdr)
    wtr.writerow(['TargetID'] + sample_ids)
    rows_kept = 0
    rows_total = 0
    for row in rdr:
        rows_total += 1
        ...
        if cpg in needed_cpgs:
            ...
            wtr.writerow(kept_row)
            rows_kept += 1
        if rows_total % 100000 == 0:
            print(f"  scanned {rows_total:,} kept {rows_kept:,}", flush=True)
print(f"Filtered: {rows_kept} CpG rows × {len(sample_ids)} samples → {filtered_path}", flush=True)

# Now load the filtered TSV with pandas — small enough
beta_df = pd.read_csv(filtered_path, sep='\t', index_col='TargetID')
...
CHK_31A_EXTREME_MIN = 0.25
CHK_31A_MIDDLE_MAX = 0.13
CHK_31A_NVALID_MIN = 400000

print("Scoring samples...", flush=True)
results = []
for sid in beta_df.columns:
    sample_b = beta_df[sid].dropna()
    chk = chip_to_chk.get(sid, {})
    subtype = chip_to_subtype.get(sid, 'unknown')
    chk_pass = (chk.get('n_valid', 0) >= CHK_31A_NVALID_MIN and
                chk.get('f_extreme', 0) >= CHK_31A_EXTREME_MIN and
                chk.get('f_middle', 1) <= CHK_31A_MIDDLE_MAX)
    
    salas_b = sample_b[sample_b.index.isin(salas_cpgs)]
    if len(salas_b) > 0:
        p = float((salas_b >= 0.5).mean())
    ...
    stage2 = {}
    common = sample_b.index.intersection(loyfer_df.index)
    if len(common) > 0:
        sample_aligned = sample_b.loc[common]
        ref_aligned = loyfer_df.loc[common]
        for tile in loyfer_tile_names:
            ...
            if len(tc) > 0:
                ...
                stage2[tile] = {'A': float(diffs.mean()), 'n': int(len(tc)),
                                'class': LOYFER_TILE_CLASS.get(tile, '?')}
            else:
                stage2[tile] = {'A': None, 'n': 0, 'class': LOYFER_TILE_CLASS.get(tile, '?')}
    
    uni_b = sample_b[sample_b.index.isin(unilife_cpgs)]
    if len(uni_b) > 0:
        p = float((uni_b >= 0.5).mean())
...
print(f"QC pass: {len(qc_pass)} ({(1-qc_fail_rate)*100:.1f}%)", flush=True)

def cohen_d(a, b):
    a = [x for x in a if x is not None]
    b = [x for x in b if x is not None]
    if len(a) < 2 or len(b) < 2: return None
    sd_a = statistics.stdev(a); sd_b = statistics.stdev(b)
    pooled_sd = math.sqrt((sd_a**2 + sd_b**2)/2)
    if pooled_sd == 0: return None
...
print("\nStage 1 contrasts (Salas proxy):", flush=True)
stage1_pair_d = {}
...
print("\nStage 2 contrasts (Loyfer per-tile):", flush=True)
stage2_pair_d = {}
for i, st1 in enumerate(target_subtypes):
    for st2 in target_subtypes[i+1:]:
        pair = f"{st1}_VS_{st2}"
        ...
        print(f"  {pair} top 5 by |d|:", flush=True)
        for t, d in ranked:
...
for atlas, key in [('UniLIFE', 'stage3_unilife_H'), ('Salas', 'stage3_salas_H')]:
    stage3_pair_d[atlas] = {}
    print(f"  {atlas}:", flush=True)
    for i, st1 in enumerate(target_subtypes):
        for st2 in target_subtypes[i+1:]:
            d = cohen_d([r[key] for r in samples_by_st[st1]], [r[key] for r in samples_by_st[st2]])
            stage3_pair_d[atlas][f"{st1}_VS_{st2}"] = d
            print(f"    {st1} vs {st2}: {d}", flush=True)

stage1_strong = any(abs(v['d']) >= 0.5 for v in stage1_pair_d.values() if v['d'] is not None)
stage2_strong = any(abs(stage2_pair_d[p][t]['d']) >= 0.5 for p in stage2_pair_d for t in stage2_pair_d[p] if stage2_pair_d[p][t]['d'] is not None)
stage3_strong = any(abs(d) >= 0.5 for atl in stage3_pair_d for d in stage3_pair_d[atl].values() if d is not None)

if qc_fail_rate >= 0.10:
    outcome = "O5_DATA_INTEGRITY_FLAG"
elif stage1_strong:
    outcome = "O1_CARDIO_EPIC_3SUBTYPE_DIFFERENTIATED_AT_STAGE_1"
elif stage2_strong:
    outcome = "O2_CARDIO_EPIC_3SUBTYPE_DIFFERENTIATED_AT_STAGE_2_TILES"
elif stage3_strong:
    outcome = "O4_STAGE_3_DIFFERENTIATING"
else:
    outcome = "O3_CARDIO_EPIC_3SUBTYPE_UNDIFFERENTIATED"
...
output = {
    ...
    'stage2_pair_d_summary': {
        pair: {'top5': sorted([(t, v['d']) for t, v in tiles.items() if v['d'] is not None], key=lambda x: -abs(x[1]))[:5],
