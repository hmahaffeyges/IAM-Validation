"""VAL-110 — Cardio-epic on GSE84274 aortic tissue."""
import csv as _csv
...
print("Loading GSE84274 β matrix...", flush=True)
beta_df = pd.read_csv('sm.txt', sep='\t', skiprows=59, index_col='ID_REF',
...
print(f"GSE84274 GenomeStudio substrate CHK-3.1A:", flush=True)
...
geo_to_disease = {m['geo']: m['disease'] for m in metadata}
...
results = []
for sid in beta_df.columns:
    disease = geo_to_disease.get(sid, 'unknown')
    ...
    results.append({'sample': sid, 'disease': disease, 'chk_pass': chk_pass,
...
target_groups = ['normal', 'dissection', 'BAV_dilation']
samples_by_g = {g: [r for r in qc_pass if r['disease']==g] for g in target_groups}
...
print("\nStage 2 per-tile contrasts:", flush=True)
stage2_pair_d = {}
for i, g1 in enumerate(target_groups):
    for g2 in target_groups[i+1:]:
        pair = f"{g1}_VS_{g2}"
...
vasc_strong = any(abs(stage2_pair_d[p]['Vascular_endothelial_cells']['d']) >= 0.5
                  for p in stage2_pair_d if stage2_pair_d[p]['Vascular_endothelial_cells']['d'] is not None)
stage2_any = any(abs(stage2_pair_d[p][t]['d']) >= 0.5
...
diss_vs_bav = (stage1_pair_d.get('dissection_VS_BAV_dilation', {}).get('d') is not None
               and abs(stage1_pair_d['dissection_VS_BAV_dilation']['d']) >= 0.5) or \
              any(abs(stage2_pair_d.get('dissection_VS_BAV_dilation', {}).get(t, {}).get('d') or 0) >= 0.5
                  for t in stage2_pair_d.get('dissection_VS_BAV_dilation', {}))

if qc_fail_rate >= 0.10: outcome = "O5_DATA_INTEGRITY_FLAG"
elif vasc_strong: outcome = "O1_AORTIC_VASCULAR_DIFFERENTIATING"
elif stage1_strong or stage2_any or stage3_strong: outcome = "O2_AORTIC_ANY_TILE_DIFFERENTIATING"
elif diss_vs_bav: outcome = "O4_DISSECTION_VS_BAV_DIFFERENTIATING"
else: outcome = "O3_AORTIC_FRAMEWORK_UNDIFFERENTIATING"

print(f"\n========== OUTCOME: {outcome} ==========", flush=True)

output = {
    'val_id': 'VAL-110', 'prereg_sha256': '1041738ccc8bcdd45a4754d599a28ad80fde3a7b37b6c18b4d528f4fe0271bc8',
    'sealed_at': '2026-04-28T22:55Z', 'executed_at': datetime.now(timezone.utc).isoformat(),
    'cohort': 'GSE84274 ascending aorta n=24 (normal 6 + dissection 12 + BAV+dilation 6)',
    'substrate': 'GenomeStudio V2011.1, HM450K',
    'chk_31a_self_cal': {
...
with open('per_sample.csv', 'w', newline='') as f:
    fields = ['sample','disease','chk_pass','stage1_immune_H','stage1_immune_A','stage3_unilife_H',
    ...
    for r in results:
        w.writerow({
            'sample': r['sample'], 'disease': r['disease'], 'chk_pass': r['chk_pass'],
