# cardio-epic v0.3 — Cardiovascular EDEAR Card

**Version:** 0.3 (run-everything calibration ship)
**Built:** 2026-04-29 evening
**Validation tier:** multi_modal_validated + multi_atlas_calibrated
**Built under:** CHK-3.1A/B/C split convention (CHK-3.1C added 2026-04-29 from CCL-047) + CCL-041 calibration-before-scoring + CCL-048 substrate-normalization-required (CHK-0.7 gate added). v0.3 is the run-everything calibration ship: VAL-112 + VAL-113 calibrated three Stage 2 atlases (layered Moss+Loyfer **deduped**, EpiSCORE HeartRef bridged, Caggiano CelFiE TIM **array-bridged**) on TCGA HM450 sesame Level 3 adjacent-normal n=210 (KIRC + PRAD; same cohort that anchored VAL-106/107). Phase C executed all three atlases against all three cardio cohorts. 31,948 calibrated A-score readings sealed.
**Supersedes:** cardio-epic v0.2.2 (2026-04-29 same-day afternoon honesty patch). All sealed VAL outcomes from v0.1/v0.2/v0.2.1/v0.2.2 (VAL-108/109/110/111) preserved unchanged in audit-trail; v0.3 outcomes (VAL-112 calibration + VAL-113 calibration + Phase C cohort scoring) ADD to the same cohorts under correct calibration discipline.

## Stage 2 atlases in atlases_run (3 calibrated, all CHK-3.1A/B/C passed)
...
Per-tile healthy-floor A-score distributions sealed in VAL-112_calibration_results.json + VAL-113_calibration_results.json.

## Phase C run-everything findings

**GSE84395 PAH (n=39: control 18 + iPAH 11 + hPAH 10) — convergent strong cardiac signal across 3 atlases.** Caggiano `heart` = +1.42 control vs iPAH (STRONGEST single-tile signal of any atlas tested); HeartRef `CM` = −0.80 control vs iPAH; Loyfer `Vascular_endothelial_cells` = +0.83 control vs hPAH. **Three independent atlases, three different cardiac references, three convergent positive findings. PAH detection is the most robust cardio application validated.**

**GSE84274 BAV/dissection (n=24: normal 6 + dissection 12 + BAV 6) — multi-atlas exposes Loyfer's small-n confounders.** Loyfer normal_vs_BAV |d| > 2 on Colon_epithelial / Hepatocytes / Pancreatic — these tiles have no biological connection to BAV; with n=6 vs n=6 the within-group sd is artificially small and Cohen's d inflates spuriously. **Cardiac-specialized atlases (Caggiano `heart` = +1.40, `endothelial` = +1.52, `fibroblast` = +2.10; HeartRef `CM` = −0.60) show the real cardiac-cell-type signal cleanly without spurious tissue noise.** Multi-atlas triangulation catches single-atlas small-n artifacts. Mandate multi-atlas reporting for v0.3+ cardio scoring.

**GSE69138 stroke etiology (n=589) — convergent null across 3 atlases.** All three atlases agree max |d| = 0.19 across SVD/LAA/CE/atherothrombotic comparisons. Three different reference panels arriving at the same null is much stronger than VAL-108's single-atlas null. The absence of detectable methylation discrimination between stroke etiologies in whole blood is robust to atlas choice.

## Operational implications for v0.3 ship

1. **Stage 2 atlas stack: layered Moss+Loyfer (deduped) + EpiSCORE HeartRef bridged + Caggiano TIM array-bridged.** Three atlases, three references, all calibrated on TCGA n=210, all CHK-3.1A/B/C passed.
2. **PAH detection is the strongest validated cardio application.** Production scoring should report A-scores from all three atlases as triangulation. Caggiano `heart` tile is the strongest single-tile signal at d = +1.42 control vs iPAH.
3. **BAV detection requires multi-atlas triangulation to avoid small-n confounders.** Single-atlas |d| > 2 in n<10 cohorts should be flagged as small-n confounder candidate unless replicated by ≥1 other atlas.
4. **Stroke etiology in whole blood is a confirmed null.** Cardio Stage 1 immune workhorse (Xu-538 panel; VAL-110 d=+1.08 BAV in tissue) remains the primary cardio signal.
5. **Substrate normalization is REQUIRED before scoring (CHK-0.7 / CCL-048).** Raw IDAT files cannot be scored directly. They must first go through sesame normalization (recommended) or customer-specific calibration on within-cohort self-cal substrate.

## v0.2.2 → v0.3 changes (additive ship — sealed v0.2.x outcomes preserved unchanged)

1. **Atlas vault deduplication.** Layered Moss+Loyfer reference_atlas.csv deduped from 7,890 rows → 6,105 unique CpGs (CCL-047). Original preserved as `reference_atlas_v0.2_with_duplicates.csv` for audit-trail.
2. **Caggiano CelFiE TIM array-bridged.** New atlas built from Caggiano 2021 WGBS regions via HM450 hg19 manifest CpG-in-region intersection (1,581 → 254 array CpGs × 19 cell types).
3. **Three Stage 2 atlases calibrated on TCGA n=210.** VAL-112 (Loyfer + HeartRef) + VAL-113 (Caggiano TIM). CHK-3.1B q5 thresholds + per-tile healthy-floor A-score distributions sealed.
4. **Phase C run-everything execution.** All three atlases scored against all three cardio cohorts. 31,948 calibrated A-score readings sealed at VAL-112_113_unified.
5. **EpiSCORE HeartRef + Caggiano TIM promoted from atlases_deferred to atlases_run** after multi-atlas triangulation confirmed convergent cardio signal.
6. **CCL-047 LL-ATLAS-DEDUPE + CHK-3.1C atlas-deduplication gate** added to TESTING_CHECKLIST.md.
7. **CCL-048 LL-SUBSTRATE-NORMALIZATION-REQUIRED + CHK-0.7 substrate-normalization gate** added to TESTING_CHECKLIST.md.
8. **Part 22 substrate normalization pipeline architecture** added to PIPELINE_REFERENCE.
9. **§7.24 + §7.25 added to GAPE Reproduction Paper.**
10. **Atlas vault commit 57beb38 pushed to GitHub** with deduped Loyfer + Caggiano TIM bridged + INVENTORY.json updated with calibration anchors.

## Earlier (v0.2.2, v0.2.1, v0.2) honesty patches preserved as audit-trail below
