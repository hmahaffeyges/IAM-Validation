...
# ═══════════════════════════════════════════════════════════════════════════════
RULES = [
 ('SURF-RC1', 'Wrong surface: the per-cell A is defined on identity loci, never on discriminative marker panels.', "Marker panels are near-binary in the cell (extreme fraction 0.01-1.00, median 0.70); mean per-CpG entropy over them collapses regardless of the cell's state, so a cell's own atlas reference read 0.0099-1.06 (median 0.457) instead of 1.0. On identity loci the same references read 0.936-1.020. Enforced by kit/test_percell_physics.py RC1, propagate rule 11.", 'REFERENCE_AUDIT.md 2026-09-26', 'current'),
 ('SURF-RC2', 'Unmapped betas: the per-cell path scale-maps stage-1 betas onto the atlas before scoring, as the class gauge always did.', 'Raw betas sit ~0.07 above the atlas at the identity loci; every present healthy cell read SUPPRESSED (neutrophils 0.87, NK 0.80). Mapped: 0.968-1.053, NORMAL. Enforced by test_percell_physics RC2 (source order + a real healthy array).', 'REFERENCE_AUDIT.md 2026-09-26', 'current'),
 ('SURF-RC3', 'Two formulas, two surfaces, never crossed: mean-of-per-CpG-H on marker panels (LESSON-ASCORE-02); H-of-the-mean-beta on identity loci (RULING A3, the commissioned gauge).', "Applying the marker rule to identity loci read healthy cells SUPPRESSED (Jensen gap 0.25 median on real blood). Enforced by test_percell_physics RC3: the per-cell scorer equals stage_b_identity's H() to 1e-9; the formula name rides in every cell record.", 'SOP LESSON-SURFACE-01', 'current'),
 ('SURF-RC4', 'A runtime file the code cannot resolve is not part of the instrument, whatever a document says.', "percell_reference_v0_3.json sat in a folder not on the chain's search path for four days; the per-cell bands existed and were never loaded. Enforced by test_percell_physics RC4 and propagate rule 12, which measures readership from the live modules' source and fails the push if any file the code reads is missing from the inventory, the SOP, the reviewer manifest or COMPONENT_MAP.", 'SOP LESSON-SURFACE-01', 'current'),
 ('SURF-CAL', "Each cell's NORMAL is judged against its own centre in the calibration record, never a universal line.", "On the commissioned surface every present blood cell reads NORMAL in ~970 healthy arrays from four laboratories, at its own centre reproduced in all four: CD4 1.027, monocytes 1.018, neutrophils 1.008, B 0.996, CD8 0.976, NK 0.969, GMP 0.960. One laboratory offset per lab (agreeing in sign and order with the class gauge's z_lab) takes the median between-lab range from 0.055 to 0.007. percell_reference_identity_v1_0.json is the instrument's calibration record - noise floor per cell and scale check - not a healthy band competing with A = 1.0.", 'percell_reference_identity_v1_0.json', 'current'),
...
MCMC_HMIN = [  # G-002 / G-003b sampler settings (EDEAR s5)
 ...
 ("Deposited", "Samplers, the 37 reference cells as a table and reproduce.sh: <a href='https://doi.org/10.5281/zenodo.22905819' color='#1a5fb4'>10.5281/zenodo.22905819</a>. Re-run 2026-09-22 returned all eight floors inside their own posterior SDs (max difference 0.000245; R-hat < 1.001). The posterior samples were never written to disk by any run, so the deposit carries the re-run, not an archive of chains."),
]
ATLAS_BUILD_DETAIL = [  # EDEAR s5.4-5.6, SOP s26-29
 ...
 ("Walther vs NILC", "substrate-level L1 disagreement of ~0.10-0.25 was EXPECTED and did not gate; inference-level disagreement on the disease-relevant class did (SOP s32-33). NILC cut 2026-07-02, reinstated 2026-09-22 as the class-level second opinion (PROC-NILC-01)."),
...
CHAIN01 = {
 "input": "the eleven Stage-1 betas of PROC-CAL-01 (7 whole blood, 4 EPIC colorectal tissue); cpg_conductor.run_full at repo layout of 2026-09-19; MethylPhys/atlas/IAMAtlasREBUILD.csv; ages as documented, 60 where unknown",
...
CHAIN_LINKS = [
 ("MethylPhys/atlas/IAMAtlasREBUILD.csv(.xz)", "DATA", "The reference sky: 483,092 CpGs x 115 cell types, per-class MCMC posterior mean, sd and CI per CpG. Every patient is read against it. 2026-05-28, frozen."),
 ...
 ("cpg_patient_cmb.py + MethylPhys/atlas/healpix_mapping/", "CODE", "Stage 4.6: the patient's per-CpG z = (beta - mu)/sigma against the Atlas posterior, on the HEALPix NSIDE-128 sky (Plate 05). The residual map of one observation against a reference map with per-pixel uncertainty - the Planck workflow."),
...
CHAIN_TERMS = [
 ...
 ("HEALPix / NSIDE / Mollweide", "HEALPix: the equal-area sphere pixelisation used for CMB maps. NSIDE 128 -> 196,608 pixels. CpGs are laid on the sphere in genomic order (chr1 ... chrY, MAPINFO within chromosome), i-th CpG -> pixel floor(i x npix / n); ~2.46 CpGs per pixel, averaged. Mollweide is the equal-area full-sky projection every plate uses. Deterministic: same atlas + manifest -> byte-identical mapping. [MethylPhys/atlas/healpix_mapping/README]"),
 ("Component separation", "Cosmology's name for pulling a mixed signal apart into its sources; here, deconvolving a bulk sample into cell-type fractions (Stage 2). The Walther NNLS deconvolver is the composition the report stands on. NILC (the Planck needlet method) was tried as a second, cut on 2026-07-02 after it appeared to collapse on correlated blood mixtures, then rerun in September against the same arrays and vindicated: the disagreement was the finding, not the defect (PROC-NILC-01). It runs now as stage_2b_second_opinion in the conductor - compared with Walther AT CLASS LEVEL, where PROC-SEP-03 showed the atlas is separable, and reported as an agreement flag rather than as the composition. Cell-level disagreement inside one lineage is expected, because the atlas cannot split the blood classes. [walther_iam_deconvolver.py; commit c1be0c3]"),
 ...
 ("Null runner / the eight nulls", "The methylome's CMB null tests: no CPG-VAL is sealed until its declared nulls pass. N1 label permutation (1000x); N2 age-strata permutation; N3 sex-strata; N4 cohort split-half replication with consistent sign; N5 plate / array-position; N6 injection-recovery (inject a known signal into HC, recover it); N7 end-to-end simulation (synthetic patients with known truth through L1-L8); N8 look-elsewhere (Bonferroni/FDR). Required for every VAL: N1, N4; for end-to-end claims: N7. 'A VAL that doesn't declare its nulls is not a sealed VAL - it's a preliminary analysis.' [MethylPhys/chain/CPG_Null_Runner/cpg_null_runner.py]"),
...
FUTURE_GOALS = [
 ...
 ("after the anchor", "C(d): two-point correlation of residuals vs genomic distance, per class", "map rows 17, 23, 25; sprint C1",
  "The methylome's power spectrum. Look for characteristic-scale features (the 'acoustic peaks' of the map), with MASTER-style correction for masked CpGs. Needs CHR/MAPINFO on every CpG (the manifest is in MethylPhys/atlas/external_manifests).", "one session; the Atlas + anchor cohort"),
...
REFERENCE_LAYERS = {
 ...
 "residual": "What the three layers do NOT remove: the Sentrix-chip term within a laboratory (Karolinska ANOVA p = 8e-16, Uppsala 2e-19). A constant cannot touch it; it sits inside the band width today and is the next residual after the gauge switch. In code: MethylPhys/chain/lab_zero.py (panels under 40 refused; UNSET not reportable), Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json, beta_scale_maps_v1.json; kit test test_lab_zero.py recovers a synthetic -0.046 laboratory.",
}
CHAIN_TERMS += [
...
SCOPE["what_it_reports"] = ("In the author's words (2026-09-21): CPG reports whether a person's cellular write process is operating within the "
...
CMB_PROC = [("question","Can a patient's sky be drawn against a healthy expectation, and is a healthy person's sky quiet?"),
 ...
 ("standing","Row 4.6 COMMISSIONED by PROC-CMB-05 (fifth seal; bar restated 0.030->0.025 under the author's make-the-best-decision instruction, logged): C2' 4/4, gate 160/160, mapping and plate deterministic; the sealed script's own verdict True. Constant on every sky: a healthy sky is quiet at 2.6-3.2%, not 5%.")]
FALSIFICATION += [("PROC-CMB-04 C2' (2026-09-21) [EXPLORATION - see sealing rule]", "held-out healthy tail bar [0.03,0.08] in >=3/4 labs; measured 0.029/0.026/0.032/0.029 - scale ~1.1x conservative", "FAIL as sealed; recorded; row 4.6 withheld until PROC-CMB-05 passed the restated bar 4/4 the same day"),
                  ("PROC-CMB-03 C4' render-cap bar (2026-09-21) [EXPLORATION - see sealing rule]", "'<=3 healthy test arrays render a non-blood class' assumed a fixed composition for healthy blood (secretory, cycling, terminal absent) - a prior about biology, not a test of the gate; RETIRED", "RETIRED by the author same day"),
                  ("PROC-CMB-01 C1 retired sky formula (2026-09-21) [EXPLORATION - see sealing rule]", "z against a pure-class mean over the atlas posterior SD reads 60.7% of a healthy genome as |z|>2", "DEFECT CONFIRMED; module closed")]

TIER_PROC = [("question","Do the Stage 7 tier words follow one definition, only on a reportable gauge, and where do healthy people fall under them?"),
 ("found","Three tier definitions in code (report builder x2 with NORMAL ending at 1.04, a colour map with the same) against tier_breakpoints.json v1.3 (NORMAL ends at 1.01, the July move). Consolidated into cpg_tiers.py; every boundary tested both sides; no tier word without a reportable gauge (s108, UNMAPPED, UNSET); A >= 1/H_min reports AT_CEILING."),
 ("measured","1,379 healthy donors on the commissioned identity gauge under v1.3: SUPPRESSED 2.2%, NORMAL 67.5%, ELEVATED 30.2% (25.9-31.4% in each of four labs), >=1.07 one donor, >=1.10 none. Central 95% of healthy A'' = [0.954, 1.041] - the pre-July NORMAL band to three decimals. Analyst's sealed prediction (>=25% ELEVATED) correct."),
 ("standing","Row 7 COMMISSIONED (T1, T3, T4). PROC-TIER-02 then set NORMAL to the healthy central 95% - tier_breakpoints v1.4, [0.95, 1.04): 2.5% of 1,379 healthy read ELEVATED (predicted 2-3%), Warburg 1.07 admits one donor, breach 1.10 none. Analyst decision under the author's instruction, logged; the July 1.01 onset had been set on the retired marker-union statistic.")]
FALSIFICATION += [("PROC-TIER-01 T2 (2026-09-21)", "sealed prediction: under the July 1.01 onset >=25% of healthy donors read ELEVATED on the identity gauge; measured 30.2%, every lab 26-31%", "PREDICTION CORRECT; onset ruling requested from the author")]

MATCH_PROC = [("question","Does Stage 8 disease matching run on the commissioned chain, fail closed, and see a patient?"),
 ("fixed","The cell-of-origin map used to fall back to {} when missing (fail-OPEN); it now refuses to match (NOT AVAILABLE, zero candidates). Substrate firewall verified both ways. The per-cell A entering Stage 8 equals the sealed anchor formula to 4e-16."),
 ("measured","11 cached arrays: 0-5 cells enter the departure profile; no candidates. Healthy per-cell A on the separation surface is ~0.44-0.52 against class floors 0.77-0.98, so the (A - 1.0) departure with a below-floor gate excludes 104 of 115 cells. 52 of the 129 signature-matrix columns are tissue cells the blood atlas cannot score."),
 ("standing","REMOVED FROM THE CHAIN by the author the same day: the matrix and its cards come from the preliminary VAL record, and naming a disease from a pattern is not what the instrument does. The report shows cells detected, fractions, A per cell and class, placement, flags. The matrix is record-side; the fail-closed gate, firewall and surface checks stand in the code. This was exploration, not a sealed test (see the sealing rule, s14).")]
FALSIFICATION += [("PROC-CMB-05 C2' (2026-09-21)", "bar restated [0.025,0.08]; prediction 4/4 labs", "PASS 4/4 (0.029/0.026/0.032/0.029); row 4.6 commissioned by the script's verdict"),
                  ("PROC-TIER-02 U2 (2026-09-21)", "prediction: under NORMAL [0.95,1.04) ELEVATED+ = 2.0-3.0% of 1,379 healthy", "2.61% - PREDICTION CORRECT; tier_breakpoints v1.4"),
                  ("PROC-MATCH-01 M5 (2026-09-21) [EXPLORATION - see sealing rule]", "prediction: <10 present cells enter Stage 8's departure profile on healthy whole blood", "0-5 cells, median 3 - PREDICTION CORRECT; row 8 OPEN, gate named"),
                  ("PROC-MATCH-01 M1 (2026-09-21) [EXPLORATION - see sealing rule]", "origin gate fell back to {} on a missing file (fail-open)", "FIXED: fail-closed, kit test")]

SEALING_RULE = ("SEALING RULE (author, 2026-09-21). We seal when we have a working tool. A pre-registration is for a BUILT tool whose question is whether it performs against a bar; "
 "building it - finding the zero, the scale, the floors, the surface - is exploration and gets a dated working note, not a seal and not a falsification row. "
 "Retrospective marking: PROC-CMB-01 to -04 and PROC-MATCH-01 in this document were exploration; the sky was sealed once as PROC-CMB-05. The falsification register records claims that failed, not construction that iterated.")
...
PHYSICS = {
 ...
 "three_quantities": [
  ...
  ("A_c", "the gauge", "H(beta_bar_c) / H_min(c), beta_bar_c the sample's mean methylation over the identity loci of class c, on the mapped scale", "none (ratio)", "class x sample", "the sample, over the frozen reference", "cpg_conductor.stage_b_identity; then A'' = A - c(decade) - z_lab (s3.5)"),
 ],
