...
## What a plate is compared to (Stage 4.6, stated 2026-09-25)

**Never to a healthy picture.** There is no reference image anywhere in this comparison. Every pixel of the
plate is

```
z = (beta - SUM_c f_c * mu_c - m_lab) / s_lab
```

- `beta` - this specimen's methylation at that address
- `SUM_c f_c * mu_c` - what **this specimen's own composition** predicts there: each class's atlas mean `mu_c`
  weighted by the fraction `f_c` that Stage 2 measured **in this specimen**
- `m_lab` - this laboratory's measured zero
- `s_lab` - this laboratory's measured per-address spread across **its own** healthy donors

So the comparison is to a healthy cohort's **statistics**, not to an image, and the quantity to read is the
fraction of addresses beyond `|z| = 2` against the healthy range **2.6 to 3.2 per cent**. Red is above the
composition expectation, blue below, black not assessable (the class sits under its presence floor).

**Two plates are comparable only within one laboratory**, because `m_lab` and `s_lab` are that laboratory's
own. A plate has no meaning as a picture of disease; it is a map of where this specimen departs from what its
own composition predicts.

This is printed at the top of the report's Sky tab as well.

## The cosmology toolkit is a register, not a paragraph (2026-09-25)

`MethylPhys/chain/cmb_tools.py (historical path)` lists every method borrowed from CMB analysis with a check that runs on the finished
bundle and returns PASS, FAIL, NOT_RUN, NOT_APPLICABLE or NOT_BUILT for that specimen. The Safeguards tab
renders it; a FAIL is also emitted as a red flag. Nine entries are NOT_BUILT and listed deliberately -
[`ENHANCEMENTS.md`](../doors/ENHANCEMENTS.md) ranks them by impact. **To add a tool: append one entry with
its check. Nothing else needs changing.**
...
**How.** Fit the class design WITHOUT the candidate class, project that class's atlas profile off the columns
already in the fit, and regress one on the other unconstrained, weighting each address by the inverse of its
atlas posterior variance. Implemented in [`stage_2c_trace_detection.py`](../chain/stage_2c_trace_detection.py) against the frozen panel
`MethylPhys/chain/Runtime Matrices/trace_detection_panel_v1.json`; called by `cpg_conductor.run_full`, which passes the
RAW Stage-1 betas - the same input the composition solver receives.

**What it may say.** Detection from about 2 %; naming the class requires about 5 %. Whole blood only. No
fraction, no A and no tier are reportable for a detected trace class. A specimen on any other substrate
reads UNCALIBRATED and no call is made.

**Operator note.** Two guards refuse rather than mislead: a scale guard (the panel's thresholds were measured
on raw betas; mapped betas shift the statistic by about 24 units) and a substrate guard.

## Red flags, and why a run can no longer hide one (added 2026-09-25)

Every refusal, withheld number, deferred intake check, uncalibrated component and tripped guard is collected
on the report's **Red flags** tab, ordered by severity (STOP, WITHHELD, CAUTION, NOTE), with the same list as
JSON for a reader that is a program.

This exists because of a failure worth remembering: `matplotlib` was not installed in the chain environment,
so the specimen's own sky plate could not be drawn, the builder caught the error, and the page fell back to
reference illustrations that are identical in every report - saying so only in a grey note in the middle of a
long tab. Every report produced before 2026-09-25 carries reference figures only. `matplotlib` is now a
stated requirement in `chain/requirements.txt`, the runner warns at the start of a run if it is missing, and
the Sky tab opens with a warning when no plate could be drawn.

**A tier word carries its laboratory.** A placement outside the band is printed beside that laboratory's own
measured false-alarm rate and the bound it implies; a reading inside that bound is flagged as not
distinguishable from the laboratory's own healthy spread.

## Naming a run (added 2026-09-25)

`RUN-YYYYMMDD-NN`, assigned by [`run_sample.py`](../chain/MethylPhys_Interface/run_sample.py) at the moment of the run and written into the bundle, the
ledger row and the report. A run is one execution on one specimen: it makes no claim and passes no bar, so it
is neither a `VAL-###` (the frozen pre-atlas record) nor a `PROC-XXX-##` (a test with pre-registered bars).
`MethylPhys/chain/build_run_index.py` regenerates `MethylPhys/chain/example_runs/RUN_INDEX.csv` from every ledger in the tree.

**After any change to the chain or any sealed procedure, run `MethylPhys/chain/propagate.py (historical path)` before pushing.**
It regenerates everything derived and fails if any document has drifted from the tree.
...
> **Orchestrator note (v2.0.0).** Wherever this Part names [`walther_clinical.py (RETIRED 2026-09-25 -> RETIRED_2026-09/v1_conductor_2026-09/; MethylPhys/chain/disease_matching.py (historical path))`](../../RETIRED_2026-09/v1_conductor_2026-09/walther_clinical.py (historical path)) as the script that runs a stage, read `MethylPhys/chain/cpg_conductor.py` (2026-07; its docstring: "replaces the confusing walther_clinical.py"). The conductor pairs every per-cell A with its deconvolved fraction and treats a class below DETECT_FLOOR = 0.01 as absent. [`walther_clinical.py`](../../RETIRED_2026-09/v1_conductor_2026-09/walther_clinical.py (historical path)) remains in `MethylPhys/chain/` for reference only.
...
- §29  Step 2.2 — Per-class marker pool extraction from [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json)
...
- §35  Step 3.1 — Age-axis foreground subtraction (`age_axis_foreground.py (not part of the chain - superseded by the measured age curve)`)
...
- §44  Step 4.4 — Per-cell-type A-score (115 cell types) via [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py)
...
- §48  Step 5.2 — HC centroid load ([`mahalanobis_healthy_reference_v2_0_age_matched_derived.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v2_0_age_matched_derived.json) current production; v0_1/v0_2/v0_3/v0_4 retained for lineage)
...
- §59  Step 7.1 — Per-class A-score tier call ([`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json))
...
- §71  Step 9.2 — Literature anchors ([`literature_anchors.json`](../chain/Runtime%20Matrices/Literature_anchors_Report%MethylPhys/chain/Runtime Matrices/Literature_anchors_Report building/literature_anchors.json)) — reporting-layer translator
...
## §0.9 HARD-WON LESSONS (read before touching the deconvolver)

> **LESSON-SURFACE-01 — The per-cell A is physics only on the commissioned surface. Four silent regressions took it off that surface in one month, and no sentence in any document prevented any of them. Each is now paired with a machine check that refuses the push.** _(established 2026-09-26, by scoring real arrays through the chain and reading the output against the tier scale)_
>
> The author's diagnosis, which this lesson exists to honour: *"The drift is real, and I believe its mostly because the confusion that happens when we are using cohorts to actually test the physics itself."* When a cohort becomes the yardstick, a defect in the instrument looks like biology. Every one of the four was found only when a cell's **own atlas reference** — which must read A = 1.0 by construction — was scored and did not.
>
> | root cause | what it looked like | the physics it violated | the check that now refuses it |
> |---|---|---|---|
> | **RC1 WRONG SURFACE** — the per-cell A was computed on discriminative MARKER panels | a cell's own reference read far below 1.0 (Cortical_neurons 0.0099; median 0.457 across 115 cells) | marker panels are selected to be near-binary in the cell (extreme fraction 0.01–1.00, median 0.70, corr with A -0.961); mean per-CpG entropy over binary addresses collapses toward zero regardless of the cell's state. The A is defined on **identity loci** — addresses sitting *at* the floor, \|mean − H_min_β\| ≤ 0.05 — the class panels' own criterion | `MethylPhys/kit/test_percell_physics.py` RC1: six cells' own atlas means must read within (0.90, 1.10) **on surface `identity_loci`**; the surface is carried in every cell record and a lying tag fails |
> | **RC2 UNMAPPED BETAS** — the per-cell path scored raw stage-1 betas | every present cell in healthy blood read SUPPRESSED (neutrophils 0.87, NK 0.80) | the class gauge scale-maps stage-1 betas onto the atlas before reading them (`stage_1s_scale_map`, §21); raw betas sit ~0.07 above the atlas at the identity loci. Same betas mapped: 0.968–1.053 | RC2: `stage_a_cells` must call `stage_1s_scale_map` before `score_per_celltype` (source order), and a real healthy Uppsala array's present cells must read NORMAL against their own centres |
> | **RC3 CROSSED FORMULA** — LESSON-ASCORE-02's mean-of-per-CpG-H form was applied to identity loci | healthy cells SUPPRESSED; Jensen gap H(mean β) − mean H(β) of 0.25 median on real blood at these loci | **two rules govern two surfaces.** LESSON-ASCORE-02 (mean of per-CpG H) governs *marker* panels, which are bimodal. RULING A3 / PROC-SWITCH-01 (H of the mean β) governs *identity* loci and is the commissioned gauge (§41, §106). Crossing them in either direction is wrong | RC3: the per-cell scorer and `stage_b_identity`'s own `H()` must agree to 1e-9 on the same loci and betas; the formula name is carried in every cell record |
> | **RC4 UNREACHABLE REFERENCE** — [`percell_reference_v0_3.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Percell_Reference/percell_reference_v0_3.json) sat in `Runtime Matrices/Percell_Reference/`, a folder not on `_SEARCH` | per-cell bands existed for four days and were never loaded; `_find` raised | a runtime file the code cannot resolve is not part of the instrument, whatever a document says | RC4: the per-cell reference must resolve through `_find` with ≥ 100 cells; **propagate rule 12** measures readership from the live modules' source and fails if any file the code reads is missing from the inventory, this document's reference table, the reviewer manifest or COMPONENT_MAP — or is labelled "read by nothing" |
>
> **Two constraints that are always true and are therefore asserted:** A ≤ 1/H_min for every cell (binary entropy is at most 1.0 — a reading above the ceiling is a bug, not a result); and a cell at fraction 0 is not scored (**fraction is a detection gate, never a band matter**). **And one measured consequence:** on the commissioned surface every present blood cell reads NORMAL in ~970 healthy arrays from four laboratories, but each cell sits at its own centre — CD4 1.027, monocytes 1.018, neutrophils 1.008, B 0.996, CD8 0.976, NK 0.969, GMP 0.960, reproduced in all four laboratories — so a cell's NORMAL is judged against **its own centre** in [`percell_reference_identity_v1_0.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Percell_Reference/percell_reference_identity_v1_0.json), never against a universal line. That file is the instrument's *calibration record*: its noise floor per cell and its scale check. It is not a "healthy band" competing with A = 1.0; the physics defines healthy.
>
> The tests these four regressions would have failed, had they existed, are the ordinary ones: *score the reference; does it read 1.0?* No cohort was needed to find any of them, and no cohort could have.

> **LESSON-ASCORE-02 — ON DISCRIMINATIVE MARKER PANELS the A-score is mean_i(H(β_i)/H_min), the MEAN of per-CpG entropies, NEVER H(β_mean)/H_min. (On IDENTITY loci the commissioned form is the reverse — RULING A3, §41/§106 — see LESSON-SURFACE-01 RC3: two rules, two surfaces, never crossed.) Score each locus, THEN average — never average β first.** _(established 2026-06-30, end-to-end reproduction of the sealed GSE51032 anchor)_
>
...
> - **Guard:** [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) carries a unit test that scores a known bimodal panel and asserts the mean-of-per-CpG value. See §105 for the full record and the open convention/Recipe items.
...
>
> When real-IDAT Tier-2 output resolves only a handful of cells (e.g. 4 immune cells, CD4/monocytes zeroed), the temptation is to blame NNLS conditioning over ~115 correlated columns and "fix" it with a hierarchical within-class solve. **This was tested and is wrong on both counts:**
> - **Global NNLS recovers known mixtures PERFECTLY.** Synthetic blood mixtures built from the atlas's own profiles came back at MAE = 0.000, every cell exact (CD4 and monocytes included), nothing invented. It stays robust under noise: recovers all 6/6 cells up to sd = 0.10 (MAE 0.031). The solver is not the problem.
> - **Hierarchical within-class refinement is WORSE — it invents cells.** It manufactured HSC/GMP/neurons not present in the mixture; on pure monocytes it smeared 1.0 → mono 0.59 / GMP 0.32 / HSC 0.09 (MAE 0.271). It does NOT ship.
> - **The real-data collapse is REFERENCE MISMATCH.** Real GSE180683 blood β fits the atlas ~2.6× worse than a clean synthetic mixture (class residual 0.0615 vs 0.024). Real patient CD4/monocyte signal does not match the atlas profiles closely enough to earn weight, so NNLS — correctly — lands on the subset that does match and zeros the rest.
>
> **Consequences (binding):** (1) The cell tier is INDICATIVE by design in v0.1; the two-tier report (class RELIABLE / cell INDICATIVE) is the validated honest posture, not a placeholder. (2) Richer per-cell resolution is **reference/atlas work** (why does real β fit ~2.6× worse?), **not a solver swap** — and any such work must clear a composition-recovery + null suite before it touches production. (3) "More cells resolved" is only progress if they are resolved CORRECTLY; spreading mass to fill a chart is worse than honest sparsity. (4) Validation-before-trust caught both the wrong diagnosis and the bad fix — judge a deconvolver by recovery on known mixtures, never by theory.
...
- **Stage 4 — A-score computation.** For each architectural class and cell type, compute the per-CpG Shannon entropy H(β_i) at each marker locus, divide each by the frozen H_min(class), and average across the panel: A = mean_i( H(β_i) / H_min(class) ). The β values are NEVER averaged before the entropy is taken (that is the H(β_mean) regression — see §105). The result is the A-score — a dimensionless ratio measuring departure from the architectural floor. *(Entropy scoring, formally between L3 and L4)*
...
| **L4** | Component separation | Walther IAM Deconvolver + NILC v2 cross-check + age-axis foreground module (Phase B3) | Sex, batch, ancestry, smoking foreground modules formalized; foreground_registry.py (not part of the chain - no foreground registry exists; the decision was not to subtract) to chain them | **C** (was D before Phase B2.1 + B3) |
...
|---|---|---|
| `cfdna_weight.json (not part of the chain - a placeholder that was never derived)` | `substrate == "plasma_cfdna"` | Silently skipped at Stage 7 (see §61) |
| `family_history_multiplier.json (not part of the chain - the chain applies no risk multiplier)` | `patient_metadata.family_history` present and non-empty | Silently skipped at Stage 9 (§73) — Stage 9 falls back to overall-population prior with the absence noted in audit trail |
...
The atlas is the SOURCE for all runtime artifacts but is itself proprietary IP. The Recipe (§9 of `v1_CPG_Recipe.md (no such file in this repository)`) is the vault. The runtime artifacts are the public surfaces derived from the vault.
...
| Stage 3 (age foreground) | `age_axis_foreground.py (not part of the chain - superseded by the measured age curve)` | Per-CpG age-drift slopes ([`reference_age_curve_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json)) |
| Stage 4 (A-score) | [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) | Per-class marker CpGs + H_min per class |
| Stage 4 (per-cell-type A-score) | `iamatlas_a_scoring.py` | Per-cell-type marker CpGs from [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json) |
| Stage 5 (Mahalanobis) | [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) | HC centroid + covariance from [`mahalanobis_healthy_reference_v2_0_age_matched_derived.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v2_0_age_matched_derived.json) (current production, n=1,721; v0_1/v0_2/v0_3/v0_4 retained for lineage) |
| Stage 6 (cellular age) | [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) | 80-cell age × class baseline curve |
...
## §5. The eight frozen floors H_min *(v2.0.0: formerly "The Mahaffey Number (H_min)" — H_min and the Mahaffey number M = 20.94 are different quantities)*
...
When a patient's A-score = mean_i( H(β_i) / H_min(class) ) exceeds 1, the cell has paid more than the floor — it has departed from the architectural minimum. When it falls below 1, the cell is operating below the architectural floor, which is itself a measurement (suppressed below baseline, the opposite direction of departure). (On marker panels the entropy is taken per locus and then averaged — see §105. The class gauge on identity loci uses H(β_mean)/H_min against its own reference — see §106.)
...
> **A_class = mean_i ( H(β_i) / H_min(class) )** over the marker loci i

The entropy is computed **per locus** and then averaged. It is NOT H of the panel-averaged β — averaging β first is the regression that re-introduces the very cancellation this section exists to defeat (a bimodal locked panel averages to ~0.5, which entropy misreads as maximal disorder; see §105).
...
Because of this symmetry, when disease drives a locus's β value **away from its equilibrium toward 0.5** — regardless of which direction in β space — that locus's entropy goes up monotonically. When disease drives it **toward the extremes (0 or 1)**, that locus's entropy goes down. The A-score captures **departure from H_min in entropy space**, which is direction-agnostic in β space by construction. This direction-agnostic, no-cancellation behavior holds **only because each locus is scored individually before averaging** — taking the entropy of the panel mean would let opposing loci average to ~0.5 and cancel, the failure mode §105 records.
...
When a patient's per-locus entropies average to mean_i( H(β_i) ) > H_min(class), the patient's cellular architecture has DEPARTED from the floor — in whichever β-space direction. Hypermethylation and hypomethylation BOTH register as departure-from-floor in entropy space, contributing to A in the same direction, because each locus raises its own entropy as it walks toward 0.5.
...
This boundary is **enforced at Stage 9 (report assembly).** Every word that goes into the rendered report is checked against the Stage 6 language collapse ([`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json) engine → customer mapping) and the literature anchors. No engine-internal language ("FLOOR_BREACH", "URGENT") appears in the customer report. The customer sees "SIGNIFICANTLY_ELEVATED" with literature context.
...
> production engine — see [`chain_inventory_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json)), the SOP now says so honestly. Where
...
> are documented in `chain_inventory_v1.json`; any path NOT in that inventory should be
> treated as not-yet-existing until verified against the repo.

---


*Continuation of CPG_Chain_of_Custody_SOP_v1_PART_I.md (not part of the chain - this document is the current SOP; earlier parts are in git history). Read Part I first.*
...
**As applied in code.** `step_0_1_idat_arrival(manifest_entry, grn_path, red_path, intake_log_path)` runs
...
**Files invoked.**
- Intake handler: `<intake handler inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**As applied in code.** `step_0_2_manifest_creation(step_0_1_result, questionnaire_answers, manifest_dir,
...
type is how a sample gets scored against the wrong probe set. [`run_sample.py`](../chain/MethylPhys_Interface/run_sample.py) hashes a cleartext identifier
...
**Files invoked.**
- Manifest builder: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Schema validator: `<manifest schema — TBD per orchestrator design>`
- Per-sample record output: `<intake staging directory — TBD per orchestrator design>per_sample.csv`
...
**Files invoked.**
- Hash module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**As applied in code.** `step_0_4_control_probe_validation(record, grn_path, red_path, control_summary)`.
...
**Files invoked.**
- Control probe extractor: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**As applied in code.** `step_0_5_detection_pvalue_qc(record, probe_intensities, neg_control_stats)`, with
...
**Files invoked.**
- Detection p-value calculator: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Bead count reader: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**As applied in code.** `step_0_7_call_rate(record, detection_pass_mask, bead_pass_mask)` — the fraction of
...
**Files invoked.**
- Call rate calculator: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Sex predictor: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**As applied in code.** `step_0_9_decision_gate(record, verdict_log_path)` reads the statuses the eight
...
surfaced two gates later as a detection failure for exactly this reason. [`run_sample.py`](../chain/MethylPhys_Interface/run_sample.py) tests for a
...
**Files invoked.**
- Decision gate: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Dye-bias correction module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` (uses `minfi`/`methylprep` upstream)>`
...
**Files invoked.**
- Normalization module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` (uses `minfi`/`methylprep` upstream)>`
...
**Files invoked.**
- Batch correction module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` (wraps `sva::ComBat`/`combat-py`)>`
...
**Files invoked.**
- BS efficiency calculator: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- β calculator: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- β QC module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Output packager: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
Location: `<β matrix output — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>` (plus `.json` provenance).
...
**What this step does.** From the loaded atlas, extract the per-class marker CpG lists — the subset of CpGs that are maximally informative for each architectural class. These are pre-computed (delivered as the runtime artifact [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json)); this step loads them and pairs them with the patient's β values at the corresponding CpGs.

**Inputs.** The loaded IAMAtlas object from §28 + the per-sample β matrix from §27.

**Atlas reference.** **IAMAtlas consulted.** Specifically: the marker CpG lists at `MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json` (115 cell types × 100 markers, derived from IAMAtlas REBUILD's per-cell-type posteriors by the one-vs-rest top-N algorithm).

**Files invoked.**
- Marker loader: `<marker-pool extraction inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Marker artifact: `MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json`
...
**Atlas reference.** **IAMAtlas consulted.** Specifically: the per-class β posterior means at marker CpGs serve as the reference matrix X in the NNLS system. Loaded from `MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json` extended with the atlas posterior moments at those CpGs.

**Files invoked.**
- Walther deconvolver: `Biological_Physics/MethylPhys/chain/Walther_iam_deconvolver/walther_iam_deconvolver.py`
...
**Files invoked.**
- NILC deconvolver: `MethylPhys/chain/nilc_celltype_deconvolver.py`
...
**Canonical cross-references.** Phase B2.1 finding doc at `Biological_Physics/chain_of_custody/L4_component_separation/Phase_B2_1_FINDING.md (historical path) (not called by run_full - a pre-build finding, kept in the record)`. Roadmap §10.1.1 L4 row.
...
**Files invoked.**
- Stage 2 packager: `<Stage 2 packaging inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
Location: `<Stage 2 output — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`.
...
**Atlas reference.** **IAMAtlas consulted.** Specifically: the per-CpG age layer at `MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json` (8,199 CpGs × {α intercept, γ slope per year, R², n_samples}), built from EPIC-Italy HC by the Phase B3 age-axis foreground module.

**Files invoked.**
- Foreground module: `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/components/age_axis_foreground.py (not part of the chain - superseded by the measured age curve)`
- Age layer: `MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json`
...
**Canonical cross-references.** Recipe §4.1 (Age foreground). Roadmap §10.2.2 Phase B3 (the age-axis module). Phase B3 finding doc at `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/age_clock/Phase_B3_FINDING.md (not called by run_full - a pre-build finding, kept in the record)`.
...
**Decision points.** v1.2 default behavior: PASS THROUGH (no β-level subtraction applied) until `IAMAtlas_smoking_layer.csv (not part of the chain - no foreground layer is subtracted; smoking is measured as a null on this gauge)` is fit at v1.3. Stage 7 smoking-bin threshold-stratification in `tier_breakpoints.json v1.2` (`tier_by_smoking_bin.elevated_floor_by_bin`) absorbs the bulk effect as interim mitigation: current → ELEVATED floor 1.10; former_0_5y → 1.08; former_5_15y → 1.07; former_15plus_y → 1.05; never → 1.04. Layer CSV FIT 2026-06-06: apply β-level correction, Stage 7 smoking-bin stratification interim mitigation retires.
...
**Files invoked.**
- Stage 3 packager: `<Stage 3 packaging inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
Location: `<Stage 3 output — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`.
...
**Implemented in:** [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py); [`cpg_gauge_engine.py`](../chain/cpg_gauge_engine.py); called by [`cpg_conductor.stage_b_classes`](../chain/cpg_conductor.py) - runs in the live path
...
**Atlas reference.** **IAMAtlas consulted.** Specifically: the per-class marker CpG lists at `MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json`. (Markers are AGGREGATED to class — each class's marker pool is the union of its constituent cell types' marker pools.)

**Files invoked.**
- A-scoring module: `MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py` (specifically: `score_per_class()`)

**The math.** For each class c, retain the per-locus β vector:
> **β_vector_c = { β_corrected[i] : i in class_markers[c] }**  (no averaging at this step)

**CANONICAL (corrected v1.4.0).** H is computed **per locus and then averaged** — A_c = mean_i( H(β_i) / H_min(c) ) — **NOT** H of the panel-mean β. The choice matters and is the whole regression: by Jensen's inequality H(mean β) ≥ mean(H(β)), so the two formulas give different numbers. The validated instrument that produced every sealed VAL (and the +2.088 anchor) uses the **mean of per-CpG entropies**; the H-of-the-mean form inflates every score into false breach. **Recipe §6.3 is cited in prior versions as declaring "H of the mean" canonical — that citation must be corrected at the vault source (flagged in the v1.4.0 changelog; see §105).** Any formula that does not reproduce the sealed anchor is wrong by definition.

**CMB equivalent.** The right analog is the **power spectrum C_ℓ**, which is the *mean of per-mode power* (each multipole coefficient is squared, then averaged) — never the power of the mean coefficient, which vanishes by isotropy. The methylome analog of "information per component" is therefore mean_i( H(β_i) ) — per-locus information averaged — not H of the blended β. Averaging β first and then taking entropy is the methylome equivalent of averaging the a_ℓm coefficients before squaring: it destroys the very signal the statistic exists to measure.
...
**Outputs.** A per-class panel of per-locus β values (8 panels, ~2,978 loci each), passed to §42 for per-CpG entropy. No averaging has occurred yet.
...
**Files invoked.** Same module as §41 ([`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py)).

**The math.** For each marker locus i:
> **H_i = −β_i · log₂(β_i) − (1 − β_i) · log₂(1 − β_i)**

(with the convention 0 · log(0) = 0). H is bounded [0, 1] — maximum at β = 0.5, zero at β ∈ {0, 1}. A locus locked near 0 or 1 carries low entropy (at/near floor); a locus drifting toward 0.5 carries high entropy (departure). This is computed for **every** locus before any averaging — the bimodality of a healthy marker panel is preserved, not blended away.

**CMB equivalent.** This is the **per-mode power** — the squared coefficient at a single multipole, before the average that forms C_ℓ. Each locus's entropy is the methylome's per-mode information content. The averaging into a class-level number happens in §43, exactly as C_ℓ averages per-mode power.
...
**How it's the same in principle.** Both compute a per-component information-theoretic quantity at the finest available resolution, to be averaged in the next step.

**Outputs.** A per-class set of per-locus entropies (8 panels, ~2,978 values each), each in [0, 1].

**Decision points.** Proceed to §43.

**Failure modes.**
- *β_i = 0 or β_i = 1 (boundary loci).* H_i = 0 by convention. A perfectly locked locus contributes a 0 to the average — it sits exactly at maximal commitment, the opposite of departure.

**Canonical cross-references.** Recipe §6.2 (Shannon entropy). Roadmap §7 (Bidirectional calibration solved by entropy space).

**CPG Plate references.** Plate 3 (the bimodal vs Gaussian texture comparison — the methylome's non-Gaussianity is exactly why the entropy must be taken per locus, not on the blended mean).
...
Equivalently: score each locus against the floor, then average. This is the mean of per-CpG entropies, NOT H of the panel-mean β (see §41, §105). It reproduces every sealed VAL anchor exactly.
...
**Inputs.** Cleaned β matrix from §40 + per-cell-type marker CpG lists from [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json) + the `celltype_to_class` mapping from the same artifact.
...
**Files invoked.** [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) (specifically: `score_per_celltype()`).
...
2. Compute the per-locus entropy H(β_i) at each marker locus
3. Look up class assignment: c = celltype_to_class[ct]
4. Compute A_ct = mean_i( H(β_i) / H_min(c) ) — the mean of per-CpG entropies over the cell type's loci (NOT H(β_mean); see §41, §105)
...
**Files invoked.**
- Same module: [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py)
...
- For each panel locus i: H(β_i)
- A_panel = mean_i( H(β_i) / H_min(class_anchor) ) over i in panel_cpgs (NOT H of the panel-mean β; see §41, §105)

For the Xu-538 panel anchored to immune class: A_panel = mean over the 538 loci of H(β_i) / 0.838889.
...
**Files invoked.**
- Stage 4 packager: `<Stage 4 packaging inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
Location: `<Stage 4 output — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`.
...
Location: `reports/{patient_id}/stage_4_5/{patient_id}directional_panels_v1_0.json`.
...
The HEALPix mapping is generated one time per atlas version by `Biological_Physics/MethylPhys/atlas/healpix_mapping/generate_cpg_healpix_mapping.py`. Production mapping is committed; the generator script exists so the build is reproducible from inputs (IAMAtlas REBUILD CSV + EPIC v1 B4 manifest) without manual intervention.
...
**Canonical cross-references.** BUILD_SPEC v1.2 §3.5b (CPG Plates) + §5 Stage 4.6. Brightness_Comparison/README. HEALPix mapping README at `MethylPhys/atlas/healpix_mapping/README_HEALPix_Mapping.md`.
...
**Implemented in:** [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py); called by [`cpg_conductor.stage_5_mahalanobis`](../chain/cpg_conductor.py) - runs in the live path
...
**Inputs.** Per-sample per-cell-type A-scores + status codes from <A-score output — emitted by [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) (real module per inventory)>.

**Atlas reference.** Indirect — the 115 cell-type ordering is fixed by the atlas's celltype_to_class mapping.

**Files invoked.** [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) — class `MahalanobisHealthyHull`, method `_assemble_patient_vector(per_celltype_a)`.
...
**Inputs.** Current production: `mahalanobis_healthy_reference_v2_0_age_matched_derived.json` (n_hc=2,523 from GSE51057 + GSE51032 + GSE40279 Hannum + GSE50660 Tsaprouni + GSE144858 AddNeuroMed + GSE153712 AIBL + GSE53740 GIFT + GSE141682 Han Chinese — 8 cohorts, 4 populations). Engine loads whichever version is named in BUILD_SPEC §3.4b. Prior versions (v0_1 n=601, v0_2 n=1,257) retained in same folder for traceability.
...
**Files invoked.** [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) — `MahalanobisHealthyHull(reference_path)`.
...
**Files invoked.** [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) — `MahalanobisHealthyHull.score(patient_vector)`.
...
**Files invoked.** [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) — `MahalanobisHealthyHull._decompose_axes(patient_vector)`.
...
**Outputs.** <Mahalanobis output — emitted by [`iamatlas_mahalanobis_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/iamatlas_mahalanobis_scoring.py) (real module per inventory)> carrying:
...
**Implemented in:** [`reference_age_curve_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json); [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py); called by [`cpg_conductor.stage_6_cellular_age`](../chain/cpg_conductor.py) - runs in the live path

**What this step does.** Receives the patient's 8 per-class A-scores from Stage 4 (§43). These are the inputs to the inversion.

**Inputs.** Stage 4 output (<A-score output — emitted by [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) (real module per inventory)>).

**Atlas reference.** Indirect.

**Files invoked.** [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) — class `IAMCellularAge`, method `score_patient(per_class_a)`.
...
**Files invoked.** [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) — `IAMCellularAge.load_reference()`. Or directly via [`reference_age_curve_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json)'s helpers `age_ref_A()`, `age_ref_sd()`, `age_ref_percentiles()`.
...
**Files invoked.** [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) — method `_invert_per_class(a_class, class_name)`.
...
**Files invoked.** [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) — method `_analyze_saturation(per_class_status)`.
...
**Files invoked.** [`reference_age_curve_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json) — helper `age_ref_percentiles(class, age)` returns the 5-percentile vector for the patient's age.
...
**Outputs.** <cellular-age output — emitted by [`iam_cellular_age_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Cellular_Age/iam_cellular_age_scoring.py) (real module per inventory)> carrying per patient:
...
**Implemented in:** [`cpg_tiers.py`](../chain/cpg_tiers.py); [`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json); called by [`cpg_conductor.stage_b_identity`](../chain/cpg_conductor.py) - runs in the live path
...
**Files invoked.** `Biological_Physics/MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json` (v1.2). Engine consumes the JSON via a small helper. v0 4-tier statistical-percentile predecessor archived in `MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json`.
...
| NORMAL | 0.95 ≤ A[c] < 1.01 | Normal | Within healthy sampling variance *(was 1.04; changed 66f37fe 2026-07-03 to match the gauge MARGINAL onset)* |
| ELEVATED | 1.01 ≤ A[c] < 1.07 | Elevated | Recoverable drift; intervention window — closes at the 1.07 Warburg line |
...
**Smoking-bin interim mitigation (v1.2; retires when `IAMAtlas_smoking_layer.csv (not part of the chain - no foreground layer is subtracted; smoking is measured as a null on this gauge)` fit at v1.3):** ELEVATED floor shifted by smoking_bin: current=1.10 / former_0_5y=1.08 / former_5_15y=1.07 / former_15plus_y=1.05 / never=1.04.
...
**Inputs.** Per-cell-type A from Stage 4 (§46) + cell-type-specific breakpoints (inherit from parent class breakpoints, with optional cell-type-specific overrides in [`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json)).
...
**Inputs.** Per-class fractions from Stage 2 (§34) + `cfdna_weight.json (not part of the chain - a placeholder that was never derived)` + patient `substrate` from manifest.
...
**Files invoked.** `cfdna_weight.json (not part of the chain - a placeholder that was never derived)`. Per-class scoring module consumes the JSON.
...
**Files invoked.** [`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json) — its `customer_label_map` section.
...
**Outputs.** <tier vector — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`> carrying per patient:
...
- Stage 7 output: <tier vector — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>.
...
**Files invoked.**
- Module: `<card-matching logic inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Lookup table: `disease_cell_signature_matrix_v1_5.csv (not part of the chain; use disease_cell_signature_matrix_v1_13.csv - disease matching is not part of the chain; the matrix is record-side)` (77 rows × 131 columns, 354 populated cells, SHA-256 hashed at load).
- Card registry: `<card registry — currently embedded in `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>` — maps each card to the disease_id rows it pulls.
...
**Canonical cross-references.** Recipe §8 (disease-state matching). Roadmap §10.2.6 Phase F (re-audit through complete chain). Disease matrix README at `disease_signature_matrix_README.md (not part of the chain - disease matching is not part of the chain)`.
...
**Files invoked.**
- Module: `<inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Per-card residual maps live at `<per-card residual map — placeholder dir at `pipeline_runtime_matrices/card_residual_maps/`, populated as VALs lock>`. Currently in production: `breast_epic_residual_map_v0_1.csv (not part of the chain; use breast_epic_residual_map_chr_annotated.csv - the maps are record-side)`. Other cards pending residual map derivation per Phase F backlog.
...
**Canonical cross-references.** Recipe §8.2. VAL-003 outcome (residual map source). breast-epic card README at [`breast-epic_README.md`](../chain/Disease%20Cards%20:%20Residual%MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : MethylPhys/chain/Disease Cards : Record/disease_cards_residual_maps/Breast_EPIC/breast_epic_card_json/breast-epic_README.md).
...
- Card JSON: `<per-card definition — currently embedded in `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>` — declares the matching rule.
...
**Canonical cross-references.** Recipe §8.3. Each card's JSON contains its own matching rule (`breast-epic_card_v2_3.json (not part of the chain; use breast-epic_card_v3_1.json - the cards are record-side)` §`matching_rule`).
...
**Outputs.** <card verdict package — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`> carrying per patient:
...
- Lookup: [`tier_breakpoints.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Tier_breakpoints/tier_breakpoints.json) (engine-to-customer mapping block, same file used at §63).
...
This is **the only place in the chain** where information from external (non-IAMAtlas, non-IAM-derived) sources enters the customer-facing output. The discipline: literature anchors come from peer-reviewed publications cited in [`literature_anchors.json`](../chain/Runtime%20Matrices/Literature_anchors_Report%MethylPhys/chain/Runtime Matrices/Literature_anchors_Report building/literature_anchors.json), with explicit DOI and effect-size mapping. Never paraphrased from secondary sources. Never invented.
...
**Files invoked.**
- Module: `<anchor selection inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Module: `<prior lookup inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Lookup: `cancer_prior.json (not part of the chain - the chain applies no disease prior)`. Schema per prior: `{disease_id, sex, age_decade, lifetime_incidence, citation}`.
...
**Files invoked.**
- Module: `<multiplier lookup inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Lookup: `family_history_multiplier.json (not part of the chain - the chain applies no risk multiplier)`. Schema per multiplier: `{disease_id, relative_relationship, multiplier, citation}`.
...
```
posterior_context_class = baseline_prior          (from cancer_prior.json (not part of the chain - the chain applies no disease prior))
                        × age_factor              (US incidence by decade)
                        × sex_factor              (e.g., secretory: female 1.4×, male 1.2×)
                        × fh_factor               (from family_history_multiplier.json (not part of the chain - the chain applies no risk multiplier) IF present; else 1.0)
...
**Files invoked.**
- Module: `<sex conditioning inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
...
**Files invoked.**
- Module: `<report rendering inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- Per-card templates: <inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` — no standalone module file at present writing> (Markdown with slot syntax).
- Global template: `<global report skeleton — inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>` (the report skeleton — patient header, summary, per-card sections, disclaimers).
...
**Outputs.** A rendered Markdown report. Per-patient file at `<draft report — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`. Status: DRAFT until §76 clears it for shipping.
...
**Files invoked.**
- Module: `<legal-boundary gate inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`
- The CANNOT_SAY list: `<legal-boundary CANNOT_SAY list — TBD; currently enforced inline within `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`.
...
**Outputs.** Cleared report: `<cleared report — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`. Or HALT_FOR_REVIEW flag with detailed violation report.
...
**Files invoked.**
- Module: `<delivery packaging inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` — or `web.commercial.py (no such file in this repository)` orchestrator when built>`
...
**Outputs.** `<final report — emitted internally by `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>` with embedded checksums and engine version metadata.
...
**Files invoked.**
- Module: `<channel routing inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` — or `web.commercial.py (no such file in this repository)` orchestrator when built>`
...
**Files invoked.**
- Module: `<audit-trail capture — TBD when `web.commercial.py (no such file in this repository)` orchestrator is designed>`
...
> in [`chain_inventory_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json); any path NOT in that inventory should be treated as
...
- `prereg.json (no such file in this repository)` — pre-registration document declaring the statistical test, the cohort, the significance threshold, the multiple-testing correction approach.
...
**Files invoked.**
- `MethylPhys/chain/CPG_Null_Runner/cpg_null_runner.py` — the orchestration module.
...
- `MethylPhys/chain/Synthetic_Patient_Generator/synthetic_patient_generator.py` — N6 and N7 invoke this.
...
**Failure modes.**
- **Insufficient permutation budget.** Default 1,000; for borderline VALs, framework can run 10,000 or 100,000. The framework declares the budget per VAL in `prereg.json (no such file in this repository)`.
...
**Files invoked.** `MethylPhys/chain/Synthetic_Patient_Generator/synthetic_patient_generator.py`.
...
The PREREG is SHA-256 hashed and committed to the repo at `Biological_Physics/validation_runs/CPG-VAL-XXX/prereg.json (historical path) (no such file in this repository)` before any case/HC labels are looked at.

**Stage S2 — DATA RUN.** The VAL runs Stages 0-10 of the chain on the declared cohort. Outputs `per_sample.csv` (per-patient reconstructions) and `headline_results.json (not called by run_full - a results summary, not a chain input)` (the declared statistic computed on the actual cohort).
...
**Inputs.** PREREG + per_sample.csv + headline_results.json (not called by run_full - a results summary, not a chain input) + null_results.json.
...
**Outputs.** The OUTCOME document at `Biological_Physics/validation_runs/CPG-VAL-XXX/outcome.md (historical path)`. The sealed VAL artifact bundle.
...
**Outputs.** A per-VAL `null_invocation_log.json (no such file in this repository)` recording per-null start/end times, framework version, dispatch decisions.
...
**Atlas reference.** None.
**Files invoked.** `<cross-stage flag composition logic — currently inline in `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`>`.
...
**Canonical cross-references.** §32 (NILC), §33 (gate), Phase_B2_1_FINDING.md (not called by run_full - a pre-build finding, kept in the record).
...
**Files invoked.** `<chain re-run orchestration — TBD when `web.commercial.py (no such file in this repository)` is built>` orchestrates.
...
Authoritative inventory: `MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json`. This SOP section mirrors that inventory and adds the SOP-section cross-reference column.

§97 is organized in three blocks: **(A) Real artifacts that exist as files in the repo today; (B) Logic currently embedded inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` — the production engine — and not yet isolated as standalone modules; (C) Pending — does not exist yet, awaits the `web.commercial.py (no such file in this repository)` orchestrator design discussion.**
...
### §97.A — Real files / modules per [`chain_inventory_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json)
...
| Phase B2 finding | `Biological_Physics/chain_of_custody/L4_component_separation/Phase_B2_FINDING.md (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record) (historical path) (not called by run_full - a pre-build finding, kept in the record)` | (historical) | Initial NILC+Walther cross-check |
...
| Age layer diagnostics | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/components/age_layer_diagnostics.json (not part of the chain - diagnostics of the retired foreground layer)` | §35 | Per-CpG fit diagnostics |
| **A-score scoring** | `MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py` | §41–§45 | `score_per_class()` + `score_per_celltype()` |
| Marker artifact | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/iamatlas_celltype_markers_v0_2.json` | §29, §44, §65 | Per-cell-type one-vs-rest top-100, 115 cell types |
| Marker SHA anchor | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/iamatlas_celltype_markers_v0_2.sha256` | §29 | SHA: `46ea5be1db377f2b8773a02418a7f481a191630e0fa833d3294eab1fd19c47bd` |
| **Mahalanobis scoring** | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/iamatlas_mahalanobis_scoring.py` | §47–§51 | `MahalanobisHealthyHull` class — distance + top-10 axis decomp |
| Mahalanobis reference | `Biological_Physics/MethylPhys/chain/Runtime Matrices/Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v2_0_age_matched_derived.json` (current production; v0_1/v0_2/v0_3/v0_4 retained for lineage) | §48 | HC centroid + covariance (n_hc=1,721, Ledoit-Wolf shrinkage=0.001317, percentile-calibrated Route A threshold p95=13.54) |
| Mahalanobis validation | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/mahalanobis_per_patient_breast_predx_validation.csv (not called by run_full - a validation output, not an input)` | (audit) | Per-patient breast pre-dx distances (n=648) |
| **Cellular age scoring** | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/iam_cellular_age_scoring.py` | §52–§58 | `IAMCellularAge` class — canonical Recipe §6.3 inversion |
| Age reference matrix (JSON) | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/age_reference_matrix.json` | §53 | 80-cell baseline: 8 classes × 10 decades |
| Age reference matrix (CSV) | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/age_reference_matrix.csv` | §53 | Same data, flat CSV |
| Age reference matrix (PY) | `MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json` | §53 | Same data, with interpolation helpers |
| Cellular age validation | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/cellular_ages_v4_epic_italy_validation.csv (not called by run_full - a validation output of the retired cellular-age layer)` | (audit) | 1,174 EPIC-Italy v4 output |
| Tier breakpoints | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/tier_breakpoints.json` | §59, §60, §63 | A-score thresholds + customer-label collapse |
| cfDNA weights | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/cfdna_weight.json (not part of the chain - a placeholder that was never derived)` | §61 | Healthy-blood per-class expected (Snyder/Moss) |
| Literature anchors | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/literature_anchors.json` | §71 | Per-class published anchors |
| Cancer prior | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/cancer_prior.json (not part of the chain - the chain applies no disease prior)` | §72 | US lifetime incidence per class |
| Family history mult. | `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/pipeline_runtime_matrices/family_history_multiplier.json (not part of the chain - the chain applies no risk multiplier)` | §73 | First-degree-relative RR per class |
| Disease signature matrix v1.5 | embedded inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` at present writing | §65 | 77×131 card-level lookup; standalone binary export pending at `pipeline_runtime_matrices/disease_signature_matrix/` |
...
| **L9 null runner** | `MethylPhys/chain/CPG_Null_Runner/cpg_null_runner.py` | §80–§88, §91 | Unified 8-null framework (N1–N8) |
| **Synthetic patients** | `MethylPhys/chain/Synthetic_Patient_Generator/synthetic_patient_generator.py` | §86, §87, §89 | FFP10/NPIPE analog — signal-injection harness |
...
| **Production engine** | `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` | §11–§79 (where logic is embedded) | The actual runtime — see §97.B |
| System inventory | `MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json` | (this table) | Source of truth for paths |
...
| SOP § | Operation | Notes |
|---|---|---|
| §11–§19 | Stage 0 — IDAT intake, control probes, detection-p, bead count, call rate, sex check, decision gate | Stage 0 QC runs inside Walther per [`chain_inventory_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/chain_inventory_v1.json) §1; the wider intake/manifest layer is engine-internal. |
...
| §28, §29 | Atlas loading + marker-pool extraction | Engine loads `IAMAtlasREBUILD.csv` and [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json) at startup. |
...
| §46 | Stage 4 output consolidation | Same. The scoring module ([`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py)) is real and standalone; the consolidation is engine-internal. |
...
| §77–§79 | Stage 10 delivery — packaging, routing, audit capture | All inside the engine today; expected to move into the future `web.commercial.py (no such file in this repository)` orchestrator. |
| §93, §96 | Cross-stage flag composition; chain re-run orchestration | Inline within `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)` today; cleanly modularized when orchestrator is built. |

### §97.C — Pending: does not exist yet

| Item | What it would be | Blocked by |
|---|---|---|
| `web.commercial.py (no such file in this repository)` orchestrator | Top-level driver that calls the modules in §97.A in sequence, replacing the engine-internal orchestration in §97.B. Working name — final naming TBD. | Open design discussion between Heath and Walther |
| Standalone disease-signature-matrix v1.5 CSV | Binary export of the matrix currently embedded inside `GAPE_WEB_v13.py (not part of the chain - a report generator from the preliminary era)`. Placeholder dir already created at `pipeline_runtime_matrices/disease_signature_matrix/`. | Pending decision on whether to externalize before orchestrator design |
...
| Sex / batch / ancestry / smoking foreground modules | Companions to `age_axis_foreground.py (not part of the chain - superseded by the measured age curve)`; would live in `Biological_Physics/RETIRED_2026-09/PostBuild_atlas_vault_snapshot_2026-06/components/`. | Phase B4 per Roadmap §10.2.2 |
...
## §99. The eight class floors H_min — frozen 2026-04-06 *(v2.0.0: formerly titled "Mahaffey Numbers"; H_min is not the Mahaffey number — see LEDGER last row)*
...
| §32-§33 | NILC + cross-method gate | §6.2 | §10.2.2 Phase B2 | Phase_B2_1_FINDING.md (not called by run_full - a pre-build finding, kept in the record) |
| §35 | Age foreground | §5 | §10.2.2 Phase B3 | Phase_B3_FINDING.md (not called by run_full - a pre-build finding, kept in the record) |
...
| §65-§69 | Card matching | §8 | §10.2.6 Phase F | disease_signature_matrix_README.md (not part of the chain - disease matching is not part of the chain) |
...
> **⚠ v1.4.0 RECONCILIATION (2026-06-30) — read with §105.** §103.1 below correctly observed the all-breach symptom and even wrote its exact mechanism ("panel MEAN collapses to ~0.5; H(0.5)=1.0 → A=1/H_min"), but attributed the cause to the choice of markers (discriminative vs most-methylated). That mechanism is the signature of the **H(β_mean) aggregation order**, not of the marker set. Under the corrected formula A = mean_i(H(β_i)/H_min) (§41–§43, §105), the all-breach cannot occur with ANY marker set, because each locus is scored before averaging and a bimodal locked panel reads near its floor. The "A-SCORE LOCI GUARD" prescribed here is therefore a workaround for a formula bug, and it contradicts the sealed VALs, which used the v0_2 discriminative panel with mean(H) and produced the +2.088 anchor (reproduced exactly in §105). **The marker convention (v0_2 / healthy≈0.52 vs most-methylated loci / healthy≈1.0) remains an open baseline-placement decision for Heath — but it is independent of the formula, which is settled: mean of per-CpG entropies.** The §103.2 / §103.3 atlas-flatness and source-doc points stand unchanged.
...
## §105. LESSON-ASCORE-02 — The A-score is the mean of per-CpG entropies, not the entropy of the mean β (2026-06-30)

**The one sentence.** The A-score is **A = mean_i( H(β_i) / H_min(class) )** — score each marker locus against its class floor, then average. On marker panels it is **never** H(β_mean)/H_min, the entropy of the panel-averaged β. *(v1.5.0: this rule is scoped to discriminative-marker panels — the separation surface. The class gauge on identity loci is a different instrument with a different reference; see §106.)* Score per locus, then average; never average β first.

**What happened.** A formula change quietly replaced mean_i(H(β_i)/H_min) with H(β_mean)/H_min in the runtime A-scoring module on or about 2026-06-11. Because Shannon entropy is concave, Jensen's inequality guarantees H(mean β) ≥ mean(H(β)), so the swapped formula inflates every score. On a real marker panel — which is bimodal by construction, since a marker earns its place by being locked high in some lineages and low in others — averaging β first manufactures a fake value near 0.5, which entropy reads as maximal disorder. Healthy cells were driven into false BREACH; roughly 30% of marker cells pinned above the 1.10 architectural-fidelity line. Reports were unreadable for weeks. The change was not caught because the SOP itself (this document, pre-v1.4.0) and the Recipe §6.3 citation it carried both declared "H of the mean" canonical — the source of truth agreed with the mistake.

**Worked numeric guard (a patient's real HSC panel, GSM1235534).** Of the cell type's marker loci with measured β: 11 sit below 0.2 (locked low), 24 sit above 0.8 (locked high) — a textbook bimodal locked panel.
- β_mean across the panel = 0.600.
- **Broken:** H(β_mean)/H_min = H(0.600)/0.874 = **1.111 → BREACH (false alarm).**
- **Correct:** mean_i( H(β_i) )/H_min = **0.864 → at floor, healthy.**
The fake 0.5-ish mean is an artifact of averaging in the wrong order. The per-locus formula preserves what each locus actually wrote.
...
**Reconciliation with §103.** §103 (2026-06-11) saw the all-breach and wrote its mechanism correctly — "panel MEAN collapses to ~0.5; H(0.5)=1.0 → A=1/H_min" — but attributed it to scoring discriminative markers and prescribed switching to most-methylated loci plus an A-SCORE LOCI GUARD. That mechanism is the signature of the H(β_mean) order, not of the marker set. Under mean-of-H the all-breach cannot occur with any marker set. §103's guard is a workaround for the formula bug and contradicts the sealed VALs, which used the v0_2 discriminative panel.

**Open decisions (Heath's, NOT resolved here).**
1. **Recipe §6.3** is the upstream source of the "H of the mean is canonical" ruling. It must be corrected at the vault source, or a future implementer re-introduces this exact regression. (The Recipe is vault IP and is not edited from this document.)
...
**Guard (binding).** [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) carries a unit test that scores a known bimodal panel and asserts the mean-of-per-CpG value; any build computing H(β_mean) fails it. The reproduction standard is permanent: **any A-score formula that does not reproduce the +2.088 GSE51032 anchor is wrong by definition.** Before changing anything in Stage 4, re-run the GSE51032 reproduction and confirm corr 1.0 / maxdiff 0.0 against the sealed artifacts.


## §106. RULING A3 — One aggregation per surface, fixed by the reference each was built with (2026-09-19)

**Why this section exists.** At repo HEAD (66f37fe) two modules committed one day apart each called the other the regression: [`iamatlas_a_scoring.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_a_scoring.py) + [`test_a_score_canonical.py`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/test_a_score_canonical.py) (2026-06-30) assert mean_i(H(β_i))/H_min and declare any other build wrong by definition; [`cpg_gauge_engine.py`](../chain/cpg_gauge_engine.py) + the Stage-4 wiring (2026-07-01) compute H(β_mean)/H_min over the class **identity loci** and declare that it supersedes §105. Both were run on the same eleven Stage-1-calibrated samples (PROC-FORMULA-01, [[Issue 003](../manual/MethylPhys_CPG_Operations_Manual.pdf)](../manual/MethylPhys_CPG_Operations_Manual.pdf) §3.3) and against the sealed references (PROC-ANCHOR-01). The result is that **both are correct, each on its own surface**, and the conflict was a scoping error in §105, not a formula error in either module.

**The ruling.**

| surface | loci | statistic | reference it is read against | why |
|---|---|---|---|---|
| **GAUGE** (8 classes; the detection call) | identity loci ([`iamatlas_gauge_identity_loci_v1_0.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/iamatlas_gauge_identity_loci_v1_0.json), ~20–32k per class, selected where the class sits at its characteristic β, unimodal) | **A = H(β_mean)/H_min(class)** | H_min (G-002/G-003b: H of a mean β) and [`age_reference_matrix.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/age_reference_matrix.json) (compiled as H(β_mean), verified to 5 decimals) | the reference is itself H of a mean; Jensen inflation on a unimodal panel is bounded (measured 0.033 on whole blood) |
| **SEPARATION** (115 cell types; disease direction) | discriminative markers ([`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json), ~100 per cell, bimodal by construction) | **A = mean_i(H(β_i))/H_min(class)** | the sealed anchors `GSE51032/GSE51057_115celltype_ascores.csv (historical path)` | §105 exactly: bimodal panels make H(β_mean) manufacture ≈0.5; reproduced r = 1.00000 on 648 samples |
...
**Guard, in code not prose.** The gauge refuses to return a number when (a) the class is absent from the substrate (deconvolver fraction < DETECT_FLOOR), or (b) the panel it actually receives is one where averaging β first is invalid — tested directly as the Jensen gap H(β_mean) − mean_i H(β_i) > 0.05. Calibrated 2026-09-19: identity loci in whole blood 0.033 (pass); identity loci in bulk tissue 0.20 (refused — a mixture); HSC markers 0.31 and hepatocyte markers 0.20 (refused). Reference implementation: `MethylPhys/kit/cpg_kit.py::gauge_A`. `test_a_score_canonical.py` guards the separation surface and its docstring now says so.

**§105 open decisions.** (2) marker convention — closed: identity loci for the gauge, discriminative markers for separation; the "most-methylated loci" variant is retired. (3) §46.5 — closed: the gauge keeps H(β_mean), so §46.5's premise holds as written. (1) Recipe §6.3 — **remains Heath's**, vault-side.

**Consequence for the whole-blood age band (Issue 003 §6a).** In all 7 Stage-1-calibrated whole-blood test samples the immune identity loci read β_mean 0.786–0.822 against the atlas reference 0.7318 — outside the ±0.05 selection band in 7 of 7. This is a reference-β offset between the atlas source pipelines and noob Stage-1 output, not a Stage-1 error (bit-identical, PROC-CAL-01) and not the donors. Closing test: re-derive H_min_β and the age band from ≥30 public healthy whole-blood IDATs run through Stage 1.
...
6. **Mahalanobis Option A + cellular age** (edc91e6). The hull now scores the eight class-gauge A's against the band (μ = A_mean(class, age), σ = (A_mean − A_p10)/1.2816), distance over assessable classes, n-adaptive χ² alarm; the v1_0 reference is retired and refused. Cellular age inverts the band's A_mean curve per class; accuracy gated on Stage-1 normalization.
7. **NILC cut** (c1be0c3). Stage 2 is Walther alone. §9 retired in place.
...
## §108. Reporting rule (2026-09-19) — extends the §3.2 / DETECT_FLOOR presence gate

The presence gate (DETECT_FLOOR = 1%) decides whether a class is *in the sample*. This rule decides whether its gauge goes *on the report*. A class gauge is **reported** on a substrate only if the class is **determined** (its fraction separable from its neighbours in the Atlas on that substrate - condition number of the substrate sub-problem < ~10 - or reported jointly with what it cannot be split from) and **present** (typical fraction >= 5% on that substrate). Classes failing either are composition-only. Whole blood: immune reported; progenitor + stem_adult reported jointly as one haematopoietic-progenitor component; all others composition-only. Basis: PROC-CHAIN-01 (stem_adult false BREACH 6/7 healthy), PROC-NILC-01 (r = +0.99 progenitor-stem_adult, kappa 30.6), and the record that stem_adult has carried no finding in nine validations. Decided from kappa and fraction before any A is computed.

**Scope (added same day, after the author's challenge on MDS / CML / CHIP / AD):** the merge is gauge-level only. The separation surface keeps HSC, MPP, CMP, GMP and all other cells separate, so disease direction is read as the matrix writes it. MDS (HSC up, GMP down) is the case that requires this. stem_adult earns a separate blood gauge through the myeloid-arm validation (MDS / CML / CHIP), not by rule.


---

## §109. BETA SCALE — the gauge input must be on the floor's scale (added 2026-09-20)
...
**Operating rule.** Stage 4 / Stage B computes A = H(β̄)/H_min on identity loci. Before that step, β is mapped onto the Roadmap scale with the entry in [`beta_scale_maps_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/beta_scale_maps_v1.json) matching the pipeline that produced it (Stage-1 noob → `stage1_noob_450K`; a GEO series matrix → the author-processed entry once fit). If no map exists for the pipeline, the reading is labelled `scale=UNMAPPED` and is not reported as a tier. This section supersedes any earlier text implying A can be read directly from any β.
...
**LAB ZERO — PANEL SPECIFICATION (PROC-PANEL-01 → PROC-PANEL-03, 2026-09-20; supersedes the '20–30 arrays' wording above).** A laboratory's zero is measured once on **40** healthy arrays of any age mix through the same Stage 1 and map: z = median[A − c(decade)] − 1, where c is the reference healthy age curve ([`reference_age_curve_v1.json`](../chain/Runtime%MethylPhys/chain/Runtime Matrices/A_Scoring_Module/reference_age_curve_v1.json); healthy immune A rises ≈0.045 from the teens to the eighties within a lab, while between-lab offsets are parallel). A patient reads A″ = A − c(decade) − z. Tested leave-one-lab-out on four labs: a band built on three holds 75–84 % of the fourth's healthy donors. In code: `MethylPhys/chain/lab_zero.py` — panels under 40 are refused and `lab_zero=UNSET` is not reportable. Record: `Record/PROC_data/PROC-PANEL-01 → PROC-PANEL-03/`.
...
_Generated from [`chain_inventory_v1.json`](../chain/Runtime%20Matrices/chain_inventory_v1.json) (built 2026-09-26 12:53, commit `3f1d5c5`) by [`sop_repoint.py`](sop_repoint.py). Do not hand-edit: re-run the script after any file move._

| file | role | stage | what it is |
|---|---|---|---|
| [`stage_0_1_qc_handoff.py`](../chain/stage_0_1_qc_handoff.py) | chain |  | decodes the array's own controls, bead counts and sex intensities so the intake gates can be measured rather than deferred |
| [`stage_2c_trace_detection.py`](../chain/stage_2c_trace_detection.py) | chain |  | Stage 2c: is there evidence of a trace class at all, by a score test that the non-negativity boundary cannot pin |
...
| [`CHAIN_SEQUENCE.md`](../doors/CHAIN_SEQUENCE.md) | reference |  | the step order as the code calls it, derived by AST - never typed |
| [`ENHANCEMENTS.md`](../doors/ENHANCEMENTS.md) | reference |  | what would make the chain more sensitive, ranked by whether it changes a reported number |
| [`README_CPG_Plates.md`](../plates/README_CPG_Plates.md) | reference |  | the reference plates and their conventions - four of them are embedded in every report |
| [`REPORT_TAB_REFERENCE.md`](../doors/REPORT_TAB_REFERENCE.md) | reference |  | the report described tab by tab with a figure of each, generated from a finished report |
| [`REPO_INVENTORY.md`](../doors/REPO_INVENTORY.md) | reference |  | what is in the repository, measured, and what should not be |
| [`REPRODUCTION.md`](../hmin_calibration/reproduction/REPRODUCTION.md) | reference |  | how to reproduce every class floor in about fifteen seconds |
| [`REVIEWER_MANIFEST.md`](../doors/REVIEWER_MANIFEST.md) | reference |  | the reviewer download list: every file a referee could want, resolved from the tree |
| [`appendix_vi_vii.json`](../manual/appendix_vi_vii.json) | interface |  | appendix content for the manual |
...
| [`g003_mcmc_framework.py`](../hmin_calibration/g003_mcmc_framework.py) | reference |  | the framework the floor calibration runs on |
| [`gape002_lib.py`](../manual/gape002_lib.py) | interface |  | the shared renderer both editions of the manual are built with |
...
| [`om_part3.py`](../manual/om_part3.py) | interface |  | the manual's Part III sections |
| [`percell_reference_v0_3.json`](../chain/Runtime%20Matrices/Percell_Reference/percell_reference_v0_3.json) | reference |  | the per-entry healthy references the per-cell columns are zeroed on |
| [`reference_cells_37.csv`](../hmin_calibration/reference_cells_37.csv) | reference |  | the 37 published reference methylomes the class floors were fitted on |
| [`report_tabs.json`](../manual/report_tabs.json) | reference |  | the tab descriptions as data, read by both the SOP generator and the manual build |
| [`toc_pages.json`](../manual/toc_pages.json) | interface |  | page numbers measured in the first build pass, read by the second |
| [`trace_detection_panel_v1.json`](../chain/Runtime%20Matrices/trace_detection_panel_v1.json) | reference |  | the frozen panel and thresholds for Stage 2c trace detection |
| [`detection_panel_v1.json`](../chain/Runtime%20Matrices/A_Scoring_Module/detection_panel_v1.json) | reference | 2d | Stage 2d commissioned detection panel: per laboratory, the centre, per-locus inverse-variance weights and p99 line for every foreign cell, from that laboratory's own healthy panel; the 1,506 |
...
| [`build_percell_identity.py`](../kit/build_percell_identity.py) | interface | builder | Builds iamatlas_percell_identity_loci_v1_0.json from the atlas by the class panels' criterion (kit). |
| [`build_percell_reference_identity.py`](../kit/build_percell_reference_identity.py) | interface | builder | Builds percell_reference_identity_v1_0.json: per-cell per-lab A bands on the identity surface, commissioned form, mapped betas, disjoint held-out split, laboratory offsets (kit). |
...
| [`stage1_betas_GSE111629.pkl.xz`](../reference_data/stage1_betas_GSE111629.pkl.xz) | reference | reference_data | Stage-1 calibrated betas for the 80-array healthy panel of GSE111629 (the laboratory's commissioning panel; lab_zero and the per-cell calibration record are built from it). |
| [`stage1_betas_GSE111629_MANIFEST.json`](../reference_data/stage1_betas_GSE111629_MANIFEST.json) | reference | reference_data | sha256 and provenance of stage1_betas_GSE111629.pkl.xz. |
| [`stage1_betas_GSE125105.pkl.xz`](../reference_data/stage1_betas_GSE125105.pkl.xz) | reference | reference_data | Stage-1 calibrated betas for the 80-array healthy panel of GSE125105 (the laboratory's commissioning panel; lab_zero and the per-cell calibration record are built from it). |
| [`stage1_betas_GSE125105_MANIFEST.json`](../reference_data/stage1_betas_GSE125105_MANIFEST.json) | reference | reference_data | sha256 and provenance of stage1_betas_GSE125105.pkl.xz. |
| [`stage1_betas_GSE42861.pkl.xz`](../reference_data/stage1_betas_GSE42861.pkl.xz) | reference | reference_data | Stage-1 calibrated betas for the 80-array healthy panel of GSE42861 (the laboratory's commissioning panel; lab_zero and the per-cell calibration record are built from it). |
| [`stage1_betas_GSE42861_MANIFEST.json`](../reference_data/stage1_betas_GSE42861_MANIFEST.json) | reference | reference_data | sha256 and provenance of stage1_betas_GSE42861.pkl.xz. |
| [`stage1_betas_GSE87571.pkl.xz`](../reference_data/stage1_betas_GSE87571.pkl.xz) | reference | reference_data | Stage-1 calibrated betas for the 80-array healthy panel of GSE87571 (the laboratory's commissioning panel; lab_zero and the per-cell calibration record are built from it). |
| [`stage1_betas_GSE87571_MANIFEST.json`](../reference_data/stage1_betas_GSE87571_MANIFEST.json) | reference | reference_data | sha256 and provenance of stage1_betas_GSE87571.pkl.xz. |
...
| [`iamatlas_percell_identity_loci_v1_0.json`](../chain/Runtime%20Matrices/A_Scoring_Module/iamatlas_percell_identity_loci_v1_0.json) | reference | stage A | PER-CELL IDENTITY LOCI (2026-09-26): the class panels' own criterion (|mean - H_min_beta| <= 0.05, one entropy branch per cell) applied to 102 of 115 cells, MIN_LOCI 100. THE SURFACE the per |
...
| [`percell_reference_identity_v1_0.json`](../chain/Runtime%20Matrices/Percell_Reference/percell_reference_identity_v1_0.json) | reference | stage A | THE PER-CELL CALIBRATION RECORD (2026-09-26). For each of 102 cells: p10/p50/p90 of A on the cell's identity loci, per laboratory (four labs, ~970 arrays, disjoint held-out split, coverage 0 |
...
| [`composition_guard_v1.json`](../chain/Runtime%20Matrices/A_Scoring_Module/composition_guard_v1.json) | reference | stage B | THE COMPOSITION GUARD threshold (PROC-FOREIGN-01, commissioned 2026-09-25): the foreign (non-haematopoietic) fraction above which the immune tier is withheld, 0.0207, set on 318 healthy arra |
...
_Generated from `MethylPhys/manual/report_tabs.json` by `MethylPhys/kit/build_report_tab_reference.py`, read off a real report (`../../../results/e2e_final/REPORT.html`, commit `01e34d2`). Re-run it after any change to the report builder._

One run writes **one self-contained HTML file of 7.42 MB with 19 tabs** - 9 carry this specimen's own measurements and 10 carry reference material identical in every report. A reference tab tells you how the instrument works; only a specimen tab tells you anything about the patient.

| tab | kind | what it carries | size |
|---|---|---|---|
| **Reading** (`reading`) | SPECIMEN | The reading itself: the class gauge value, where it sits against the healthy band for this age, its tier word, and the composition that produced it. If a clinician reads one tab, it is this one. | 27 KB, 4 tables |
| **How to read** (`howto`) | REFERENCE | How to read the report: what each number means, what it does not mean, and the vocabulary the chain is allowed to use. | 633 KB, 4 tables |
| **Cells** (`cells`) | SPECIMEN | Every atlas cell type scored for this specimen - all of them, placed or not - each with its 95 per cent interval, the healthy range on its own markers, how many of its markers were found, and its position against healthy. | 91 KB, 11 tables |
| **Departure** (`departure`) | SPECIMEN | How far this specimen sits from the healthy centre in the banded space, which axes drove it, the patient value against the age-matched mean and the sigma used, and this laboratory's own measured false-alarm rate beside them. | 4 KB, 1 tables |
| **Sky** (`sky`) | SPECIMEN | The residual sky: a Mollweide plate of this specimen's own residuals, per class, plus the statistics behind each plate. Opens with what a plate is compared to, because it is never compared to a healthy picture. | 5998 KB, 3 tables |
| **Physics** (`physics`) | REFERENCE | The physics under the gauge: Landauer's bound, the Mahaffey number, the entropy floor and why the zero is fixed rather than a control group. | 18 KB, 4 tables |
| **Story** (`story`) | REFERENCE | What astro-genetics is, in the author's words - the introduction that now also opens Issue 003. | 379 KB, 0 tables |
| **Reference** (`reference`) | REFERENCE | Every constant the reading was corrected by, with where each came from - the floor, the age term, the laboratory zero, the scale map, and the calibration DOI. | 9 KB, 2 tables |
| **Coverage** (`coverage`) | SPECIMEN | What was measurable on this specimen's platform and what was not: marker coverage per class, and which atlas entries the array could not reach. | 7 KB, 3 tables |
| **Red flags** (`flags`) | SPECIMEN | Red flags: everything this run refused, withheld or could not measure, in one place, ordered by severity - STOP, WITHHELD, CAUTION, NOTE. A failing CMB tool arrives here as CMB_TOOL_FAIL. | 3 KB, 1 tables |
| **Safeguards** (`safeguards`) | SPECIMEN | Every guard and whether it passed on this specimen, including the register of all 17 methods borrowed from CMB analysis with PASS, FAIL, NOT_RUN, NOT_APPLICABLE or NOT_BUILT for this run. | 17 KB, 4 tables |
| **Troubleshooting** (`trouble`) | REFERENCE | Troubleshooting: every refusal string the chain can print, what it means, and what the operator does about it. | 9 KB, 4 tables |
| **Integrity** (`integrity`) | SPECIMEN | The fail-safes that kept this reading honest: the Stage 0 custody record for this specimen, both file hashes, and each intake gate's own result. | 18 KB, 3 tables |
| **Chain** (`chain`) | REFERENCE | The chain that produced the reading, stage by stage, derived from the code rather than described - so a stage cannot be claimed that the code does not call. | 29 KB, 21 tables |
| **Files** (`files`) | REFERENCE | Every file the chain uses, enumerated from the live tree with its role. | 67 KB, 6 tables |
| **Findings** (`findings`) | REFERENCE | Findings that changed a reported number, with the procedure that sealed each. | 12 KB, 3 tables |
| **Roadmap** (`roadmap`) | REFERENCE | What is not built yet and what it would buy - the same list ENHANCEMENTS.md ranks. | 8 KB, 1 tables |
| **Record** (`record`) | REFERENCE | The validation record: every series and what it found. | 52 KB, 2 tables |
| **Run** (`run`) | SPECIMEN | How to run it yourself, what produced this reading (chain commit, decoder version, a hash of every input read), and whether every derived document was current when this report was built. | 16 KB, 6 tables |

Figures of every tab, with the sections each one contains, are in [`REPORT_TAB_REFERENCE.md`](../doors/REPORT_TAB_REFERENCE.md).

### The CMB tool register, and what a FAIL does

The Safeguards tab carries every method borrowed from CMB analysis with a check that runs on the finished bundle: **17 methods, 7 PASS on the commissioning specimen, 9 NOT_BUILT**. A FAIL is also emitted to the Red flags tab as `CMB_TOOL_FAIL`, so a borrowed method that stopped working cannot be missed in the middle of a long tab. NOT_BUILT entries are listed on purpose - the shelf is part of the record.
...
**To add a tool:** append one entry to `TOOLS` in `MethylPhys/chain/cmb_tools.py` with a `check(bundle) -> (status, evidence)`. The table, the counts and the red-flag routing all follow from it; nothing else needs editing.

### The three gates that keep this document true

| gate | what it refuses |
|---|---|
| `MethylPhys/chain/propagate.py` | regenerates every derived document, then checks the rules a human wrote (every live module named here and in the reviewer manifest, every sealed procedure in the commissioning table, every relative reference resolving). Exits non-zero on drift. |
| `MethylPhys/kit/link_check.py` | every relative path in the live documentation set must resolve - a path in a document is a claim like any other. |
| `chain/guarded_push.sh` | runs [`propagate.py`](../chain/propagate.py) **without a pipe** and refuses to commit or push if it fails. A pipe hands the shell the pipe's exit status, not the gate's, which is how a push once proceeded over a printed failure. |

Every report prints the first gate's verdict on its own Run tab, so a reading whose documents had drifted says so on the page.
...
- [`identity_band_v2_PROVISIONAL.json`](../chain/Runtime%20Matrices/A_Scoring_Module/identity_band_v2_PROVISIONAL.json) - superseded by identity_band_v3.json, the commissioned band; kept because sealed records cite it
