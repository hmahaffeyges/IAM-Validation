...
| `class_archives/` | Eight per-class `.tar.xz` archives (LFS). Each contains that class's `per_celltype.csv` (written by a run, not stored) (post-reconciliation), `brightness.csv` (v0.1 vault layer, not in this repository), and `result.json` (written by a run, not stored), plus the raw pre-reconciliation `per_celltype.csv` (written by a run, not stored) under `/raw/` for provenance. **Use these when re-running a single class** without rebuilding the others. |
...
| `merge_iamatlas_v0_1_REBUILD.py` | Combines the 8 per-class outputs into the unified `IAMAtlasREBUILD.csv` (not stored - decompress `IAMAtlasREBUILD.csv.xz` in `atlas/` to produce it; the chain reads the CSV). Reads the reconciled inputs from the previous step. |
...
4. Distinctness-test the new `per_celltype.csv` (written by a run, not stored) against the prior class results before merging. The test procedure is in `IAMAtlas_FLATNESS_LESSON.md`.
