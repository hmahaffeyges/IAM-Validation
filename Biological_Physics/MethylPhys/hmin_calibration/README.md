...
| [`gape_mcmc_nbio_ordering.py`](gape_mcmc_nbio_ordering.py) | the class **ordering** of the retired per-class n_bio (ρ = 0.905, p = 0.002). The ordering is all that was ever established; the absolute per-class values awaited a G-007 run that **was never made** (PROC-HISTORY-01), and the quantity itself is retired — superseded by the Mahaffey number M = 20.94, one number for the cell. See [[Issue 003](../manual/IAMPerformance_GAPEIssue003_RC1.pdf)](../manual/IAMPerformance_GAPEIssue003_RC1.pdf) §5.0.4. |
