...
| file | source | notes |
|---|---|---|
| `IAMAtlasREBUILD.csv` | repo `Biological_Physics/MethylPhys/atlas/IAMAtlasREBUILD.csv.xz` → `xz -d` | 605 MB, 483,092 rows, build 2026-05-28 |
...
[`stage_0_intake.py`](../chain/stage_0_intake.py) steps 0.1–0.9 on the eleven raw IDAT pairs (methylprep env). Array type is verified from the IDAT header, files are SHA-256 hashed with re-transmission detection, bead counts pass. Detection-p, call rate and sex check need Stage 1's decoded intensities and report DEFERRED until that hand-off is wired. **Defect fixed:** the decision gate ignored Step 0.1 quarantines (fail-open) — guard added.
