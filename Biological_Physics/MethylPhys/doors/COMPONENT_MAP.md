...
Readership is measured from the live modules' source (literal names and f-string templates). A file here is loaded by the code named beside it; propagate.py fails the push if any is missing from the SOP, the manifest or this table.

| runtime file | read by | what it is |
|---|---|---|
| `age01_results.json` | cpg_conductor.py | The PROC-AGE-01 measurement: the slope, the within-laboratory spread, and the resolution that follows. |
| [`age_reference_matrix.json`](../chain/Runtime%20Matrices/A_Scoring_Module/age_reference_matrix.json) | cpg_conductor.py, cpg_gauge_engine.py, iam_cellular_age_scoring.py | The April 80-cell age table. Typed beta means with A by formula - NOT a per-sample measurement; kept because parts of the record cite it. Th |
...
| [`detection_panel_v1.json`](../chain/Runtime%20Matrices/A_Scoring_Module/detection_panel_v1.json) | cmb_tools.py, cpg_conductor.py, stage_2c_trace_detection.py | Stage 2d commissioned detection panel: per laboratory, the centre, per-locus inverse-variance weights and p99 line for every foreign cell, f |
...
| [`iamatlas_percell_identity_loci_v1_0.json`](../chain/Runtime%20Matrices/A_Scoring_Module/iamatlas_percell_identity_loci_v1_0.json) | cpg_conductor.py, iamatlas_a_scoring.py | PER-CELL IDENTITY LOCI (2026-09-26): the class panels' own criterion (|mean - H_min_beta| <= 0.05, one entropy branch per cell) applied to 1 |
| [`identity_band_v3.json`](../chain/Runtime%20Matrices/A_Scoring_Module/identity_band_v3.json) | cpg_conductor.py, run_sample.py, val_finding.py | The commissioned healthy band, pooled and per decade, with each laboratory's measured zero. Placement (IN_BAND / above / below) is read from |
| [`reference_age_curve_v1.json`](../chain/Runtime%20Matrices/A_Scoring_Module/reference_age_curve_v1.json) | cpg_conductor.py, cpg_gauge_engine.py, lab_zero.py | The healthy age curve: about +0.47 milli-A per year, built leave-one-laboratory-out from four cohorts (n=1,379). The decade term is subtract |
| [`iamatlas_celltype_markers_v0_2.json`](../chain/Runtime%20Matrices/Celltype_Marker/iamatlas_celltype_markers_v0_2.json) | cpg_conductor.py, disease_matching.py, val_finding.py | The ~100 discriminative marker CpGs per cell type, used for the per-cell separation statistic. The sealed foundation-cohort anchors reproduc |
...
| [`literature_anchors.json`](../chain/Runtime%20Matrices/Literature_anchors_Report%20building/literature_anchors.json) | disease_matching.py | Published reference A-score anchors per class (healthy / disease / cancer), extracted from the April web build. Orientation values from the  |
| [`mahalanobis_healthy_reference_v2_0_age_matched_derived.json`](../chain/Runtime%20Matrices/Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v2_0_age_matched_derived.json) | cpg_conductor.py, disease_matching.py, iamatlas_mahalanobis_scoring.py | The healthy centre and spread per class and age the departure is measured against. |
...
| `residual_scale_GSE111629.npz` | cpg_conductor.py, propagate.py | UCLA's per-address residual scale for the sky. |
| `residual_scale_GSE125105.npz` | cpg_conductor.py, propagate.py | Munich's per-address residual scale for the sky. |
| `residual_scale_GSE42861.npz` | cpg_conductor.py, propagate.py | Karolinska's per-address residual scale for the sky. |
| `residual_scale_GSE87571.npz` | propagate.py | Uppsala's per-address healthy residual scale for the sky - the denominator that makes a patient's z a z. One per commissioned laboratory. |
| [`percell_reference_identity_v1_0.json`](../chain/Runtime%20Matrices/Percell_Reference/percell_reference_identity_v1_0.json) | cpg_conductor.py | THE PER-CELL CALIBRATION RECORD (2026-09-26). For each of 102 cells: p10/p50/p90 of A on the cell's identity loci, per laboratory (four labs |
| [`percell_reference_v0.json`](../chain/Runtime%20Matrices/Percell_Reference/percell_reference_v0.json) | val_finding.py | Per-entry healthy reference: for each of the 115 entries, the healthy range of its own per-cell reading, per laboratory. Exploration, unseal |
| [`percell_reference_v0_3.json`](../chain/Runtime%20Matrices/Percell_Reference/percell_reference_v0_3.json) | cpg_conductor.py, run_sample.py | the per-entry healthy references the per-cell columns are zeroed on |
...
| [`trace_detection_panel_v1.json`](../chain/Runtime%20Matrices/trace_detection_panel_v1.json) | stage_2c_trace_detection.py | the frozen panel and thresholds for Stage 2c trace detection |
