...
| 20 | Component-separation methods (Commander, NILC, SMICA, SEVEM, GNILC) | Walther IAM Deconvolver = one method. We need to build at least one INDEPENDENT method (parametric Bayesian vs blind/ILC-style) so we can compare, the way Planck compared Commander/NILC/SMICA/SEVEM. | ⟳ | Critical addition. No single-method CMB result was ever trusted. Need at least 2 deconvolvers. | REVERSED, THEN RESTORED — NILC was built as the second method and CUT 2026-07-02 (c1be0c3) for collapsing on correlated blood mixtures and deleting correct calls. Rerun as designed in September (PROC-NILC-01) it was vindicated: the divergence marked where the atlas does not separately determine the composition, which is what the method exists to report, so the cut had removed the instrument reporting the problem. REINSTATED 2026-09-22 as `stage_2b_second_opinion` — compared with Walther by class, agreement bar L1 ≤ 0.10, reported as a flag and never as the composition. The Planck principle this row cites is followed again, and RUNBOOK §11 forbids disabling either relay. |
