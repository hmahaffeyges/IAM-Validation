# PROC-MF-03 — outcome: NOT COMMISSIONED. B1–B6 met again; B7 failed on the fifth laboratory — its healthy null is 4–11× wider than any of the four, and its line was set by two controls that are not blood-like.
...
1. **Its healthy null is 4–11× wider than any of the four, at full marker resolution.** MAD of f̂ for Breast: the
   four laboratories 0.0010–0.0014, GSE51032 **0.0084**; colon 0.0002–0.0006 vs **0.0043**; neurons 0.0004–0.0014
   vs **0.0099**; prostate 0.0013–0.0030 vs **0.0132**. This is the bulk of the distribution, not a tail.
...
3. **The line was set by two arrays that are not blood-like.** GSM1236355 and GSM1236248 read **0.18–0.29 on Breast,
   Prostate and neurons simultaneously**, and GSM1235872 reads 0.16 on neurons. Every foreign cell rising together is
   the substrate-mismatch signature already documented in PROC-TISSUE-01 and DISC-BLADDER-003 — not a detection of
   anything. The pre-registered line rule, the 1 − 1/n quantile, is on 424 arrays the second-highest value, and these
   arrays are it. Without them the Breast p99 is 0.030; the bulk width alone would still put the Breast limit near
   5 %, so removing them would not have passed B7 either — the bar fails on the width, not only on the outliers.
