...
def _atlas_baseline_for_cpgs(cpg_set, baseline_cell, cfg):
    ...
    arch = next((d for d in (root/"atlas", root.parent/"atlas", root.parent/"MethylPhys"/"atlas") if d.is_dir()), root/"atlas") / "iamatlas_class_archives/immune_v0_1_REBUILD.tar.xz"
    if not arch.exists():
        return {}
