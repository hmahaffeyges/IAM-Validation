...
- [x] **Strawman LIT UP**: fixed path resolution (find engine assets via `CPG_ENGINE_ROOT`) + the age/sex `None`→`""` bug (`html.escape(None)` crash). Patient wall (81 cols) + crown-jewel wall (81×81) render in iframes. [`strawman_data_v2.json`](Crown%20Jewel%20and%20Patient%20Strawman/strawman_data_v2.json) (builders/) is the live one.
