...
**What this folder is.** The instrument. Everything a run executes, everything those modules import, the runtime matrices they read, the test package that proves it works, and the generators that keep the documents true. If a file is in here, something in a run touches it - and nothing else belongs: presentation material, the pre-atlas record and the author's own copies live under `Record/`, and superseded code lives under `RETIRED_2026-09/`. Every file below is classified by what calls it, not by its name, and [`kit/evaluate_necessity.py`](../kit/evaluate_necessity.py) will tell you which.


The chain that scores a sample. Start at [`../doors/RUNBOOK.md`](../doors/RUNBOOK.md) to run one and [`../doors/CHAIN_SEQUENCE.md`](../doors/CHAIN_SEQUENCE.md) for the step order as the code calls it. When a run refuses, the cause and the fix are under the step that refused it in [`../sop/MethylPhys_CPG_SOP.md`](../sop/MethylPhys_CPG_SOP.md) (§11-§19), in Issue 003 section 3b, and on the report's own Troubleshooting tab.
