...
def decompress_atlas(root):
    # root is the chain directory; the atlas is its sibling. Both layouts tried so a move cannot break this.
    _ad = next((d for d in (root.parent/"atlas", root/"MethylPhys"/"atlas", root.parent.parent/"MethylPhys"/"atlas") if d.is_dir()), root.parent/"atlas")
    xz = _ad / "IAMAtlasREBUILD.csv.xz"
    csv = _ad / "IAMAtlasREBUILD.csv"
    if csv.exists():
