# Walther Clinical Pipeline — Build Specification v1.1
...
### 4.3 Patient metadata (V1 — doctor-supplied)

```json
{
  "patient_id": "string — any unique identifier the doctor chooses",
  "age_years": 47,
  "sex_at_birth": "F" or "M",
  ...
  "family_history": {           // optional — conditional consumption per §4.5
    "breast_cancer_first_degree": false,
    "colorectal_cancer_first_degree": false,
    "alzheimers_first_degree": false
  },
  "current_medications": ["array, optional"],
  "lifestyle_notes": "string, optional"
}
```

Eventual production flow (V2+) — patient creates online account, pays for test, fills questionnaire, orchestrator pulls metadata via API — is **out of V1 scope**. V1 reads `patient_metadata.json` from the doctor's folder.
...
Every file ships in the production folder ahead of the data that activates it. The orchestrator never errors on a missing optional input — it logs the absence in the audit trail and moves on.
...
**Output of Stage 4:** 8 per-class A-scores + 115 per-cell-type A-scores, each with status.
...
### Stage 7 — Tier breakpoints (SOP §59–§64 / walkthrough Stage 4)

1. Load `tier_breakpoints.json`.
2. Apply engine tiers to each per-class A-score:
   - A ≤ 1.05 → NORMAL
   - 1.05 < A ≤ 1.07 → MARGINAL
   - 1.07 < A ≤ 1.10 → DETECTABLE
   - A > 1.10 (or above class ceiling 1/H_min) → BREACH
   - A significantly below age-matched expectation → BELOW_NORMAL
3. **Bidirectional flag** (SOP §63 / walkthrough §4 Stage 4):
   If a class shows `BELOW_NORMAL` AND any disease pattern matching at Stage 8 includes a documented suppression signature (AD-instance B-vs-T-lineage divergence, long-window pre-diagnostic breast secretory homogenization), the report flags this explicitly rather than treating BELOW_NORMAL as "uninteresting." Suppression is signal.
4. Apply customer-facing collapse (BELOW_NORMAL→SUPPRESSED, NORMAL→NORMAL, MARGINAL→ELEVATED, DETECTABLE/BREACH→SIGNIFICANTLY_ELEVATED) per `tier_breakpoints.json`'s `engine_to_customer_translation` block. In V1 doctor report, retain both engine-tier and customer-tier; in V2 patient report, show only customer-tier.
5. If substrate is `plasma_cfdna` (NOT in V1 lock — but the code path exists for future unlock): apply `cfdna_weight.json` to compute expected-vs-observed per-class fractions. Surface significant departures.

**Output of Stage 7:** Tier classifications at both engine and customer granularity; bidirectional flags; cfDNA context (when applicable).
