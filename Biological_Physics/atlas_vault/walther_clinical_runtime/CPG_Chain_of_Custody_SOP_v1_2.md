# CPG Chain-of-Custody Standard Operating Procedure (SOP) — v1.2 (walkthrough aligned)
...
2. **Marker version restored** — `iamatlas_celltype_markers_v0_2` is canonical (v1.1 wrongly downgraded these to v0_1; v0_1 is SUPERSEDED). 15 references restored; SHA-256 updated to `46ea5be1db377f2b8773a02418a7f481a191630e0fa833d3294eab1fd19c47bd`.
3. **New §1.5** — three-component architectural separation (orchestrator + report builder + future website) per walkthrough §6. Includes §1.5.1 conditional-consumption discipline.
4. **New §10.5** — consolidated 13 non-negotiable rules per walkthrough §8.
5. **§17 Step 0.7b** — HM450 ≥80% coverage gate + VAL-091 platform tagging.
6. **§31 Step 2.4b** — credible intervals from atlas MCMC posteriors propagated through deconvolution.
7. **§62 Step 7.4b** — bidirectional flag discipline for BELOW_NORMAL + documented-suppression-pattern coincidence (AD-immune B-vs-T-lineage; long-window breast secretory homogenization).
8. **§65 dual-matching preamble** — Stage 8 runs Path A (cards) + Path B (matrix) in PARALLEL with worked example.
9. **§73 Step 9.4b** — risk-context formula `posterior_context_class = baseline_prior × age_factor × sex_factor × fh_factor × match_magnitude` with strict framing rule.

No paragraphs from v1.1 deleted. All v1.1 content preserved; v1.2 is additive + targeted L-mapping corrections.

---

# CPG Chain-of-Custody SOP — Part I (Foundations) (v1.2 — walkthrough aligned)
