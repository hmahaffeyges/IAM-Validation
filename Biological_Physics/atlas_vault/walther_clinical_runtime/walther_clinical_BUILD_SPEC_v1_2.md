...
**Plate 1 conventions are the binding contract for Stage 4.6:** HEALPix NSIDE=128 (npix=196,608), Mollweide projection, CpGs ordered by chromosome × MAPINFO with sequential pixel assignment in genomic order, multi-CpG-per-pixel averaging. Patient projections use the same `iamatlas_cpg_to_healpix_nside128.npy` mapping file (to be generated at IAMAtlas build time) so the customer's personal CMM sits on the same grid as the reference.
