...
| **L6** Covariance modeling | FILLED via Mahalanobis hull | Stage 5 (Mahalanobis distance against `mahalanobis_healthy_reference_v0_4.json`'s pooled HC covariance n=2,481 across 7 cohorts including EPIC platform via AIBL, Ledoit-Wolf shrinkage, percentile-calibrated Route A threshold) |
...
│   ├── mahalanobis_healthy_reference_v0_4.json (CURRENT PRODUCTION: n_hc=2,481, 7 cohorts, HM450+EPIC)
...
# Production loads CURRENT version — currently v0_4 (n=2,481 pooled HC, 7 cohorts, HM450 + EPIC).
hull = MahalanobisHealthyHull("Mahalanobis_healthy_reference/mahalanobis_healthy_reference_v0_4.json")
...
**Route A trigger threshold is percentile-based, NOT a fixed value.** With 112 features under multivariate normality, expected median Mahalanobis distance is √112 ≈ 10.58 — a fixed threshold like `d ≥ 2.0` would fire on ALL samples (the v0_1 mistake). The current v0_4 thresholds are:
- p95 (default Route A): d ≥ 13.62
- p99 (strict): d ≥ 18.59
calibrated against the pooled n=2,481 HC distance distribution stored in the artifact under `route_A_calibration_v0_4`. Production engine reads the threshold from the artifact at session startup — not hard-coded in any card or module.

**Validation anchor lineage** (preserved as HC hull expands):
- Breast pre-dx Cohen's d (GSE51057 n=11): v0_1 +1.871 → v0_2 +0.981 → v0_3 +0.896 → v0_4 +0.593
- Breast pre-dx Cohen's d (GSE51032 n=36): v0_1 +2.088 → v0_2 +1.653 → v0_3 +1.611 → v0_4 +1.450
- Case detection % at p95 threshold (GSE51032): v0_2 50.0% → v0_3 55.6% → v0_4 38.9%
- Case detection % at p95 threshold (GSE51057): v0_2 9.1% → v0_3 27.3% → v0_4 9.1%
...
- **Phase 4** (queued): Add Asian-population HC cohort. Candidate cohorts identified: Han Chinese first-episode schizophrenia n=476 HC on EPIC (Mol Psychiatry 2020); GSE89093 IHEC n=92 HM450; HELIOS Study Singapore EPIC (Nat Comm 2025). Requires cohort acquisition session (not blocking June 11 meeting; v0_4 is production-ready as cross-platform multi-population reference).
