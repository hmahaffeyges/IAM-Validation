...
**Operating rule.** Stage 4 / Stage B computes A = H(β̄)/H_min on identity loci. Before that step, β is mapped onto the Roadmap scale with the entry in  matching the pipeline that produced it (Stage-1 noob → ; a GEO series matrix → the author-processed entry once fit). If no map exists for the pipeline, the reading is labelled  and is not reported as a tier. This section supersedes any earlier text implying A can be read directly from any β.
