...
Kit path prefixes below are relative to the kit root. Repo paths are relative to the repo root; `CPG_Engine/` = `Biological_Physics/CPG_Engine/`, `VAULT/` = `Biological_Physics/RETIRED/PostBuild_atlas_vault_snapshot_2026-06/`.
...
| age reference band (8 classes × 10 decades) | REPO `CPG_Engine/` (trial bundle copy identical) `age_reference_matrix.json` | `runtime/` | canonical; compiled as H(β̄)/H_min |
...
| conductor (presence-paired scoring, DETECT_FLOOR 0.01; 'replaces walther_clinical.py', 2026-07) | **YOUR FOLDER only** — supplied to this session four times as an upload; **NOT in the repo at 66f37fe, NOT in CPG_TRIAL_CODE.zip** → **must be committed to REPO** `CPG_Engine/` | `engine/cpg_conductor.py` (289-line version; a 95-line stub also circulates — discard it) | repo is missing the file the chain is defined by (RECON D2 for the 3% second floor) |
...
|---|---|---|---|
| foundation-cohort anchors v1 (GSE51032 n=460, GSE51057 n=188, 115 cells) | REPO `Biological_Physics/Testing_and_Code/VAL_PostAtlas/foundation_cohort/` | `anchors_v1/` | SUPERSEDED by v2 (RULING M1b); keep |
...
8. **commit `cpg_conductor.py` (289-line, 2026-07) to `CPG_Engine/`** — the running chain's orchestrator exists only in your folder
