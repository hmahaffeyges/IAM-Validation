...
V50=os.path.join(BP,"Testing_and_Code/VAL_PreAtlas/val_050_aibl"); V51=os.path.join(BP,"Testing_and_Code/VAL_PreAtlas/val_051_ad_directional")
ENG=os.path.join(BP,"CPG_Engine"); DATA=os.environ.get("CPG_KIT_DATA",os.path.join(HERE,"data")); OUT=os.path.join(HERE,"results"); os.makedirs(OUT,exist_ok=True)
...
if not os.path.exists(geo): res["B5"]={"pass":None,"note":"GSE153712_normalized_average_betas.txt.gz not in CPG_KIT_DATA - B5 not run; row 4.5 NOT commissioned"}; print("B5 raw GEO: file absent - SKIPPED (row not commissioned)")
    ...
    f=gzip.open(geo,"rt",errors="replace"); hdr=None; rows={}
    for l in f:
        if hdr is None:
            if l.startswith("ID_REF") or l.startswith("\"ID_REF") or l.lower().startswith("probe") or l.startswith("cg") is False and "\t" in l and not l.startswith("!"):
                hdr=[c.strip('"') for c in l.rstrip("\n").split("\t")]; continue
            continue
        cg=l[:l.find("\t")].strip('"')
        if cg in cpgs: rows[cg]=[c.strip('"') for c in l.rstrip("\n").split("\t")[1:]]
        if len(rows)==len(cpgs): break
    f.close()
    cols=hdr[1:]; want=[(s,i) for i,c in enumerate(cols) for s in [c] if s in betas]
    # GEO columns may be sentrix ids possibly with suffixes; try exact then prefix
    if not want: want=[(s,i) for i,c in enumerate(cols) for s in betas if c.startswith(s) or s.startswith(c.split(".")[0])]
    md=0;cnt=0;nan=0
    for s,i in want:
        for cg,vals in rows.items():
            v=vals[i]
            if v in ("","NA","null"): nan+=1; continue
            g=betas[s].get(cg)
            if g is None: continue
            cnt+=1; md=max(md,abs(float(v)-g))
    res["B5"]={"header_cols":len(cols),"matched_samples":len(want),"cpgs_found":len(rows),"values_compared":cnt,"missing":nan,"max_abs_diff":md,"pass":bool(len(want)==len(betas) and len(rows)==len(cpgs) and md<1e-4)}
    print(f"B5 raw GEO: samples matched {len(want)}/{len(betas)}, cpgs {len(rows)}/{len(cpgs)}, values {cnt}, max|diff| {md:.2e} -> {'PASS' if res['B5']['pass'] else 'FAIL'}")
