...
**WHAT THE REPORT SAYS (author, 2026-09-21).** Cells detected and their percentages (Stage 2); the A-score of each cell the chain could calculate (separation surface) and of each class (identity gauge on the three-layer reference) with band placement and tier; the Stage 5 departure with the laboratory's false-alarm rate; the patient's sky; every flag raised, logged. It names NO disease and gives NO age in years. The disease matrix is record-side: as trusted-chain cohorts accumulate we log each cohort's cellular behaviour meticulously; the instrument's job is not to say "this pattern looks like X".
