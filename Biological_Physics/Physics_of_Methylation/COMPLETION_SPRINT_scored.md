...
|---|---|---|
| A1 | `cpg_null_runner.py`, nulls N1–N8 | **BUILT, live** (`CPG_Engine/CPG_Null_Runner/`). Every post-Atlas CPG-VAL carries `null_results.json`. The one phase built on trusted bones; still the best-engineered part of the chain. |
| A2 | synthetic patient generator | **BUILT** (`RETIRED/…/Synthetic_Patient_Generator/`; harness in `CPG_Engine/report_builders/`). N7 depends on it — to be restored beside the null runner. |
