...
def cover(story):
    ...
    news = [
     ...
     ("Healthy is a band, not a line - and the band has three layers.", "A = 1.0 is the architectural commitment line. A patient is read against FLOOR (H_min, physics, universal) + PIPELINE MAP (one affine per pipeline onto the Roadmap scale; Stage 1s COMMISSIONED, PHASE 1c) + LAB ZERO (set once per lab on healthy controls, CLSI EP28; control-probe route under test, LAB-ZERO-01/02), then placed in a per-decade percentile band of healthy mapped A (identity_band_v2, PROVISIONAL). The shipped age_reference_matrix.json was compiled on the marker-union statistic and is NOT the reference for the identity-loci gauge (PROC-N7-01). The per-class input offset explored in July is retired."),
