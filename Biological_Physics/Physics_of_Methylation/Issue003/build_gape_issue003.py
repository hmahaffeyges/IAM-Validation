...
def cover(story):
    ...
    news = [
     ...
     ("Cellular age is closed as not reportable (PROC-AGE-01) - the gauge measures fidelity, not time.", "The healthy immune curve rises 0.045 across a lifetime against a within-lab spread of 0.0235: one array resolves age to about 50 years (16 of 100 healthy within 10 yr). The report prints the resolution, never an age. With this, no reported number in the chain reads the marker-union statistic."),
...
def sec8_procedures(story):
    ...
    proc(story, 'PROC-AGE-01 - cellular age on the identity gauge: not reportable at single-array resolution', 'The gauge measures fidelity, not time', [(k, dict(D.AGE_PROC)[k]) for k in ('question','finding','correction')])
