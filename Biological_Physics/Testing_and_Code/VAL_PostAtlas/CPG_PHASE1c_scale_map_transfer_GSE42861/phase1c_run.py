#!/usr/bin/env python3
"""PHASE 1c — scale-map + band TRANSFER to GSE42861 controls (n=335). Executes PREREG.md as sealed: NOTHING is fit here.
...
TAR = os.path.join(ROOT, "idats/GSE42861/GSE42861_RAW.tar"); OUT = os.path.join(ROOT, "results/phase1c"); os.makedirs(OUT, exist_ok=True)
IDAT_DIR = os.path.join(ROOT, "idats/GSE42861/idats"); os.makedirs(IDAT_DIR, exist_ok=True)
...
def metadata():
    ...
    url = "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE42nnn/GSE42861/matrix/GSE42861_series_matrix.txt.gz"
    ...
    for l in txt.split("\n"):
        ...
        if l.startswith("!Sample_characteristics_ch1"):
            ...
            if vals and vals[0].lower().startswith("disease state"): dz = [v.split(":")[-1].strip() for v in vals]
...
def calibrate_all(pairs):
    ...
    cache = os.path.join(OUT, "betas_GSE42861_controls.pkl")
...
def main():
    ...
    mband = json.load(open(os.path.join(E, "Runtime Matrices/A_Scoring_Module/scale_map_addendum.json")))["mapped_band"]   # GSE87571, frozen
    meta = metadata(); ctrl = set(meta.index[meta.disease == "Normal"]); log(f"metadata {len(meta)} | controls {len(ctrl)}")
    ...
    # P5 lab layer: control median per decade minus GSE87571 mapped median (p50 not stored in mapped_band; use band midpoint proxy AND raw GSE87571 per_sample if present)
    p5 = {}
    try:
        S1 = pd.read_csv(os.path.join(ROOT, "results/phase1/per_sample.csv"), index_col=0)
        S1 = S1[(S1.immune_frac >= 0.80) & (S1.epi_frac <= 0.02) & S1.age.notna()]
        add = json.load(open(os.path.join(ROOT, "results/phase1/scale_map_addendum.json")))
        g87 = add["gain"]
        # recompute GSE87571 mapped A with the AFFINE map for a like-for-like median (Phase 1 addendum used the gain form)
        df1 = pd.read_pickle(os.path.join(ROOT, "results/phase1/betas_GSE87571.pkl")); l1 = [c for c in IMM if c in df1.index]
        S1["A_mapped_affine"] = [float(H((np.nanmean(df1[g].loc[l1].values) - icpt) / slope) / ident["immune"]["H_min"]) for g in S1.index]
        for k in per_dec:
            lo, hi = map(int, k.split("-")); v1 = S1[(S1.age >= lo) & (S1.age <= hi)].A_mapped_affine
            if len(v1) >= 30: p5[k] = dict(n42861=len(per_dec[k]), med42861=float(np.median(per_dec[k])), n87571=int(len(v1)), med87571=float(v1.median()), offset=float(np.median(per_dec[k]) - v1.median()))
        gse87571_median_affine = float(S1.A_mapped_affine.median())
    except Exception as e:
        gse87571_median_affine = None; p5 = {"error": str(e)[:120]}
    ...
    # N-random
    rng = np.random.default_rng(1); mb = df.mean(1); tgt = mb.loc[loci].mean(); cand = mb[(mb > tgt - 0.05) & (mb < tgt + 0.05)].index.values
    RND = list(rng.choice(cand, size=min(len(loci), len(cand)), replace=False))
    ...
    res = dict(P1_coverage=round(p1, 4), P2_presence=round(p2, 4), n_after_gate=int(len(Sb)), P3_median_mapped_A=round(med, 4), P3_pass=bool(p3),
               ...
               P6_test_arrays=p6, P6_cohort_p5_p95=(round(lo5, 4), round(hi95, 4)), N_random_in_band=None if nrand is None else round(nrand, 4), N_sex_d=nsex, N_smoke=nsmoke, N_plate=nplate,
               unmapped_median_A=round(float(Sb.A_immune.median()), 4))
    json.dump(res, open(os.path.join(OUT, "phase1c_results.json"), "w"), indent=1, default=str)
