#!/usr/bin/env python3
"""PROC-CMB-01 — CHAIN_COMMISSIONING row 4.6: the patient's sky, commissioned against held-out healthy arrays (PREREG sealed 2026-09-21).
...
RT=os.path.join(ENG,"Runtime Matrices"); ATLAS=os.path.join(BP,"IAM_Atlas/IAMAtlasREBUILD.csv")
...
r=subprocess.run([sys.executable,os.path.join(RT,"Patient_CMB/build_healpix_mapping.py"),ATLAS,os.path.join(BP,"IAM_Atlas/external_manifests/EPIC_plus_HM450_combined_manifest_normalized.csv"),tmp],capture_output=True,text=True)
...
# C2/C3/C4
scales={}; held={}; res["C2"]={}; res["C4"]={"arrays":0,"conforming":0,"violations":[]}
for lab,fn in LABS.items():
    ...
    S.save_scale(sc,os.path.join(RT,"Patient_CMB",f"residual_scale_{lab}.npz"),lab,{"panel":panel,"seed":2028,"pipeline":"stage1_noob_450K","proc":"PROC-CMB-01"}); scales[lab]=sc
    held[lab]={g:mapped(B,g) for g in test}; del B,P
    fz=[];mz=[];skies={}
    for g,b in held[lab].items():
        sky=S.patient_sky(b,F[g],MEANS,sc,LOCI,mapping); fz.append(sky["all"]["frac_abs_z_gt2"]); mz.append(sky["all"]["median_z"])
        res["C4"]["arrays"]+=1; f=F[g]; ok=all((sky["classes"][c]["assessable"]==(f.get(c,0)>=0.02)) for c in S.CLASSES) and all(not sky["classes"][c]["assessable"] for c in ("stromal","cycling","secretory","terminal","stem_pluri")) and sky["classes"]["immune"]["assessable"]
        res["C4"]["conforming"]+=int(ok)
        if not ok: res["C4"]["violations"].append({"gsm":g,"fractions":{c:round(f.get(c,0),3) for c in S.CLASSES}})
        skies[g]=sky
    med_f,med_z=float(np.median(fz)),float(np.median(mz)); ok=0.03<=med_f<=0.08 and abs(med_z)<=0.15
    res["C2"][lab]={"n_test":len(fz),"median_frac_abs_z_gt2":med_f,"median_z":med_z,"frac_range":[float(min(fz)),float(max(fz))],"pass":ok,"immune_panel_median_frac":float(np.median([s["classes"]["immune"]["frac_abs_z_gt2"] for s in skies.values()]))}
    ...
    if lab=="GSE87571":   # C6 on the first held-out array
        g=test[0]; a1=S.render_plate(skies[g],os.path.join(OUT,f"plate_{g}_run1.png"),f"{g} · {lab} · healthy held-out"); a2=S.render_plate(S.patient_sky(held[lab][g],F[g],MEANS,sc,LOCI,mapping),os.path.join(OUT,f"plate_{g}_run2.png"),f"{g} · {lab} · healthy held-out")
...
res["C4"]["pass"]=res["C4"]["conforming"]==res["C4"]["arrays"]; print(f"C4 gating: {res['C4']['conforming']}/{res['C4']['arrays']} conforming -> {'PASS' if res['C4']['pass'] else 'FAIL'}")
# C3 cross-lab: each lab's scale applied to every other lab's held-out
res["C3"]={}
for la in LABS:
    for lb in LABS:
        if la==lb: continue
        fz=[S.patient_sky(b,F[g],MEANS,scales[la],None,mapping)["all"]["frac_abs_z_gt2"] for g,b in list(held[lb].items())[:20]]
        mz=[S.patient_sky(b,F[g],MEANS,scales[la],None,mapping)["all"]["median_z"] for g,b in list(held[lb].items())[:20]]
...
json.dump(res,open(os.path.join(OUT,"proc_cmb_01.json"),"w"),indent=1,default=str); print("row 4.6 commissioned:",res["row_4_6_commissioned"])
