#!/usr/bin/env python3
"""PROC-CMB-02 (supersedes CMB-01, which failed C2/C4 as sealed) — CHAIN_COMMISSIONING row 4.6: the patient's sky, commissioned against held-out healthy arrays (PREREG sealed 2026-09-21).
...
PF=S.presence_floors({g:F[g] for lab in LABS for g in sel[lab]["panel"]}); json.dump({"_meta":{"proc":"PROC-CMB-02","rule":"max(0.02, p99 of Stage 2 class fraction over the 160 healthy PANEL arrays of four labs)","seed":2028},"floors":PF},open(os.path.join(RT,"Patient_CMB","presence_floors_v1.json"),"w"),indent=1); res["presence_floors"]=PF; print("presence floors:",{k:round(v,3) for k,v in PF.items()})
...
for lab,fn in LABS.items():
    ...
    S.save_scale(sc,os.path.join(RT,"Patient_CMB",f"residual_scale_{lab}.npz"),lab,{"panel":panel,"seed":2028,"pipeline":"stage1_noob_450K","proc":"PROC-CMB-02","zero":"m = panel mean residual, subtracted at patient time"}); scales[lab]=sc
...
res["C4"]["pass"]=res["C4"]["conforming"]==res["C4"]["arrays"] and res["C4"]["nonblood_rendered"]<=3 and res["C4"]["immune_rendered"]==res["C4"]["arrays"]; print(f"C4' gating on TEST arrays: rule {res['C4']['conforming']}/{res['C4']['arrays']}, non-blood class rendered in {res['C4']['nonblood_rendered']} (bar <= 3), immune {res['C4']['immune_rendered']}/{res['C4']['arrays']} -> {'PASS' if res['C4']['pass'] else 'FAIL'}")
...
json.dump(res,open(os.path.join(OUT,"proc_cmb_02.json"),"w"),indent=1,default=str); print("row 4.6 commissioned:",res["row_4_6_commissioned"])
