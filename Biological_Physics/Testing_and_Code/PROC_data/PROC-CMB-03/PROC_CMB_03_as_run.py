#!/usr/bin/env python3
"""PROC-CMB-03 (supersedes CMB-01, which failed C2/C4 as sealed) — CHAIN_COMMISSIONING row 4.6: the patient's sky, commissioned against held-out healthy arrays (PREREG sealed 2026-09-21).
...
for lab,fn in LABS.items():
    ...
    S.save_scale(sc,os.path.join(RT,"Patient_CMB",f"residual_scale_{lab}.npz"),lab,{"panel":panel,"seed":2028,"pipeline":"stage1_noob_450K","proc":"PROC-CMB-03","zero":"m = panel mean residual, subtracted at patient time"}); scales[lab]=sc
...
json.dump(res,open(os.path.join(OUT,"proc_cmb_03.json"),"w"),indent=1,default=str); print("row 4.6 commissioned:",res["row_4_6_commissioned"])
