...
"""
===============================================================================
VAL-126 — TCGA-STAD Phase C run-everything scorer
===============================================================================
Pre-registered VAL-126 (sealed 2026-05-02, prereg SHA 8f47ba2e7253...).

Scores every TCGA-STAD HM450 sesame Level 3 β file against:
...
# extended to all 25 tiles for run-everything completeness in VAL-126 (8 added tiles
# are bulk-tissue epithelia → secretory class). This extension is the methodology
# gap-fill for VAL-126; CCL-class note logged.
LOYFER_CLASSES = {
    # Immune (per VAL-118)
    ...
    # Bulk-tissue epithelia — extended in VAL-126 to fill run-everything gap
    'Bladder': 'secretory',
    'Breast': 'secretory',
...
PROGRESS_NDJSON = OUTPUT_DIR / 'val126_per_sample_progress.ndjson'
...
def load_beta_file(path, all_atlas_cpgs):
    """Stream a TCGA-STAD β file. Return (full_array, atlas_betas_dict)."""
    full = []
    atlas_betas = {}
    with open(path) as f:
        for line in f:
...
def main(start_idx, end_idx):
    print('=' * 78)
    print(f'VAL-126 — TCGA-STAD Phase C run-everything scorer [{start_idx}, {end_idx})')
    print('=' * 78)
    ...
    # Load manifest
    with open(OUTPUT_DIR / 'tcga_stad_hm450_manifest_FINAL.json') as f:
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: val126_stad_phase_c.py <start_idx> <end_idx>')
        sys.exit(1)
