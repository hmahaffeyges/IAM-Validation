# VAL-123 — Phase B Calibration of BoccellatoStomachRef_HM450 v1 on VAL-106 Cohort

**Prereg version:** v1.0-FINAL (sealed 2026-05-02)
...
- VAL ID: **VAL-123** (sequential after VAL-122 bladder_stage3_immune, the most-recent VAL in repo as of seal date 2026-05-02)
...
This calibration VAL is the **anchor cohort** for BoccellatoStomachRef. Subsequent VALs (VAL-123+2 STAD, VAL-123+3 ESCA, etc.) will perform CHK-3.2 against the per-tile means and SDs sealed here. Mismatches >1 anchor-SD flagged; >3 anchor-SDs invalidate cross-cohort statistic.
...
- **Source code:** `val123_boccellato_calibration.py` (to be authored; standard NumPy + pandas + scipy.stats; ~120 lines)
- **Inputs:**
...
- **Expected output:** `val123_boccellato_calibration_results.json` containing per-tile mean/SD/quantile distributions + per-tissue stratification + atlas-family-fitness diagnostics.
