...
| bar | result | verdict |
|---|---|---|
| T1 one definition | Before: three tier definitions (report builder ×2 with NORMAL ending at 1.04, colour map a fourth; JSON v1.3 at 1.01). After: `MethylPhys/chain/cpg_tiers.py` reads `tier_breakpoints.json`; identity stage, both report-builder sites and the colour map call it; 0 literal breakpoints remain; every boundary ±1e-6 agrees | PASS |
