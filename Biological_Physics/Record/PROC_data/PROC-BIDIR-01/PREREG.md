...
- **B4 engine = seal** — `MethylPhys/chain/Runtime Matrices/Directional Panel/bidirectional_decomposition.a_dir_score` applied to the sealed 7-CpG Rule A panel returns, for every AIBL holdout sample, the same A_dir as `val051_analyze.py` (max |diff| < 1e-9).
