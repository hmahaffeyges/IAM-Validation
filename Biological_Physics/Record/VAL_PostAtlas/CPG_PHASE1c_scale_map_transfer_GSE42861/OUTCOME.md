...
| N-sex | report | \|d\| 0.64 (35–44), 0.28, 0.69 (55–64), 0.37 (unsigned as run) — see signed correction below | direction NOT established by the run |
...
4. N-sex, SIGNED (correction 2026-09-20: the runner reported |d| only; direction was asserted without support and is corrected here from per_sample.csv): GSE42861 d(f−m) = −0.08 (25–34), +0.64 (35–44), +0.28 (45–54), +0.69 (55–64), +0.37 (65–74) — women higher from 35 up. GSE87571 (Phase 1, same decades): −0.44, −0.30, −0.24 — women LOWER. The sex effect FLIPS SIGN between the two labs, so it is not a stable biological term; the sex-split band recommended from Phase 1 is WITHDRAWN pending a signed test on the pooled, mapped data with a lab term. Smoking does not move the immune gauge — a clean null worth a line in the report.
...
- Build `identity_band_v2`: GSE87571 + GSE42861 controls, mapped A, pooled per decade, lab random effect reported; sex as a SIGNED null with a lab term (split only if the sign agrees across labs); prereg first; its P3/P4 tested on a THIRD Stage-1 healthy cohort.
...
---
**CORRECTION 2026-09-20 (post-seal, labelled):** the N-sex row above was computed as |d| and the sentence 'women higher every decade' had no support in the run. Signed values from `per_sample.csv` (`nsex_signed.json`): GSE42861 women higher 35–74; GSE87571 women lower 35–64. Sign flips between labs → sex-split band withdrawn; sex becomes a signed null with a lab term in the band_v2 prereg. The sealed sha256 above covers the text before this correction.
