...
## v2.4.1 changes (2026-05-02) — DISC-BLADDER-003 retroactive flag

Per the bladder-epic v0.1 sprint of 2026-05-01, the cookbook formalized a new structural rule (DISC-BLADDER-003 → CHK-2.18 in TESTING_CHECKLIST.md): bulk-WGBS atlases (Loyfer, Caggiano CelFiE TIM, Sabedot, MARLIN) used as the primary cell-of-origin reader on a mucosal-tissue cohort produce inflated cross-tile A-scores driven by substrate-distribution mismatch rather than cell-of-origin biology. The rule mandates that mucosal-tissue cohorts (bladder, lung-airways, colon, cervical, esophagus, stomach, GI-epithelium, oral-mucosa) include at least one gene-promoter sub-cell-type atlas (EpiSCORE per-tissue bridge) in `atlases_run` as the primary cell-of-origin reader. Bulk-WGBS atlases remain in `atlases_run` for triangulation but cannot be the headline signal. The crc-epic card has retroactive exposure to this rule because colon and rectal tissue are mucosal-class (colonic epithelium plus rectal mucosa), and crc-epic Stage 2 has historically used the Loyfer 25-tile WGBS `Colon_epithelial_cells` tile as the primary tissue-of-origin reference for tissue-arm scoring.

**Affected VALs in this card (retroactive flag, sealed outcomes stand):** VAL-061 (TCGA-COAD tumor-infiltrating immune compartment scoring against immune H_min, paired d = +1.0658); VAL-062 (TCGA-COAD tumor architecture cycling-class scoring, paired d = +0.7241); VAL-098 (TCGA-READ paired tumor/adjacent-normal cycling-class scoring with run-everything 25-tile, paired d = +0.612 full-HM450, Colon_epithelial_cells tile d = −2.50); VAL-099 (TCGA-COAD revisit reproducing VAL-062 byte-for-byte plus run-everything 25-tile, full-HM450 d = +0.7241, Colon_epithelial_cells tile d = −1.603).

**What the retroactive flag means.** The sealed outcomes for VAL-061, VAL-062, VAL-098, and VAL-099 stand as they were sealed — no SHA-256 hashes change, no paired-d values change, no outcome class labels change, no clinical action matrix changes, no current tier (`cross_platform_validated (blood) + cycling_class_tissue_validated (tissue) + cycling_class_tissue_validated_with_rectal_subsite`) changes. The flag is a forward-looking documentation note: future crc-epic Stage 2 cell-of-origin work (planned for v2.5+) MUST run a gene-promoter sub-cell-type atlas (EpiSCORE ColonRef when bridged) alongside any bulk-WGBS read, and the gene-promoter atlas is the headline. EpiSCORE ColonRef exists in the pan-tissue distribution (the v2.2 atlas vault check on 2026-04-28 surveyed it as available at the source-Entrez-matrix level) but has not yet been CpG-bridged via the per-tissue bridge methodology used for HeartRef (VAL-111), ProstateRef (VAL-117), BladderRef (VAL-119) — the bridge engineering queues alongside the next crc-epic Stage 2 sprint per option-(c) deferred-bridge-build pattern from the bladder-epic v0.1 cookbook rollout.

**The CCL-039 cookbook-wide pattern is structurally consistent with DISC-BLADDER-003.** The v2.4 changes section of this README documented the three-cohort confirmation of CCL-039 LL-MARKER-CPG-TILE-FIDELITY: full-HM450 cycling-class scoring reads positive on tumor-vs-adjacent-normal (TCGA-COAD +0.724, TCGA-READ +0.612, TCGA-COAD revisit +0.7241) while the corresponding Colon_epithelial_cells run-everything tile reads strongly negative (TCGA-READ −2.50, TCGA-COAD −1.55, TCGA-COAD revisit −1.603). The negative cell-of-origin tile direction was already named by the cookbook as the cell-of-origin tile fidelity DEGRADES in tumor pattern. DISC-BLADDER-003 reframes that observation in the substrate-distribution-mismatch lens: the bulk-WGBS Colon_epithelial_cells tile encodes a population-average tumor-free colon β profile, and CRC tumor's mucosal methylation distribution shape diverges from that bulk reference globally (not just at colon-specific marker CpGs), producing the strong-negative tile read by the same |β_sample − β_bulk_ref| substrate-mismatch mechanism that produced bladder VAL-121's all-14-non-bladder-Loyfer-tiles uniform POSITIVE +2.34 to +2.92 firing. The two framings — CCL-039 cell-of-origin tile fidelity degradation vs DISC-BLADDER-003 substrate-distribution mismatch — are not mutually exclusive; both are consistent with the same observed three-cohort negative tile direction. A multi-atlas reading that included EpiSCORE ColonRef as the primary cell-of-origin reader would distinguish the two: if ColonRef sub-cell-type tiles read consistently with the cell-of-origin biology (analog to bladder VAL-121's BladderRef Epi NEGATIVE signal at d = −1.46), DISC-BLADDER-003 is the better framing; if ColonRef tiles read no signal, CCL-039 cell-of-origin tile fidelity is the operative mechanism. v2.5+ work resolves this question.

**v2.5 promotion path additions.** When the crc-epic v2.5 sprint runs, EpiSCORE ColonRef must be bridged via the same Entrez-gene-ID → 450K-CpG bridge methodology used for HeartRef (VAL-111), ProstateRef (VAL-117), BladderRef (VAL-119), with a Phase B mini-calibration on TCGA-KIRC + TCGA-PRAD adjacent-normal n=210 sealed before any disease-cohort scoring per CHK-2.7 magnitude-based |d| thresholds with direction labels. The atlas-family-fitness rule (DISC-CARDIO-004 + DISC-PROSTATE-001 + DISC-BLADDER-001) predicts ColonRef separation will depend on cell-type distinctness for the colon tissue's gene-promoter signatures, NOT cell-type count alone — bladder's 4-cell-type BladderRef separated cleanly while cardio's 5-cell-type HeartRef collapsed.

**Anatomy stratification interaction.** The v0.4 anatomy section documented the C18 (colon proper) / C19 (rectosigmoid) / C20 (rectum) Stage 1 inversion magnitudes on GSE51032 peripheral blood (d ≈ −0.57 / −0.56 / −0.13 respectively). When EpiSCORE ColonRef is bridged in v2.5, the anatomy stratification at the Stage 2 tissue level may benefit from sub-cell-type colon resolution that the current single bulk Loyfer Colon_epithelial_cells tile does not provide. ColonRef pan-tissue distribution includes 5 colonic sub-cell-types per the v2.4 atlas vault survey (2026-04-28); the Phase B calibration will determine which of these separate cleanly on the calibration cohort and which collapse to the tissue-floor-dominated baseline. This is a v2.5+ research question, not a v2.4.1 deliverable.

**EDEAR commercial deployment unaffected.** Per the `commercial_deployment_unaffected_by_validation_limitations` block inherited from CCL-037 (lung-epic v0.5 standard, propagated to crc-epic in v2.4), EDEAR commercial deployment is single-pipeline patient-vs-internal-reference scoring. The DISC-BLADDER-003 retroactive flag applies to retrospective Cookbook validation methodology, not to per-patient deployment scoring against the public Loyfer/Moss/Tanaka atlases anchored through L1 partner-lab pipeline calibration.
