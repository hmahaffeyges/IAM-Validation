...
**Version 0.5 · 2026-04-28**
**Validation tier:** `multi_modal_validated + cycling_class_tissue_validated`
**Supersedes:** v0.4 (2026-04-24, tissue arm added). v0.3 (2026-04-24, universal reference block). v0.2 (`multi_modal_validated` from VAL-056). v0.1 (initial landscape-survey-only).
**Change from v0.4:** VAL-097 added — never-smoker LUAD tissue characterization on GSE256092 vs TCGA-LUAD adjacent-normal returned O5_BASELINE_DOMINATED (cross-cohort calibration boundary documented). Never-smoker LUAD tissue tile pattern remains uncharacterized; VAL-098 queued on GSE235414 with internal matched adjacent-normal. CCL-037 and CCL-038 lessons logged.
...
## v0.5 changes (2026-04-28)

- **VAL-097 added — never-smoker LUAD tissue characterization (GSE256092 Korean NSLA EPIC, n=141, vs TCGA-LUAD adjacent-normal HM450, n=29).** Outcome `O5_BASELINE_DOMINATED`. Cross-cohort comparison structurally invalidated by SWAN-vs-sesame preprocessing pipeline mismatch + Korean-vs-Western population baseline shift + bulk-tumor-vs-adjacent-normal cell-composition difference. The lung-of-origin tile read d = −0.27 (second-weakest of 25 tiles). 22 of 25 tiles uniformly positive; 11 of 25 breached >3 anchor-SD on CHK-3.2; top-1 ΔA call was B-cells_EPIC in 79/141 (56%) of patients.

- **Never-smoker LUAD tissue tile pattern remains UNCHARACTERIZED.** VAL-063's lifelong non-smoker arm (n=2) was direction-consistent but underpowered. VAL-097 attempted to characterize the never-smoker stratum on a never-smoker-enriched cohort but the cross-cohort comparator (TCGA-LUAD adjacent-normal) was not pipeline-compatible and the result reflected calibration mismatch rather than biology. **The card status for never-smoker LUAD tissue characterization is therefore: open, follow-up VAL-098 recommended.**

- **VAL-098 queued — driver-stratified never-smoker LUAD with internal matched adjacent-normal samples.** Candidate cohort: GSE235414 (driver-stratified LUAD with internal paired adjacent-normal in the same cohort and same pipeline — proper within-cohort case-vs-control comparison per CHK-3.8 condition 1). VAL-098 prereg to be drafted ahead of execution.

- **CCL-037 LL-CROSS-COHORT-CALIBRATION lesson added.** When two methylation cohorts use different preprocessing pipelines AND different platforms AND different populations without batch correction, cross-cohort comparison is structurally invalid. Three-condition test (within-cohort paired / same-pipeline re-processing / batch-corrected anchor) is mandatory before running cross-cohort VALs as primary disease comparisons.

- **CCL-038 LL-PRELOCK-DEGENERATE-COMPARATOR lesson added.** O5 auto-assignment criteria must test the breach pattern directly (≥3 tiles >3 anchor-SD AND ≥80% same-direction = baseline-dominated), not via comparison to quantities that collapse to the same number under realistic sample-size structure.

- **`commercial_deployment_unaffected_by_validation_limitations` block to be added to card JSON.** EDEAR commercial deployment is single-pipeline patient-vs-internal-reference by construction; cross-cohort calibration boundary applies exclusively to retrospective cookbook validation. Patient one is just as valid as patient one thousand because the reference is the public Loyfer/Moss/Tanaka atlases anchored through L1 partner-lab pipeline calibration.

- **Validation tier unchanged.** lung-epic remains at `multi_modal_validated + cycling_class_tissue_validated` from v0.4. VAL-097 does not promote or demote the tier — it documents a structural-invalidity pattern in cross-cohort retrospective validation that does not affect the validity of VAL-056 (blood Stage 1 / Stage 2) or VAL-063 (tissue cycling-class paired). The cookbook record gains an honest documentation entry for what could not be characterized via available public-tier cross-cohort approaches.

- **File additions:** `val_097.py`, `VAL-097/prereg.md`, `VAL-097/outcome.md`, `VAL-097/results.json`, `VAL-097/stratified.json`, `VAL-097/cohort_manifest.json`, `VAL-097/clinical_metadata.csv`, `VAL-097/per_sample.csv`, `VAL-097/tile_heatmap.png`, `VAL-097/PREREG_SEAL.txt`. All in `Biological_Physics/validation_runs/VAL-097/` after GitHub push.

- **`future_when_support_arrives.md` log started.** Per LL-PUBLIC-TIER, biobank-gated cohorts that would resolve the never-smoker LUAD tissue characterization at higher power (Shanghai Cohort Study, Taiwan Biobank lung methylation arm, full Korean NSCLC methylation consortia data with paired adjacent-normal) are catalogued in this file but not pursued. EDEAR commercial deployment is unaffected by these access limitations.

---

## v0.5.1 changes (2026-05-02) — DISC-BLADDER-003 retroactive flag

Per the bladder-epic v0.1 sprint of 2026-05-01, the cookbook formalized a new structural rule (DISC-BLADDER-003 → CHK-2.18 in TESTING_CHECKLIST.md): bulk-WGBS atlases (Loyfer, Caggiano CelFiE TIM, Sabedot, MARLIN) used as the primary cell-of-origin reader on a mucosal-tissue cohort produce inflated cross-tile A-scores driven by substrate-distribution mismatch rather than cell-of-origin biology. The rule mandates that mucosal-tissue cohorts (bladder, lung-airways, colon, cervical, esophagus, stomach, GI-epithelium, oral-mucosa) include at least one gene-promoter sub-cell-type atlas (EpiSCORE per-tissue bridge) in `atlases_run` as the primary cell-of-origin reader. Bulk-WGBS atlases remain in `atlases_run` for triangulation but cannot be the headline signal. The lung-epic card has retroactive exposure to this rule because lung tissue is alveolar epithelium plus airway-derived mucosal tissue, and lung-epic Stage 2 has historically used the Loyfer/Moss bulk WGBS `lung_epithelial` tile as the primary cell-of-origin reader.

**Affected VALs in this card (retroactive flag, sealed outcomes stand):** VAL-056 (Stage 2 Moss NNLS deconvolution → `lung_epithelial` top-1 localization, ΔA = +0.14304 with 60.87× confidence ratio); VAL-063 (cycling-class tissue scoring on TCGA-LUAD HM450 paired pairs, full-HM450 paired d = +1.020); VAL-097 (cross-cohort run-everything 25-tile on GSE256092 vs TCGA-LUAD adjacent-normal, sealed `O5_BASELINE_DOMINATED`).

**What the retroactive flag means.** The sealed outcomes for VAL-056, VAL-063, and VAL-097 stand as they were sealed — no SHA-256 hashes change, no paired-d values change, no outcome class labels change, no clinical action matrix changes, no current tier (`multi_modal_validated + cycling_class_tissue_validated`) changes. The flag is a forward-looking documentation note: future lung-epic Stage 2 cell-of-origin work (planned for v0.6+) MUST run a gene-promoter sub-cell-type atlas (EpiSCORE LungRef when bridged) alongside any bulk-WGBS read, and the gene-promoter atlas is the headline. The bridge engineering for EpiSCORE LungRef has not yet been completed — it queues alongside the next lung-epic Stage 2 sprint per option-(c) deferred-bridge-build pattern from the bladder-epic v0.1 cookbook rollout.

**Why VAL-097 is on the flag list specifically.** VAL-097 used run-everything 25-tile Loyfer multi-cancer/cell-type readout against TCGA-LUAD adjacent-normal as the structurally-separated baseline. The 25 Loyfer tiles include `Lung_cells` (bulk WGBS mucosal-airway-derived reference), and the breach pattern observed (22 of 25 tiles uniformly positive, 11 of 25 tiles >3 anchor-SD) carries the substrate-distribution mismatch signature DISC-BLADDER-003 names. The CCL-037 (cross-cohort calibration) and CCL-038 (degenerate comparator) lessons that VAL-097 produced are the right primary frame for that VAL's outcome; DISC-BLADDER-003 is a complementary framing that reaches the same `O5_BASELINE_DOMINATED` conclusion via a different structural pathway (substrate-distribution mismatch specifically on mucosal bulk-WGBS tiles). Both framings stand.

**v0.6 promotion path additions.** When the lung-epic v0.6 sprint runs, EpiSCORE LungRef must be bridged via the same Entrez-gene-ID → 450K-CpG bridge methodology used for HeartRef (VAL-111), ProstateRef (VAL-117), BladderRef (VAL-119), with a Phase B mini-calibration on TCGA-KIRC + TCGA-PRAD adjacent-normal n=210 sealed before any disease-cohort scoring per CHK-2.7 magnitude-based |d| thresholds with direction labels. The atlas-family-fitness rule (DISC-CARDIO-004 + DISC-PROSTATE-001 + DISC-BLADDER-001) predicts LungRef separation will depend on cell-type distinctness for the lung tissue's gene-promoter signatures, NOT cell-type count alone — bladder's 4-cell-type LungRef-equivalent BladderRef separated cleanly while cardio's 5-cell-type HeartRef collapsed.

**EDEAR commercial deployment unaffected.** Per the `commercial_deployment_unaffected_by_validation_limitations` block inherited from CCL-037, EDEAR commercial deployment is single-pipeline patient-vs-internal-reference scoring. The DISC-BLADDER-003 retroactive flag applies to retrospective Cookbook validation methodology, not to per-patient deployment scoring against the public Loyfer/Moss/Tanaka atlases anchored through L1 partner-lab pipeline calibration.

**Cross-references.** TESTING_CHECKLIST.md §CHK-2.18; LESSONS_LEARNED.md §DISC-BLADDER-003; EDEAR_PIPELINE_OFFICIAL_REFERENCE_v2.md §Part 25 (atlas family selection rule per tissue class); README_MASTER v2.7 amendment line; GAPE_Reproduction_Paper_v1.md §7.27.7 (CHK-2.18 formalization) + §7.27.8 (retroactive Category-A audit); CROSS_CARD_CALIBRATION_TODO_v0_7.md changelog entry.
