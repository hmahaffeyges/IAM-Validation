...
## ABSOLUTE RULE — Run-everything architecture (CCL-033, signed off 2026-04-26)

**Every IDAT runs Stage 1 + Stage 2 + Stage 3 with all panels and atlases regardless of any single-stage result.** No conditional gating. The patient-facing report can collapse uninformative tiles for display, but the underlying scoring is exhaustive on every IDAT. Per-class A-scores are computed for every tissue every IDAT.

**Why no gating.** The framework's primary value proposition under multi-disease scoring is concurrent anomaly detection: a patient with early AD + early breast pre-clinical + chronic inflammation + cardiovascular drift fires four anomaly tiles simultaneously, which the clinician interprets in combination. Conditional gating filters out exactly the patterns the framework is designed to surface. VAL-092 demonstrated this concretely: PSP cortical-neuron drift d=−0.43 was only detected because Stage 2 ran on every IDAT regardless of Stage 1 status.

**Operational rules:**

- CHK-3.2 (cross-cohort baseline) is mandatory every run, not optional.
- Every prereg enumerates the multi-disease detection patterns it designs to surface (CHK-2.1 expanded).
- Within-cohort statistics are primary (CCL-034); cross-cohort interpretable at secondary tier only when platform AND preprocessing match.
- Patient-facing reports surface platform tag + cross-cohort baseline status of every reported tile.

Cards documented as run-everything-compliant (2026-04-26+): breast-epic v2.3, ad-immune v2.2, glioma-epic v0.2. Pre-architecture cards (using "if Stage 1 hits DETECTABLE" gating language) require retrofit at next version bump: lung-epic v0.4 → v0.5, crc-epic v2.3 → v2.4, pancreatic-epic v0.1 → v0.2, heme-epic v0.1 → v0.2, cervical-epic v0.1 → v0.2, hcc-epic v0.1 → v0.2.

---

## ABSOLUTE RULE — Substrate saturation is framework physics (CCL-037, formalized 2026-04-27)

**Each architecture class × substrate pair has a physical ceiling A_max = 1 / H_min(class, substrate).** When β approaches 0.5 (max-entropy uniformly random state), the substrate's A-score approaches A_max and stops contributing information. Saturated substrates do NOT participate in severity ranking. Including them in the AUC-weighted A_combined biases the score toward the floor and masks continued progression.

**Three patterns the cookbook treats as framework-foundational:**

1. **Nucleosome occupancy saturates below BREACH in 7 of 8 classes.** Confirmatory at DETECTABLE; cannot distinguish URGENT from FLOOR_BREACH. Exception: stem_pluri (TGCT inversion).
2. **Progenitor and stem_adult have only 2/5 BREACH-usable substrates** (methyl + frag). Uncommitted developmental architectures retain less structural signature to lose. Mask % is +55.1% / +61.9% — legacy A_combined under-reports severity by more than half the total signal at the disease reference.
3. **Stem_pluri inverts the saturation pattern entirely.** Methyl, fuzz, frag saturate; nucl, wps carry BREACH-capable signal. TGCT cells are MORE methylated than matched normal — disease produces lower A-scores. Mask sign-flips negative; the inversion is itself diagnostic.

**The substrate-scope translation rule (v1 / L2 / L3 deployment):**

- **v1 EDEAR (production today):** methyl-only buffy-coat / cfDNA on 450K/EPIC arrays. 1 of 5 substrates. AUC weight 0.8663 / 4.198 = 20.6% of total. L1 lab-partnership tier ($50–150/sample, 6–12 mo).
- **L2 (medium-term partnership, 18–24 mo, $500K–1.5M custom capture panel):** adds nucl + fuzz. 3 of 5 substrates. AUC weight ~59.5% of total.
- **L3 (full multi-assay platform, year 3+):** adds wps + frag. 5 of 5 substrates. The complete framework prediction.

When Issue 002 frames a multi-substrate prediction (e.g., "A_AML ≈ 1.10 with ΔA = +0.168"), that prediction is L3-scope. v1 readings are NOT directly comparable to that magnitude. Direction transfers; absolute magnitude does not. **VAL-082 AML is the canonical example:** Issue 002 predicts A_AML ≈ 1.10 (L3); v1 measured methyl-only A=0.54 with d=+3.71. The d effect size is the v1 deployment validation; the A magnitude is not directly comparable.

**Two-substrate saturation as binary cancer indicator (terminal class only).** Neurodegenerative disease (AD/ALS/PD/cardiac aging) produces A_methyl 1.01–1.09 without saturating either nucl or wps; glioma (LGG/GBM) produces A_methyl 1.20–1.29 with both nucl AND wps saturated. **Two-substrate saturation in terminal-class CSF cfDNA is a binary glioma indicator.** This binary property is class-specific — does not generalize to other classes that saturate only one substrate.

**Operational requirements:**

- Every prereg declares per-substrate saturation classification at the disease reference state (CHK-3.7).
- Every results JSON reports per-substrate A-score + saturation flag (boolean) + A_combined (all available) + A_combined_active (non-saturated only).
- Every outcome.md comparing to an Issue 002 prediction states the substrate-scope translation explicitly.
- The H_MIN_TABLE lives in code (`GAPE_WEB_v13.py`, `FullVersion_build_gape_issue002.py`); reference docs mirror with explicit "code wins" precedence.

The full saturation matrix (8 classes × 5 substrates ceilings + mask %) is in `EDEAR_PIPELINE_OFFICIAL_REFERENCE_v2.md` Part 14. CHK-3.7 in TESTING_CHECKLIST is the operational enforcement.

---

## ABSOLUTE RULE — Honest revision when later evidence shifts a prior outcome label (CCL-036, formalized 2026-04-27)

When a later VAL provides direct measurement of a mechanism the earlier VAL inferred indirectly, the earlier VAL's outcome label is updated transparently — original label preserved with revision date and trigger noted. The sealed pre-reg is never altered (it is a historical document); the outcome.md carries both labels; the lessons file documents the revision as a methodological example.

**The canonical pattern: VAL-088 → VAL-090.** VAL-088 sealed `O5_POSITIVE_INVERTED` (cell-fraction prior appeared inverted by the A-score). VAL-090 measured cell fractions directly via NNLS; the cell-fraction prior was correct. VAL-088's outcome label was revised to `O1_PASS_ORTHOGONAL_PRIORS_BOTH_CONFIRMED`. Cell-fraction direction (NLR-style) and Shannon-entropy A-score direction are orthogonal metrics, not opposites.

**Revision is treated as evidence the framework is working, not as a problem.** Honest revision is the alternative to silent drift. The cookbook's reputation depends on being able to say "we got it wrong here; new evidence showed the actual mechanism." Operationalized in TESTING_CHECKLIST as CHK-4.8.

---

## ABSOLUTE RULE — Stratification by sex, age, and smoking is mandatory in every prereg (CCL-002 + CCL-009 + age, formalized 2026-04-27)

The referee always asks: did you stratify by the obvious confounders before reporting d? If the obvious confounder for the disease is not in the prereg's stratification block, the prereg fails.

**Sex (CCL-002, ABSOLUTE):** every prereg declares M, F, and any reported "other" or unknown bucket. Single-sex cohorts get the explicit "opposite-sex generalization not tested" caveat.

**Age:** at minimum, mean ± SD for cases and controls. Where n permits (cases ≥ 30 per stratum), break out by decade or quartile. Cross-cohort age-distribution comparison required; if mean ages differ by > 5 years, the prereg names the age-residual analysis required (per VAL-052 AD cross-platform: age regression R² = 26%; age-adjusted residual d was the right primary statistic, not raw d).

**Smoking (CCL-009, ABSOLUTE for lung, pancreas, bladder, kidney, head-and-neck, cervix; STRONGLY ADVISED for HCC, breast, leukemia/lymphoma):** four-strata canonical: never / former-≥10yr / former-<5yr / current. Underpowered strata (n < 5) are reported with explicit "underpowered for independent inference, descriptive only" caveat per VAL-063 (lifelong non-smoker n=2). Per-CpG smoking handling: AHRR (cg05575921), F2RL3 (cg03636183), 2q37.1 region attenuate over 5–10 yr post-cessation per Baglietto 2017. For lung-epic specifically: Hong 2019 cg12169243 (DPH6) + cg25429010 (IMP3) reach genome-wide significance in **current smokers only** — these CpGs cannot be interpreted as cancer markers without smoking strata.

**Other disease-specific covariates:** tumor stage, treatment status (treatment-naive / on-treatment / post-treatment), TtD window for pre-dx cohorts (0-2 / 2-5 / 5-10 / >10 yr), grade for cancers with grade variation, IDH/MGMT for glioma, ER/HER2 for breast, MSI for CRC, viral status for HCC. The prereg names every covariate the analysis will stratify on; post-hoc covariates added after β access are forbidden.

**Honest underpower language.** When a stratum has n < 5 in either arm, the prereg pre-locks that the stratum's d is reported as "underpowered for independent inference, descriptive only." This prevents post-hoc framing of an underpowered stratum as evidence in either direction. VAL-063 lifelong non-smoker (n=2) d=+0.567 is the canonical example: directionally consistent, magnitude smaller, sample too small to power independent inference. The outcome.md said exactly that.

Operationalized in TESTING_CHECKLIST as CHK-2.7.
