...
<h3>The levels</h3><table class='t'><tr><th>level</th><th>on the gauge</th><th>what it means</th></tr>"""]
    ...
    H.append("<h3>The saturation chart - 8 classes x 5 substrates, and which cells are capped below breach</h3>"
      f"<p>Each cell shows the class's frozen floor H_min on that substrate and, in brackets, the ceiling 1/H_min it implies; <b>CAPPED</b> marks a ceiling below the breach line at {BR}. The floors come from the frozen 40-value table (methylation from the G-002 calibration, the four cfDNA/chromatin substrates from G-003b). Only the methylation column is lit today - see Coverage.</p>"
      ...
      + "".join("<tr><td>"+CLASS_LABEL.get(c,c)+"</td>"+"".join(
            (f"<td class='n' style='background:#2a1d1d'>{v:.4f} <span class='m'>[{1.0/v:.3f}]</span> <b>CAPPED</b></td>" if 1.0/v < BR
             else f"<td class='n'>{v:.4f} <span class='m'>[{1.0/v:.3f}]</span></td>")
            for v in R["hmin_table"][c])+"</tr>" for c in CLASSES if c in R["hmin_table"])
      + "</table>"
      f"<p class='m'><b>15 of the 40 combinations are capped below breach.</b> Nucleosome occupancy caps 7 of the 8 classes (its floors are all near 0.98-0.99, so there is almost no range above healthy before saturation). Fuzziness caps the three stem and progenitor classes (pluripotent stem, adult stem, progenitor). WPS caps a different three - <b>terminal</b>, adult stem and progenitor - and notably does <i>not</i> cap pluripotent stem, whose WPS ceiling is {1.0/R['hmin_table']['stem_pluri'][3]:.3f}, just above the line. On methylation - the one lit column - <b>pluripotent stem is capped at {1.0/R['hmin_table']['stem_pluri'][0]:.3f}</b>, which is below breach and barely above the healthy band, so a pluripotent-stem breach cannot be read on methylation at all.</p>"
      f"<p class='m'><b>The reference clusters past breach, against the methylation ceilings.</b> The breakpoints file carries senescent cells at {R['tiers']['tier_system_v1_2']['reference_clusters_past_breach']['senescent_cells']['a_low']}-{R['tiers']['tier_system_v1_2']['reference_clusters_past_breach']['senescent_cells']['a_high']} and malignant cells at {R['tiers']['tier_system_v1_2']['reference_clusters_past_breach']['malignant_cells']['a_low']}-{R['tiers']['tier_system_v1_2']['reference_clusters_past_breach']['malignant_cells']['a_high']}. The highest methylation ceiling of any class is the terminal class at {1.0/R['hmin_table']['terminal'][0]:.3f}. So the senescent range lies entirely below that one ceiling and above every other class's; the malignant range <b>straddles it</b> - its lower part ({R['tiers']['tier_system_v1_2']['reference_clusters_past_breach']['malignant_cells']['a_low']}-{1.0/R['hmin_table']['terminal'][0]:.3f}) is reachable on the terminal class only, and <b>its upper part exceeds every methylation ceiling there is</b>, so no class can produce those readings on this surface. Which class and which surface those clusters were measured on therefore has to be stated wherever they are quoted - the marker-union surface has a different denominator and different limits. On this chain they are carried as the corpus's reference values and have not been re-measured.</p>")
