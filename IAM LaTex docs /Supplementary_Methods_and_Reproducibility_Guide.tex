...
\begin{abstract}
This document provides complete code, data sources, and step-by-step instructions to independently reproduce all IAM validation results. The holographic horizon dynamics framework achieves 5.5$\sigma$ improvement over $\Lambda$CDM through dual-sector coupling: photon-sector H$_0 = 67.4$ km/s/Mpc (CMB, $\beta_\gamma < 1.4 \times 10^{-6}$) and matter-sector H$_0 = 72.5 \pm 1.0$ km/s/Mpc (local, $\beta_m = 0.157 \pm 0.029$). Model selection criteria ($\Delta$AIC = 26.0, $\Delta$BIC = 25.4) show no evidence of overfitting. All code executes in under 2 minutes on standard hardware. Complete theory and test results are presented in the companion Test Validation Compendium.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textit{Variational Derivation} --- Formal derivation chain (Jacobson $\to$ Cai-Kim $\to$ IAM), constrained scalar field action, perturbation theory ($\mu < 1$, $\Sigma = 1$)
    \item \textit{IAM--CAMB Technical Note} --- $\mu$--$\Sigma$ modified gravity mapping, Python-level CAMB validation, MGCAMB implementation roadmap
\end{itemize}

\subsection{Key Results Summary}

\textbf{Statistical Performance:}
\begin{itemize}
    \item $\chi^2_{\Lambda\text{CDM}} = 38.28$ (10 data points)
    \item $\chi^2_{\text{IAM}} = 8.27$
    \item $\Delta\chi^2 = 30.01$ (5.5$\sigma$ improvement)
\end{itemize}

\textbf{Model Selection (Overfitting Check):}
\begin{itemize}
    \item $\Delta$AIC = 26.0 $\rightarrow$ ``Decisive'' evidence for IAM (Burnham \& Anderson)
    \item $\Delta$BIC = 25.4 $\rightarrow$ ``Very strong'' evidence for IAM (Kass \& Raftery)
    \item Relative likelihood: $\Lambda$CDM is 444,000$\times$ less likely than IAM
\end{itemize}

\textbf{Parameters (MCMC Posteriors):}
\begin{itemize}
    \item Matter-sector: $\beta_m = 0.157 \pm 0.029$ (68\% CL, MCMC)
...
\begin{itemize}
    ...
    \item H$_0$(matter/local) = 72.5 $\pm$ 1.0 km/s/Mpc
...
%==============================================================================
\section{CAMB Background Validation}
\label{sec:camb_validation}
%==============================================================================

The script \texttt{camb\_iam\_background.py} validates IAM background cosmology against the community-standard Boltzmann code CAMB (v1.6.5). This bridges the gap between the analytic IAM framework and the precision required for CMB and large-scale structure predictions.

\subsection{Physical Basis}

IAM's dual-sector structure requires that the photon sector ($\beta_\gamma = 0$) exactly recovers $\Lambda$CDM at all redshifts. The CAMB validation implements IAM as a dark-energy fluid with equation of state $w_\text{info}(a) = -1 - 1/(3a)$ in the matter sector, leaving the photon sector untouched. This validates that:
\begin{enumerate}
    \item The pre-recombination universe is identical in IAM and $\Lambda$CDM (sound horizon, CMB peaks)
    \item The matter-sector expansion rate matches the analytic IAM Friedmann equation
    \item Distance moduli reproduce the Hubble tension magnitude
    \item BAO angular positions are photon-sector observables and therefore identical to $\Lambda$CDM
\end{enumerate}

\subsection{BAO Sector Assignment}

A critical result of the CAMB validation is the sector assignment for BAO observables. The BAO angular scale $\theta = r_s / D_A(z)$ is measured from photon arrival directions---galaxy angular positions are determined by photons traveling on null geodesics where $\beta_\gamma = 0$. Similarly, the radial BAO scale $D_H(z) = c/H(z)$ uses the photon redshift. Therefore:
\begin{align}
D_M^{\text{IAM}}(z) / r_s &= D_M^{\Lambda\text{CDM}}(z) / r_s \nonumber \\
D_H^{\text{IAM}}(z) / r_s &= D_H^{\Lambda\text{CDM}}(z) / r_s
\end{align}
IAM predicts BAO angular positions \textit{identical} to $\Lambda$CDM, consistent with DESI DR1 at $\lesssim 1\sigma$ across all redshift bins. The BAO \textit{amplitude} (clustering strength, measured by $f\sigma_8$) is matter-sector and suppressed by $\mu < 1$.

\subsection{Installation and Execution}

\textbf{Additional dependency:}
\begin{lstlisting}[language=bash]
pip install camb
\end{lstlisting}
...
\textbf{Expected runtime:} $\sim$30 seconds on standard laptop.

\subsection{Nine Background Tests}
...
\midrule
1 & $\Lambda$CDM baseline vs Planck 2020 & $\theta_s$, $r_s$, $z_*$ match \\
2 & Sound horizon invariance & $r_s = 147.22$ Mpc, unchanged \\
3 & $H(z)$ IAM vs $\Lambda$CDM & Ratio $= \sqrt{1 + \beta_m}$ at $z=0$ \\
4 & Distance moduli (SNe) & $\Delta\mu = -0.157$ mag $=$ $H_0$ tension \\
5 & BAO angular scale & Identical to $\Lambda$CDM (photon sector) \\
6 & CMB angular scale & Identical to $\Lambda$CDM ($\mathcal{E}(a) = 0$ at $z > 100$) \\
7 & Growth suppression & $\sim$7\% at $z=0$ (approximate) \\
8 & $H_0$ matter sector & 72.51 km/s/Mpc (0.51$\sigma$ from SH0ES) \\
9 & CMB power spectrum & First peak $\ell = 220$, $5753~\mu\text{K}^2$ \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Sound horizon:} $r_s = 147.22$ Mpc in both $\Lambda$CDM and IAM, because $\mathcal{E}(a) \to 0$ at $z > 100$. IAM resolves the Hubble tension \textit{without} modifying early-universe physics. This makes IAM mutually exclusive with Early Dark Energy (EDE) proposals.

\textbf{Matter-sector $H_0$:} The CAMB dark-energy fluid approximation independently confirms $H_0(\text{matter}) = 72.51$ km/s/Mpc, matching the analytic IAM prediction and within $0.51\sigma$ of SH0ES ($73.04 \pm 1.04$ km/s/Mpc).

\textbf{CMB consistency:} The CMB TT power spectrum is identical to $\Lambda$CDM because the photon sector is unmodified. The first peak location ($\ell = 220$) and amplitude ($5753~\mu\text{K}^2$) are unchanged.

\textbf{Remaining:} Full perturbation-level validation ($\mu < 1$, $\Sigma = 1$ in the Boltzmann hierarchy) requires MGCAMB. Background validation confirms all distance and expansion rate predictions.

\subsection{Output Figures}

The script generates a six-panel figure (\texttt{camb\_iam\_background.pdf}) showing:
\begin{enumerate}
    \item[(a)] Hubble parameter $H(z)$ for $\Lambda$CDM and IAM matter sector
    \item[(b)] Ratio $H_\text{IAM}/H_{\Lambda\text{CDM}}$, asymptoting to $\sqrt{1 + \beta_m} = 1.0759$
    \item[(c)] Distance modulus shift $\Delta\mu$: SNe appear closer in IAM
    \item[(d)] Gravitational coupling $\mu(z) < 1$, with $\mu(z=0) = 0.864$
    \item[(e)] CMB TT power spectrum (identical to $\Lambda$CDM)
    \item[(f)] Activation function $\mathcal{E}(a)$ with CAMB-validated regime highlighted
\end{enumerate}

Additionally, the feedback closure figure (\texttt{feedback\_closure.pdf}) and black hole bridge figure (\texttt{bh\_bridge\_formal.pdf}) are generated by separate scripts demonstrating the self-regulating expansion and multi-horizon encoding framework, respectively.
...
\begin{itemize}
    ...
    \item CAMB $\geq$ 1.5 (for Boltzmann-level validation, optional)
...
\textbf{Option 4: Including CAMB validation}
\begin{lstlisting}[language=bash]
pip install numpy scipy matplotlib emcee corner camb
\end{lstlisting}
...
\textbf{Step 2: Run the observational validation}
\begin{lstlisting}[language=bash]
python iam_validation.py
\end{lstlisting}

\textbf{Step 3: Run the CAMB background validation} (requires CAMB)
\begin{lstlisting}[language=bash]
python camb_iam_background.py
\end{lstlisting}

\textbf{Expected runtime:} 1--3 minutes for observational validation, $\sim$30 seconds for CAMB validation
...
\noindent\textbf{CAMB background validation} (\texttt{camb\_iam\_background.py}):
\begin{enumerate}
    \item[$\square$] $r_s = 147.22$ Mpc (unchanged from $\Lambda$CDM)
    \item[$\square$] $H_\text{IAM}/H_{\Lambda\text{CDM}} = \sqrt{1 + \beta_m} = 1.0759$ at $z = 0$
    \item[$\square$] $\Delta\mu = -0.157$ mag at $z = 0.01$
    \item[$\square$] CMB first peak at $\ell = 220$, amplitude $5753~\mu\text{K}^2$
    \item[$\square$] BAO angular positions identical to $\Lambda$CDM
    \item[$\square$] $H_0(\text{matter}) = 72.51$ km/s/Mpc
    \item[$\square$] All 9 background tests PASS
\end{enumerate}
...
\begin{itemize}
    \item \texttt{iam\_validation.py} --- Complete observational validation script (Section~5)
    \item \texttt{iam\_derivation\_tests.py} --- Derivation verification suite (10 tests)
    \item \texttt{camb\_iam\_background.py} --- CAMB background validation (9 tests, Section~\ref{sec:camb_validation})
    \item \texttt{mcmc\_final\_iam.py} --- Full Bayesian MCMC analysis
    \item \texttt{bh\_bridge\_formal.py} --- Black hole bridge holographic framework
    \item \texttt{feedback\_closure.py} --- Feedback closure and self-regulating expansion
    \item \texttt{data/} --- Observational data in machine-readable format
    \item \texttt{tests/} --- Individual test scripts for specific analyses
    \item \texttt{figures/} --- Scripts to reproduce all figures in Test Compendium
...
\begin{enumerate}
    ...
    \item Jacobson, T. (1995), Phys. Rev. Lett. 75, 1260 --- Thermodynamics of spacetime: Einstein equation as equation of state
    \item Cai, R.-G. and Kim, S. P. (2005), JHEP 02, 050 --- First law on FRW apparent horizon
    \item Lewis, A., Challinor, A., and Lasenby, A. (2000), Astrophys. J. 538, 473 --- CAMB Boltzmann code
\end{enumerate}
...
CAMB background validation confirms sound horizon invariance, \\
BAO consistency, and matter-sector $H_0$ prediction. \\[1em]
