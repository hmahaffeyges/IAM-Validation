...
print("=" * 70)
print("IAM VIRIAL SCALING TEST: Om_m = 0.33 (SDSS/BOSS/eBOSS)")
print("=" * 70)
print()
...
np.savez('mcmc_results_omega_scaling_high.npz',
         samples=samples,
         beta_m=beta_m_mcmc,
         beta_gamma=beta_gamma_mcmc)

print("Results saved to: mcmc_results_omega_scaling_high.npz")
print()
...
fig.suptitle('IAM Virial Scaling Test: Ω_m = 0.33', fontsize=14, y=1.02)
plt.savefig('mcmc_iam_corner_omega_scaling_high.pdf', bbox_inches='tight', dpi=300)
plt.savefig('mcmc_iam_corner_omega_scaling_high.png', bbox_inches='tight', dpi=150)
print("Corner plot saved!")
print()


# ============================================================================
# VIRIAL SCALING TEST: beta_m / Omega_m = 1/2 ?
# ============================================================================

print()
print("=" * 70)
print("VIRIAL SCALING TEST")
print("=" * 70)
print()
print(f"  Omega_m          = 0.33")
print(f"  Predicted beta_m = Omega_m/2 = {0.5 * 0.33:.4f}")
print(f"  MCMC beta_m      = {beta_m_mcmc[1]:.4f} +{beta_m_mcmc[2]-beta_m_mcmc[1]:.4f} -{beta_m_mcmc[1]-beta_m_mcmc[0]:.4f}")
print()
ratio_to_om = samples[:, 0] / 0.33
ratio_median = np.median(ratio_to_om)
ratio_16 = np.percentile(ratio_to_om, 16)
ratio_84 = np.percentile(ratio_to_om, 84)
print(f"  beta_m / Omega_m = {ratio_median:.4f} +{ratio_84-ratio_median:.4f} -{ratio_median-ratio_16:.4f}")
print(f"  Virial prediction: 0.5000")
print(f"  Deviation from 1/2: {abs(ratio_median - 0.5)/0.5 * 100:.1f}%")
print()
if abs(ratio_median - 0.5) < 0.1:
    print("  RESULT: Virial scaling beta_m = Omega_m/2 CONFIRMED within errors")
else:
    print("  RESULT: Virial scaling beta_m = Omega_m/2 NOT confirmed")
print()
print("=" * 70)
