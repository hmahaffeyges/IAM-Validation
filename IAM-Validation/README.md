...
- **mu--Sigma Mapping & CAMB:** See IAM--CAMB Technical Note
...
**Key signature:** mu < 1 with Sigma = 1 means matter feels weaker gravity while photon deflection is standard. This is testable with existing Boltzmann solvers (MGCAMB, EFTCAMB, ISiTGR) and uniquely distinguishes IAM from generic modified gravity theories. See the [IAM--CAMB Technical Note](docs/IAM_CAMB_Technical_Note.pdf) for full details.
...
### 7. Full Boltzmann Validation via MGCAMB (Separate Paper)

IAM's mu < 1, Sigma = 1 prediction has been validated through the MGCAMB modified Einstein-Boltzmann solver with zero free parameters.

Mahaffey, H. W. (2026). "Full Boltzmann Validation of the Informational Actualization Model via MGCAMB"

- Location: docs/IAM_MGCAMB_Validation.pdf
- Code: mgcamb_validation/mgcamb_reproducibility_package.py
- Figure: mgcamb_validation/iam_mgcamb_validation_6panel.pdf
- Full reproducibility log: mgcamb_validation/iam_mgcamb_reproducibility_log.txt

**Run the complete validation yourself:**
```bash
...
|------|-----------|--------|--------|
| sigma_8 | In [0.79, 0.82] | 0.7946 | PASS |
| CMB TT (ell > 30) | < 1% residual | 0.165% | PASS |
...
IAM produces sub-percent CMB TT residuals above ell = 30, preserves lensing (Sigma = 1 exact), reduces sigma_8 from 0.808 to 0.795 (easing the S8 tension), and suppresses growth at low redshift in agreement with BOSS/eBOSS data. The unique (mu < 1, Sigma = 1) signature distinguishes IAM from f(R) gravity (mu > 1, Sigma > 1), Horndeski theories (correlated mu-Sigma deviations), and w0waCDM (mu = Sigma = 1). This signature is directly testable by Euclid at 2-4 sigma within 5 years.
