# IAM -- mu < 1, Sigma = 1 (0 Free Parameters = LCDM) Perturbation & Background Levels Tested Against Planck 2018 (MGCAMB/CAMB)
12 converged MCMC chains via MGCAMB (Level 1) + 3 chains via modified CAMB (Level 2a, complete) | Delta-chi2 = -0.01 vs LCDM (Planck) | sigma_8: 0.809 -> 0.800 | Consistent with KiDS, DES, HSC weak lensing values
...
| **Level 2a** | Dual-sector perturbations, LCDM background | CAMB (modified) | 3 | **Complete** | Delta-chi2 = -0.01 (Planck); +2.00 apples-to-apples (Planck+RSD) |
| **Level 2b** | Modified dtauda background | CAMB (modified) | 4 | **In progress** | -- |
...
## Level 2: Dual-Sector Perturbation and Background Validation -- LEVEL 2a COMPLETE
...
- **[Raw MCMC chains](camb_validation/chains/)** -- complete chain data for all Level 2a runs (Runs A, C, D)
...
### Level 2a: Dual-Sector Perturbations, LCDM Background -- COMPLETE (3 chains converged)
...
|-------|------------|-----|------------|------------|-----|
| L2 Run A | IAM Level 2a, fixed | -0.13495 | ON | Planck CamSpec TTTEEE | 0.0099 |
| L2 Run C | LCDM baseline | 0 | OFF | Planck CamSpec | 0.0081 |
| L2 Run D | IAM Level 2a + RSD | -0.13495 | ON | Planck CamSpec + RSD | 0.0080 |
...
All standard cosmological parameters are consistent between IAM and LCDM (all shifts < 0.06 sigma). sigma_8 shifts from 0.809 to 0.800, a 1.1 sigma downward shift in the direction reported by weak lensing surveys (KiDS, DES, HSC). S8 shifts from 0.830 to 0.822. Delta-chi2 = -0.01, indicating that the dual-sector modification incurs no statistically significant penalty relative to LCDM under the Planck likelihood.
...
3. Delta-chi2 < 5 vs LCDM baseline: **-0.01** (Planck), **+2.00** (Planck+RSD apples-to-apples)
...
**Key test:** Does theta_s shift? If E(a) = exp(-1110) at recombination is numerically zero as expected, theta_s is protected by construction.

**Planned chains (4 chains, apples-to-apples by construction):**

| Chain | Description | Likelihood |
|-------|------------|------------|
| L2b Run A | IAM Level 2b (background + perturbation) | Planck CamSpec |
| L2b Run D | IAM Level 2b + RSD | Planck CamSpec + RSD |
| L2b Run C | LCDM baseline | Planck CamSpec |
| L2b Run F | LCDM + RSD baseline | Planck CamSpec + RSD |

### Level 2c: Modified Background + Late-Universe Datasets -- PENDING

Same modified code as Level 2b, with BAO (SDSS DR16) and Pantheon+ likelihoods added. LCDM baselines with identical likelihood sets for every new combination.
...
- **Planck MCMC (Level 2a):** 3 chains via modified CAMB Fortran (Delta-chi2 = -0.01 Planck, +2.00 Planck+RSD apples-to-apples); sigma_8 suppression confirmed as real growth physics
...
- **sigma_8 shift:** 0.8087 (LCDM) --> 0.7998 (IAM Level 2a), in the direction reported by weak lensing surveys
...
*Note: These model selection statistics apply to the H0 + f*sigma_8 dataset (10 measurements) only. Under the full Planck 2018 likelihood, IAM and LCDM are statistically indistinguishable: Level 1 Delta-chi2 = +1.43, Level 2a Delta-chi2 = -0.01. See Level 1 and Level 2 sections above for Planck MCMC results.*
...
3. **[IAM--CAMB Technical Note: Planck Level-1 Validation](docs/IAM_CAMB_Technical_Note.pdf)** (~23 pages)
 ...
 - *Note: Validates perturbation-level mu-Sigma mapping only (Level 1). Level 2a (dual-sector perturbation via modified CAMB) is complete; Level 2b (background modification) is in progress.*
...
*Note: Under the full Planck 2018 likelihood, IAM and LCDM are statistically indistinguishable (Level 1 Delta-chi2 = +1.43; Level 2a Delta-chi2 = -0.01). The preference above reflects IAM's ability to simultaneously accommodate the discrepant H0 measurements. See Level 1 and Level 2 sections above for Planck MCMC results.*
...
- Compatibility with Planck 2018 full likelihood: Level 1 across four dataset combinations (Delta-chi2 = +1.43 to +2.32); Level 2a dual-sector perturbation validation (Delta-chi2 = -0.01 Planck, +2.00 Planck+RSD apples-to-apples)
...
- **Level 2a:** Dual-sector perturbation split via modified CAMB equations.f90 -- Delta-chi2 = -0.01 vs LCDM (Planck), +2.00 apples-to-apples (Planck+RSD), sigma_8 = 0.800 (all 3 chains converged: Runs A, C, D). Raw chain data: [`camb_validation/chains/`](camb_validation/chains/)
...
**Status:** Level 1 complete (12 chains, 4 datasets, Delta-chi2 = +1.43 to +2.32 vs LCDM); Level 2a complete (3 chains, dual-sector perturbations, Delta-chi2 = -0.01 Planck / +2.00 Planck+RSD apples-to-apples, sigma_8 = 0.800); Level 2b in progress (4 chains, background modification); MGCAMB 7/7 Boltzmann tests passed; zero free parameters -- all derived from first principles
**Key Result:** Level 2a dual-sector perturbation validation yields Delta-chi2 = -0.01 relative to LCDM under the full Planck likelihood. All standard parameters shift by < 0.06 sigma. sigma_8 shifts from 0.809 to 0.800, consistent with the direction reported by weak lensing surveys. Sigma_8 suppression confirmed as real growth physics (logA shift 0.06 sigma, Omega_m shift 0.04 sigma). Level 1 perturbation-level validation (12 chains, 4 dataset combinations) yields Delta-chi2 = +1.34 to +2.32, all below the 95% CL exclusion threshold. Projected to be testable by Euclid (sigma(mu_0) ~ 0.04, yielding 3.4 sigma) and DESI Year 5.
