...
**TL;DR:** We introduce a simple, reproducible cosmological model (Informational Actualization Model, IAM) that empirically resolves the Hubble tension by showing that matter and photon observables probe different late-time cosmic expansion rates. The model’s activation function E(a) = exp(1 − 1/a) is derived from first principles using horizon thermodynamics — the 1/a arises from the ratio of structure formation rate to cosmic horizon area, confirmed to within 1% using the Sheth-Tormen halo mass function. The key prediction maps directly to μ(a) < 1, Σ(a) = 1 in standard modified gravity codes (MGCAMB, EFTCAMB), is validated at 5.5σ significance against all current data (Planck, SH0ES, Pantheon+), and can be independently reproduced by running the code in this repository in under 2 minutes.
...
**Key Finding:** 5.5 sigma empirical evidence for dual-sector cosmology resolving the Hubble tension, with activation function derived from horizon thermodynamics.
...
7. **[Variational Derivation](docs/Variational_Derivation_of_IAM.pdf)** (~12 pages)
   ...
   - Constrained scalar field action: S_info = ∫ [βH₀² exp(φ) + λ(φ̇ - H/a)] √(-g) d⁴x
   - Equation of state derived: w_info(a) = -1 - 1/(3a) (mildly phantom, consistent with DESI 2024)
   - Combined dark energy w₀ ≈ -1.06, predicting mild departure from ΛCDM
...
|------------|------------|----------|
| **DESI Year 5 (future)** | B_m to +/-1% precision | 2029 |
| **Euclid** | S8 = 0.78 +/- 0.01 | 2025-2030 |
| **Simons Observatory** | B_gamma < 0.001 (10x tighter) | 2025-2028 |
...
| Experiment | Prediction | Timeline |
|------------|------------|----------|
| **CMB-S4** | B_gamma < 10^-4 or detect nonzero coupling | 2030+ |
| **Euclid + Rubin** | BAO at z > 2 tests early-time behavior | 2030+ |
| **GW Standard Sirens** | H0(matter) consistent with distance ladder | 2030+ |
...
**Status:** 5.5 sigma preference for dual-sector cosmology over LCDM; activation function derived from horizon thermodynamics  
**Key Result:** The Hubble tension reflects measurements of two distinct expansion rates -- photons (CMB, B_gamma < 10^-5) and matter (BAO/distance ladder, B_m = 0.157). Both Planck and SH0ES are correct; they measure different sectors with empirically constrained ratio B_gamma/B_m < 10^-5 (95% CL, MCMC). Photons couple at least **100,000x more weakly** than matter to late-time expansion. The dual-sector phenomenology maps to μ(a) < 1, Σ(a) = 1 in standard modified gravity language, testable with MGCAMB/EFTCAMB.
