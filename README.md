...
**Key Finding:** 5.6 sigma empirical evidence for dual-sector cosmology resolving the Hubble tension. Zero free parameters -- coupling constant, activation function, and perturbation predictions all derived from first principles. Survives full Planck MCMC with MGCAMB Boltzmann solver (7/7 tests passed, 3 Planck chains converged).
...
- **Planck MCMC:** 3 independent chains converged cleanly (Delta-chi2 = 3.3 vs LCDM)
...
- **sigma_8 shift:** 0.811 --> 0.8014 (correct direction for S8 tension)
...
3. **[IAM--CAMB Technical Note](docs/IAM_CAMB_Technical_Note.pdf)** (~20 pages)
 ...
 - **Full Planck MCMC analysis** (3 independent chains via Cobaya + MGCAMB) with complete YAML files
...
7. **[Variational Derivation](docs/Variational_Derivation_of_IAM.pdf)** (~14 pages)
 ...
 - **Zero free parameters** beyond standard LambdaCDM: functional form, exponent, normalization, and amplitude all derived
 - **Perturbation theory derived:** delta = 0 (horizon quantity) ' standard GR perturbations on IAM background ' mu(a) < 1, Sigma(a) = 1
 - Unique mu--Sigma signature distinguishes IAM from f(R) and Horndeski theories (testable by Euclid)
...
- **mu--Sigma Mapping, MGCAMB Validation & Planck MCMC:** See IAM--CAMB Technical Note
...
### 7. MGCAMB Boltzmann Validation & Planck MCMC (in IAM--CAMB Technical Note)

IAM's mu < 1, Sigma = 1 prediction has been validated through the MGCAMB modified Einstein-Boltzmann solver (v1.5.2; Wang et al. 2023) and three independent Planck MCMC chains via Cobaya. All results are documented in the **[IAM--CAMB Technical Note](docs/IAM_CAMB_Technical_Note.pdf)**.
...
**Planck MCMC Results (3 chains, full Planck 2018 likelihood):**

Likelihoods: planck_2018_lowl.TT + lowl.EE + highl_plik.TTTEEE_lite_native + lensing.CMBMarged

| Run | mu_0 | sigma_8 | H0 [km/s/Mpc] | Best chi2 | Extra params |
|-----|------|---------|---------------|-----------|--------------|
| **A: IAM fixed** | -0.135 (fixed) | 0.8014 +/- 0.006 | 67.06 +/- 0.51 | 984.57 | 0 |
| **B: mu_0 floating** | 0.006 +/- 0.156 | 0.8146 +/- 0.016 | 67.16 +/- 0.51 | 981.24 | 1 |
| **C: LCDM baseline** | 0 (fixed) | ~0.811 | ~67.36 | ~981 | 0 |

**Key results:**
- **Delta-chi2 = 3.3** between IAM fixed and LCDM (statistically indistinguishable)
- IAM's predicted mu_0 = -0.135 is **within 0.9 sigma** of Run B posterior mean
- **sigma_8 shifted down** from 0.811 to 0.8014 in Run A (1.7 sigma toward weak lensing, correct direction for S8 tension)
- All standard parameters stable across all three runs (no pathology)
- Acceptance rates 75-77% with smooth convergence (no multimodality)
...
**Interpretation:** Planck is a gatekeeper test, not a detection test for IAM. The modification operates at z < 2 where Planck has limited sensitivity (only ISW and lensing). IAM passes cleanly, joining the small number of alternatives that survive the most constraining dataset in cosmology. The real detection tests are Euclid (sigma(mu_0) ~ 0.04, yielding 3.4 sigma) and DESI Year 5 growth rates.
...
- **Planck MCMC:** 3 independent chains (IAM fixed, mu_0 floating, LCDM baseline) -- IAM survives Planck
