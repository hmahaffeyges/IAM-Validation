...
\title{Dual-Sector Cosmology from Structure-Driven Expansion: The Informational Actualization Model}
...
For computational tractability and empirical testing, the framework is implemented as:
\begin{equation}
H^2(a) = H_0^2[\Omega_m a^{-3} + \Omega_r a^{-4} + \Omega_\Lambda + \beta \mathcal{E}(a)]
\label{eq:H_eff}
\end{equation}

with activation function derived from the ratio of structure formation rate to cosmic horizon area (see companion Variational Derivation):
\begin{equation}
...
At $z=0$ with $\beta = 0.157$:
\begin{equation}
\Omega_m(z=0) = \frac{0.315}{0.315 + 0.685 + 0.157} = 0.272
\end{equation}
...
\textbf{Matter-Sector Analysis (BAO + H$_0$ measurements)}

Dataset:
\begin{itemize}
\item SDSS/BOSS/eBOSS: $f\sigma_8(z)$ at 7 redshifts~\cite{Alam2021}
...
Result (MCMC):
\begin{equation}
\beta_m = 0.157^{+0.029}_{-0.029} \quad \text{(68\% CL)}
\end{equation}

This produces:
\begin{equation}
H_0(\text{matter}) = H_0 \sqrt{1 + \beta_m} = 67.4\sqrt{1.157} = 72.5 \text{ km\,s}^{-1}\text{Mpc}^{-1}
\end{equation}

Agreement with SH0ES: $\Delta = 0.54$ km\,s$^{-1}$\,Mpc$^{-1}$ ($0.52\sigma$).
...
With $\beta_m = 0.157$ (MCMC median, consistent with derived $\Omega_m/2$), the matter density parameter at $z=0$ becomes:
\begin{equation}
\Omega_m(z=0, \beta=0.157) = 0.272 \quad (\text{vs. } 0.315 \text{ in } \Lambda\text{CDM})
\end{equation}
...
Using BAO + $H_0$ measurements (10 data points):
...
SDSS/BOSS/eBOSS $f\sigma_8$ & 9.71 & 8.87 \\
\hline
Total & 41.63 & 10.38 \\
\hline\hline
\end{tabular}
...
\textbf{Interpretation} (Burnham \& Anderson): $\Delta$AIC $> 10$ is ``decisive'' evidence for the better model.
...
\textbf{Interpretation} (Kass \& Raftery): $\Delta$BIC $> 10$ is ``very strong'' evidence.
...
$H_0$(matter) & $72.5 \pm 1.0$ km\,s$^{-1}$\,Mpc$^{-1}$ \\
\hline\hline
\end{tabular}
...
$\beta_m$ (matter, MCMC) & $0.157^{+0.029}_{-0.029}$ \\
...
\hline
$\Delta$AIC & 27.2 (decisive) \\
$\Delta$BIC & 26.6 (very strong) \\
\hline\hline
\end{tabular}
...
\item \textbf{SDSS/BOSS/eBOSS}~\cite{Alam2021}: $f\sigma_8(z)$ at 7 redshifts
\end{enumerate}

Total: 10 independent measurements (growth: 7, $H_0$: 3).
...
\textbf{IAM (this work)} & \textbf{0} & \textbf{Yes} & \textbf{Partial} & \textbf{31.25} \\
\hline\hline
\end{tabular}
...
\item Zero free parameters: $\beta_m = \Omega_m/2$ derived from virial theorem; $\Delta$AIC $= \Delta$BIC $= 31.2$ for zero additional parameters
...
\begin{itemize}
\item Complete quantum gravity derivation (the thermodynamic derivation uses semiclassical horizon entropy; full quantum gravity remains open)
...
IAM is a \textit{physically motivated late-time framework} derived from horizon thermodynamics via the Jacobson--Cai-Kim formalism, with the activation function $\mathcal{E}(a) = \exp(1-1/a)$ derived from first principles, the coupling constant $\beta_m = \Omega_m/2$ derived from the virial theorem, and perturbation predictions ($\mu < 1$, $\Sigma = 1$) derived from the constrained scalar field condition $\delta\varphi = 0$. The result is a zero-parameter extension of $\Lambda$CDM with empirically testable predictions that unify multiple cosmological tensions.
...
\item \textbf{Zero free parameters:} The coupling $\beta_m = \Omega_m/2$ is derived from the virial theorem, matching MCMC to 0.3\%. With $\beta_m$ fixed at its derived value, $\Delta$AIC $= \Delta$BIC $= 31.2$ for zero additional parameters; $\Lambda$CDM is 6,000,000$\times$ less likely. Even treating $\beta_m$ as free ($\Delta$AIC = 27.2, $\Delta$BIC = 26.6), IAM is decisively preferred
...
\bibitem{Alam2021}
S.~Alam \textit{et al.} (eBOSS Collaboration),
\textit{Phys. Rev. D} \textbf{103}, 083533 (2021).
