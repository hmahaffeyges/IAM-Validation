...
The physical motivation for this identification is the following.
In IAM, the virial $1/2$ partition is not a free choice: it is a mathematical consequence of the $1/r$ potential, verified analytically for both Coulomb and gravitational interactions and confirmed computationally from atomic scales ($10^{-11}$\,m) to the cosmic horizon ($10^{26}$\,m) in Ref.~\cite{virial_paper}.
IAM interprets the two halves of this partition as physically distinct information channels: the kinetic half counts irreversible decoherence events (temporal encoding), while the potential half counts the configuration bits stored in spatial structure.
Assumption~\ref{ass:virialmap} applies this same partition to the boundary encoding amplitudes: $x^2$ plays the role of the configuration-like (constant offset) contribution and $y^2/2$ plays the role of the event-like (modulation) contribution, with their ratio fixed by the same $1/r$ universality.
This remains an assumption --- the mapping from gravitational virial physics to holographic encoding amplitudes is not derived here from first principles --- but it is grounded in a partition that holds without exception across every physical scale at which it has been tested.
...
For $n\ge 4$ we show $\min_k\cos\varphi_k \le -1/\sqrt{2}$ by considering two sub-cases.
\emph{$n$ even:} the phase $\varphi_{n/2}=\pi$ is always present, so $\min_k\cos\varphi_k = -1 < -1/\sqrt{2}$.
\emph{$n$ odd, $n\ge 5$:} the phase nearest $\pi$ is $\varphi_{(n-1)/2}=\pi(n-1)/n$.
Since $(n-1)/n \ge 4/5$ for all $n\ge 5$, we have $\varphi_{(n-1)/2}\ge 4\pi/5$, and $\cos(4\pi/5)=-\tfrac{1}{4}(1+\sqrt{5})\approx -0.809 < -1/\sqrt{2}\approx -0.707$.
The cosine is monotonically decreasing on $[0,\pi]$, so $\cos\varphi_{(n-1)/2}\le\cos(4\pi/5)<-1/\sqrt{2}$ for all odd $n\ge 5$.
Therefore
\[
...
\begin{table}[h]
\centering
\caption{Charged lepton masses and Koide ratio (PDG~2022 values~\cite{pdg2022}).}
\label{tab:pdg}
\begin{tabular}{@{}lll@{}}
\toprule
...
Foot and others emphasized geometric and phase-vector interpretations of Koide-like relations~\cite{foot1994}.
Brannen presented a phase form with $\{0,2\pi/3,4\pi/3\}$ phases~\cite{brannen2006}.
...
A natural question is whether an explicit entropy functional whose extremization yields both $y/x=\sqrt{2}$ and $n=3$ can be constructed.
...
\section{A possible route to deriving $y/x=\sqrt{2}$ from first principles}
\label{app:weyl}

One candidate approach would use a Weyl-squared gravitational entropy functional, which vanishes on exact FRW (since FRW is conformally flat, $C_{abcd}=0$) and is sourced only by perturbations, naturally aligning with the IAM perturbation-sector split.
A controlled derivation would require specifying a coarse-graining procedure on a spacelike hypersurface and evaluating the functional on a statistical ensemble of virialized halos; this is reserved for future work.
...
\bibitem{brannen2006}
C.~Brannen, ``The lepton masses,'' preprint (2006),
\url{http://brannenworks.com/MASSES2.pdf}.

\bibitem{pdg2022}
Particle Data Group, R.~L.~Workman et al., Prog.\ Theor.\ Exp.\ Phys.\ \textbf{2022}, 083C01 (2022).
