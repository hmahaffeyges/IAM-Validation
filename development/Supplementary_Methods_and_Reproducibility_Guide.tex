...
\begin{abstract}
This document provides complete code, data sources, and step-by-step instructions to independently reproduce all IAM validation results. The holographic horizon dynamics framework achieves 5.6$\sigma$ improvement over $\Lambda$CDM through dual-sector coupling: photon-sector H$_0 = 67.4$ km/s/Mpc (CMB, $\beta_\gamma < 1.4 \times 10^{-6}$) and matter-sector H$_0 = 72.7 \pm 1.0$ km/s/Mpc (local, $\beta_m = 0.164 \pm 0.029$). Model selection criteria (ΔAIC = 27.2, ΔBIC = 26.6) show no evidence of overfitting. All code executes in under 2 minutes on standard hardware. Complete theory and test results are presented in the companion Test Validation Compendium.
\end{abstract}
...
\begin{itemize}
    \item $\chi^2_{\Lambda\text{CDM}} = 41.63$ (10 data points)
    \item $\chi^2_{\text{IAM}} = 10.38$
    \item $\Delta\chi^2 = 31.25$ (5.6$\sigma$ improvement)
\end{itemize}

\textbf{Model Selection (Overfitting Check):}
\begin{itemize}
    \item ΔAIC = 27.2 → ``Decisive'' evidence for IAM (Burnham \& Anderson)
    \item ΔBIC = 26.6 → ``Very strong'' evidence for IAM (Kass \& Raftery)
    \item Relative likelihood: $\Lambda$CDM is 827,000$\times$ less likely than IAM
\end{itemize}

\textbf{Parameters (MCMC Posteriors):}
\begin{itemize}
    \item Matter-sector: $\beta_m = 0.164 \pm 0.029$ (68\% CL, MCMC)
...
\begin{itemize}
    ...
    \item H$_0$(matter/local) = 72.7 $\pm$ 1.0 km/s/Mpc
...
\begin{equation}
H_0(\text{matter}) = 67.4 \times \sqrt{1 + 0.157/(0.315 + 0.685)} = 72.5 \text{ km/s/Mpc}
\end{equation}
...
\subsection{DESI BAO + Growth Rate Data}
...
\bottomrule
\end{tabular}
\caption{DESI DR2 data (DESI Collaboration 2024, arXiv:2404.03002).}
\end{table}

\noindent\textbf{Data URL:} \url{https://data.desi.lbl.gov/public/dr2/}
...
\begin{lstlisting}[language=Python]
def compute_fsigma8(z_vals, beta, sigma8_0=0.811):
    """
    Compute f*sigma_8 observable for DESI comparison.
    
    Args:
        z_vals: Array of redshifts
        beta: Coupling parameter
        sigma8_0: Amplitude at z=0 (Planck value)
...
\begin{lstlisting}[language=Python]
def chi2_total(beta, h0_data, desi_data):
    """
    ...
    Args:
        ...
        desi_data: Array of [z, fsig8_obs, sigma]
    ...
    # Chi-squared for DESI
    z_desi = desi_data[:, 0]
    fsig8_obs = desi_data[:, 1]
    sig_desi = desi_data[:, 2]
    
    fsig8_pred = compute_fsigma8(z_desi, beta)
    chi2_desi = np.sum(((fsig8_pred - fsig8_obs) / sig_desi)**2)
    
    return chi2_h0 + chi2_desi, chi2_h0, chi2_desi
\end{lstlisting}
...
def log_probability(theta, h0_data, desi_data):
    ...
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(theta, h0_data, desi_data)

def run_mcmc(h0_data, desi_data, nwalkers=32, nsteps=5000, 
             burn_in=1000):
    """
    ...
    p0 = np.array([0.164, 3.3e-7])  # Initial guess
...
If \texttt{corner} is not available, the validation script will automatically attempt installation or fall back to a simplified 2×2 panel plot.
...
Reproduces main result:
  beta_m = 0.157 +/- 0.029 (68% CL)
  H0(matter) = 72.5 +/- 0.9 km/s/Mpc
  Delta chi^2 = 31.25 (5.6 sigma improvement over LCDM)
...
D_iam_today = D_iam(0.0)

suppression_pct = 100 * (1 - D_iam_today / D_lcdm_today)
print(f"  Growth suppression = {suppression_pct:.2f}%")

sigma8_eff = 0.811 * (D_iam_today / D_lcdm_today)
...
\begin{enumerate}
    \item[$\square$] $\beta_m = 0.164 \pm 0.029$ (68\% CL, MCMC)
    \item[$\square$] $\beta_\gamma < 1.4 \times 10^{-6}$ (95\% CL, MCMC)
    \item[$\square$] $\beta_\gamma/\beta_m < 8.5 \times 10^{-6}$ (95\% CL, MCMC)
    \item[$\square$] $H_0$(matter) = $72.7 \pm 1.0$ km/s/Mpc
    \item[$\square$] $\chi^2_{\Lambda\text{CDM}} = 41.63$
    \item[$\square$] $\chi^2_{\text{IAM}} = 10.38$
    \item[$\square$] $\Delta\chi^2 = 31.25$ (5.6$\sigma$)
    \item[$\square$] ΔAIC = 27.2 (decisive evidence, no overfitting)
    \item[$\square$] ΔBIC = 26.6 (very strong evidence)
    \item[$\square$] Growth suppression = 1.36\%
    \item[$\square$] $\sigma_8$(IAM) = 0.800
    \item[$\square$] $\Omega_m(z=0) = 0.272$
\end{enumerate}
...
\begin{itemize}
    \item \texttt{iam\_validation.py} — Complete validation script (this document)
...
\begin{quote}
Mahaffey, H. W. (2026). The Informational Actualization Model: Holographic Horizon Dynamics Couple Quantum Structure Formation to Cosmic Expansion. \textit{[Journal TBD]}.
\end{quote}
...
\begin{enumerate}
    \item DESI Collaboration (2024), arXiv:2404.03002
...
and \\
\textbf{The Informational Actualization Model: Holographic Horizon Dynamics} \\
\textbf{Couple Quantum Structure Formation to Cosmic Expansion} \\
