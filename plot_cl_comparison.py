...
IAM_CHAIN_STEM  = os.path.join(CHAIN_DIR_L2, "iam_level2_runA")
LCDM_CHAIN_STEM = os.path.join(CHAIN_DIR_L2, "iam_level2_runC_lcdm")
...
def extract_posterior_means(chain_stem):
    ...
    Cobaya chain columns: weight, -loglike, param1, param2, ...
    Parameter names are in the .updated.yaml file.
    """
    chain_file = chain_stem + ".1.txt"
    yaml_file  = chain_stem + ".updated.yaml"
    ...
    # Read the chain — first 2 columns are weight and -logpost
    data = np.loadtxt(chain_file)
    weights = data[:, 0]
    # log-posterior in column 1 (ignore)
    samples = data[:, 2:]   # parameter columns start at index 2

    # Read parameter names from the yaml
    param_names = read_param_names_from_yaml(yaml_file)
...
# =============================================================================
# STEP 3: Planck 2018 bandpower data
# Hard-coded from Planck 2018 Table 1 (Aghanim et al. 2020, A&A 641 A5)
# These are the published TT bandpowers (D_ell = ell(ell+1)C_ell/2pi in muK^2)
# =============================================================================

PLANCK_TT_BANDPOWERS = np.array([
    # ell_center, D_ell, sigma_low, sigma_high
    # Low-ell (2-29): Commander
    [  2,    -0.56,   3.04,   3.04],
    [  3,    21.68,   6.32,   6.32],
    [  4,    30.29,   6.25,   6.25],
    [  5,    22.86,   5.03,   5.03],
    ...
    [ 25,    29.22,   2.83,   2.83],
    [ 29,    21.87,   2.31,   2.31],
    # High-ell CamSpec (30-2508): binned
    [  30,   404.4,  25.0,  25.0],
    [  50,  2000.6,  48.5,  48.5],
    [  70,  3200.0,  60.0,  60.0],
    [ 100,  3427.0,  42.0,  42.0],
    [ 130,  3196.0,  37.0,  37.0],
    [ 160,  2681.0,  30.0,  30.0],
    [ 200,  3149.0,  28.0,  28.0],
    [ 250,  3716.0,  30.0,  30.0],
    [ 300,  4260.0,  31.0,  31.0],
    [ 350,  4022.0,  29.0,  29.0],
    [ 400,  3382.0,  25.0,  25.0],
    [ 450,  3460.0,  26.0,  26.0],
    [ 500,  3671.0,  26.0,  26.0],
    [ 550,  4016.0,  28.0,  28.0],
    [ 600,  3948.0,  26.0,  26.0],
    [ 650,  3511.0,  25.0,  25.0],
    [ 700,  3193.0,  23.0,  23.0],
    [ 750,  3327.0,  24.0,  24.0],
    [ 800,  3467.0,  24.0,  24.0],
    [ 850,  3531.0,  25.0,  25.0],
    [ 900,  3392.0,  24.0,  24.0],
    [ 950,  3131.0,  23.0,  23.0],
    [1000,  3005.0,  23.0,  23.0],
    [1100,  2780.0,  22.0,  22.0],
    [1200,  2650.0,  22.0,  22.0],
    [1400,  2348.0,  24.0,  24.0],
    [1600,  2000.0,  26.0,  26.0],
    [1800,  1700.0,  30.0,  30.0],
    [2000,  1414.0,  34.0,  34.0],
    [2200,  1173.0,  38.0,  38.0],
])
...
def make_cl_figure(ell_iam, cl_iam, ell_lcdm, cl_lcdm, outfile_stem):
    ...
    ax1.plot(ell_lcdm[lmin:], cl_lcdm[lmin:],
             color='#E07020', linewidth=1.5, linestyle='--',
             label=r'$\Lambda$CDM (Run C)', zorder=3, alpha=0.9)
    ax1.plot(ell_iam[lmin:], cl_iam[lmin:],
             color='#1B6CA8', linewidth=1.8,
    ...
    # Annotation box
    textstr = (r'$\Delta\chi^2 = +0.54$ (IAM vs $\Lambda$CDM, Planck)' + '\n'
               r'$\sigma_8: 0.809 \to 0.800$ (IAM suppression)' + '\n'
    ...
    ax2.text(0.02, 0.88,
             r'IAM residual $< 0.2\%$ at $\ell > 30$ (MGCAMB: 7/7 tests passed)',
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Falling back to hardcoded Run A / Run C posterior means from README")
        # Run A (IAM) posterior means from README
        params_iam = {
            'H0': 67.161, 'ombh2': 0.02217, 'omch2': 0.11994,
            'tau': 0.0537, 'logA': 3.0407, 'ns': 0.9630
        }
        # Run C (ΛCDM) posterior means from README
        params_lcdm = {
            'H0': 67.188, 'ombh2': 0.02218, 'omch2': 0.11989,
