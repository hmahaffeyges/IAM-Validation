...
- **Planck MCMC:** 3 independent chains converged cleanly (Delta-chi2 = +1.43 vs LCDM)
...
| **C: LCDM baseline** | 0 (fixed) | 0.8139 +/- 0.006 | 67.19 +/- 0.54 | 983.14 | 0 |

**Key results:**
- **Delta-chi2 = +1.43** between IAM fixed and LCDM (statistically indistinguishable at Planck precision)
- IAM's predicted mu_0 = -0.135 is **within 0.9 sigma** of Run B posterior mean (compatible but not confirmed at current precision)
- **sigma_8 shifted down** from 0.8139 to 0.8014 in Run A, direction favored by weak lensing surveys, while maintaining comparable likelihood
- When mu_0 floats, **Delta-chi2 = -1.90** (marginally better fit), statistically neutral after AIC penalty for one additional parameter
...
**Interpretation:** The present analysis evaluates Planck likelihood only. IAM remains statistically indistinguishable from LCDM under the full Planck 2018 likelihood (Delta-chi2 = +1.43 fixed; -1.90 floating), demonstrating internal consistency at current precision. The modification operates at z < 2 where Planck has limited sensitivity (only ISW and lensing). Joint likelihood with BAO, Pantheon+, SH0ES, and DESI full-shape data remains in progress. The definitive detection tests are Euclid (sigma(mu_0) ~ 0.04, yielding 3.4 sigma) and DESI Year 5 growth rates.
...
**Status:** 5.6 sigma preference for dual-sector cosmology over LCDM; zero free parameters -- all derived from first principles; MGCAMB 7/7 Boltzmann tests passed; Planck MCMC compatible (3 chains, Delta-chi2 = +1.43 vs LCDM, statistically indistinguishable) 
